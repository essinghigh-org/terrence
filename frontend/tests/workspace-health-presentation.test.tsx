...
const workspace = {
  id: "ws-health",
  attributes: { name: "production", "assessments-enabled": true, permissions: { "can-update": false } },
};
afterEach((): void => {
  cleanup();
...
test("health art distinguishes verified health, uncertain checks, and unavailable data", async (): Promise<void> => {
  ...
  await waitFor((): void => {
    expect(view.getByText("Everything healthy")).toBeTruthy();
  });
  ...
  await waitFor((): void => {
    expect(uncertain.getByText("completed")).toBeTruthy();
  });
  ...
  await waitFor((): void => {
    expect(unavailable.getByRole("alert").textContent).toContain("could not be loaded");
  });
