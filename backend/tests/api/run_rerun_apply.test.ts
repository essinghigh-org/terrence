...
describe("run detail lock fields + re-run (kanban 15.10 / 15.13)", () => {
  ...
  beforeAll(async () => {
    ...
    await db
      .insert(apiTokens)
      .values([{ id: tokenId, token: createHash("sha256").update(token).digest("hex"), userId: adminId }]);
    await db.insert(organizations).values([{ id: orgId, name: `rr-org-${suffix}`, email: "ops@example.com" }]);
    await db.insert(workspaces).values([{ id: workspaceId, name: "rr-workspace", orgId, autoApply: false }]);
  ...
  it("reflects a locked workspace in run detail", async () => {
    await db
      .update(workspaces)
      .set({ locked: true, lockedReason: "Maintenance window" })
