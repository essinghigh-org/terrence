...
function parseOrgLimitUpdate(
  ...
  if (
    attributes["workspace-limit"] !== null &&
    (typeof attributes["workspace-limit"] !== "number" || !Number.isInteger(attributes["workspace-limit"]))
  ) {
    (set as { status: number }).status = 422;
    return {
      error: {
        errors: [
          { status: "422", title: "Unprocessable Entity", detail: "workspace-limit must be an integer or null" },
        ],
      },
    };
  }
...
function parseOrgRoleUpdate(
  ...
  const roleId =
    typeof attributes["owners-team-saml-role-id"] === "string" ? attributes["owners-team-saml-role-id"].trim() : "";
...
function parseOrgUpdates(
  ...
  if (typeof attributes["global-module-sharing"] === "boolean")
    updates.globalModuleSharing = attributes["global-module-sharing"];
  if (typeof attributes["global-provider-sharing"] === "boolean")
    updates.globalProviderSharing = attributes["global-provider-sharing"];
...
export const orgsRoutes = new Elysia({ name: "admin-orgs" })
  ...
  .delete(
    "/api/v2/admin/organizations/:org_name",
    async ({
      params,
      user,
      set,
