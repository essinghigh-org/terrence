...
describe("Comprehensive Terrence End-to-End System Flow Test", () => {
  ...
  it("executes complete lifecycle: discovery, auth, orgs, projects, workspaces, varsets, runs, state, and audit logs", async () => {
    ...
    // Entitlements now require authentication (they disclose site capability
    // state), so the call moves after registration/login below.
    ...
    // Entitlements (now requires an authenticated caller)
    const entRes = await app.handle(new Request("http://localhost/api/v2/entitlements", { headers: authHeaders }));
