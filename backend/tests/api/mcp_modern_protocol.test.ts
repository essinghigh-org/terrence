...
function body(method: string, params: Readonly<Record<string, unknown>>, id: string | number, options: SendOptions): Record<string, unknown> {
...
async function send(method: string, params: Readonly<Record<string, unknown>> = {}, options: SendOptions = {}): Promise<Response> {
  ...
  if (options.headerVersion !== null) headers.set("MCP-Protocol-Version", options.headerVersion ?? MCP_PROTOCOL_VERSION);
  ...
  return app.handle(new Request("http://terrence.test/mcp", {
    method: "POST",
    headers,
...
describe("MCP 2026-07-28 modern protocol", () => {
  it("is POST-only and no longer exposes the legacy standalone SSE GET endpoint", async () => {
    const response = await app.handle(new Request("http://terrence.test/mcp", {
  ...
  it("returns deterministic annotated tools with private cache hints", async () => {
    ...
    expect(response.status).toBe(200);
    const payload = await response.json() as { result: { tools: { name: string; annotations: Record<string, boolean> }[]; ttlMs: number; cacheScope: string } };
  ...
  it("returns UnsupportedProtocolVersion with the supported revision", async () => {
    ...
    expect(response.status).toBe(400);
    const payload = await response.json() as { error: { code: number; data: { supported: string[]; requested: string } } };
