...
for (const engine of ["terraform", "tofu"] as const) {
  describe(`${engine} downloaded state integrity`, () => {
    ...
    it("reads inline, deferred, rollback and recovery bytes with matching serial, digest and outputs", async () => {
      ...
      const state = (serial: number, value: string): string =>
        JSON.stringify({
          version: 4,
          ...
          lineage: seed.suffix,
          outputs: {
            sample: { value, type: "string" },
            secret: { value: "SYNTHETIC_CLI_SECRET", type: "string", sensitive: true },
          },
          resources: [
            {
              mode: "managed",
      ...
      const create = async (serial: number, raw: string, inline: boolean): Promise<JsonApiResource> =>
        expectSuccessResponse(
      ...
      const verify = async (resource: JsonApiResource, value: string): Promise<void> => {
        ...
        expect((await indexed.json()).data.map((output: JsonApiResource) => output.attributes)).toEqual(
          expect.arrayContaining([
            expect.objectContaining({ name: "sample", value }),
      ...
      expect(
        (
          await request(deferred.attributes["hosted-state-upload-url"] as string, {
            method: "PUT",
            body: state(8, "newer"),
          })
        ).status,
      ).toBe(200);
      const uploaded = await expectSuccessResponse(
        await request(`/api/v2/state-versions/${deferred.id}`, { headers }),
        200,
        "state-versions",
      );
      await verify(uploaded, "newer");
      const rolledBack = await expectSuccessResponse(
        await request(`/api/v2/state-versions/${original.id}/actions/rollback`, { method: "POST", headers }),
        201,
        "state-versions",
      );
      ...
      const stale = await request(`/api/v2/workspaces/${workspaceId}/state-versions`, {
        method: "POST",
        headers,
        body: JSON.stringify({
          data: {
            ...
            attributes: {
              serial: 8,
              state: state(8, "stale"),
              md5: createHash("md5").update(state(8, "stale")).digest("hex"),
            },
          },
        }),
      });
