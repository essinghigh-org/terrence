...
import {
  ...
  githubAppSourceLabel,
  githubAppStorageNotice,
  type GitHubAppAttributes,
} from "../lib/github-app-admin";
  ...
  return (
    <div className="rounded-md border bg-muted/30 p-4 text-sm">
      ...
      {notice.canRemoveEnvironment && (
        <>
          <p className="mt-3">
            You may remove <code>GITHUB_APP_ID</code>, <code>GITHUB_APP_SLUG</code>, <code>GITHUB_APP_PRIVATE_KEY</code>
            , and <code>GITHUB_WEBHOOK_SECRET</code> from the deployment.
          </p>
          <p className="mt-2 text-muted-foreground">
            Keep the database and <code>STORAGE_DIR</code> persistent, and retain <code>ENCRYPTION_PASSWORD</code> if
            configured. Keep custom GitHub API/HTTP URLs for future setup. Removing deployment variables does not revoke
            the GitHub key.
          </p>
        </>
      )}
  ...
  return (
    <form
      ...
      onSubmit={(event): void => {
        ...
        void onCreate(organization, publicApp);
      }}
    >
      <p className="text-sm text-muted-foreground">
        {replacing
          ...
          : "Terrence fills in the URLs, events, and permissions. Review the app name on GitHub, create it, then grant repository access."}
      </p>
      <label className="block space-y-2 text-sm font-medium">
        ...
        <Input
          ...
          placeholder="e.g. essinghigh-org"
          autoComplete="off"
          disabled={busy}
        />
        <span className="block font-normal text-muted-foreground">
          Use the organization login to register the app there. Leave blank to register under your personal GitHub
          account.
        </span>
      </label>
      <label className="flex items-start gap-2 text-sm">
        <input
          ...
          checked={publicApp}
          onChange={(event): void => {
            setPublicApp(event.target.checked);
          }}
          disabled={busy}
          className="mt-1"
        />
        <span>
          Allow other GitHub accounts to install this app.
          <span className="block text-muted-foreground">
            Leave unchecked for an app restricted to its owning account. Enable this when replacing an app installed
      ...
      <Button type="submit" disabled={busy}>
        ...
        {busy ? "Please wait…" : replacing ? "Create replacement on GitHub" : "Create GitHub App"}
      </Button>
    </form>
  );
}
  ...
  return (
    ...
    >
      <p className="text-sm text-muted-foreground md:col-span-2">
        Use the credentials for an existing app. Terrence validates its identity before storing encrypted credentials.
        This replaces the stored credentials immediately; use automatic replacement above to migrate installations to a
        new app.
      </p>
      ...
      <label className="space-y-2 text-sm">
        <span>GitHub API URL (optional)</span>
        <Input
          value={apiUrl}
          onChange={(event): void => {
      ...
      <label className="space-y-2 text-sm">
        <span>GitHub HTTP URL (optional)</span>
        <Input
          value={httpUrl}
          onChange={(event): void => {
      ...
      <div className="md:col-span-2">
        <Button type="submit" variant="outline" disabled={busy}>
          Validate and save existing app
        </Button>
      </div>
    </form>
  );
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant={status === "active" ? "secondary" : "outline"}>{label}</Badge>
          {attributes.configured === true && (
            <span className="text-sm text-muted-foreground">{githubAppSourceLabel(attributes.source)}</span>
          )}
        </div>
        {invalidReason !== "" && (
          ...
          <>
            <dl className="grid gap-2 text-sm sm:grid-cols-3">
              <div>
                ...
                <dd>{attributes.owner ?? "Unknown"}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground">App slug</dt>
                <dd className="break-all">{attributes.slug ?? "—"}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground">App ID</dt>
                <dd>{attributes["app-id"] ?? "—"}</dd>
              </div>
            </dl>
        ...
        {status === "invalid" && (
          <p className="text-sm text-muted-foreground">
            Reconnect with corrected credentials or create a replacement under Advanced below.
          </p>
        )}
      ...
      <CardHeader>
        ...
        <CardDescription>
          {hasApp
            ...
            : "The app has been created. Grant repository access on GitHub to finish connecting it."}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {missingOwners.length > 0 && (
          <p className="text-sm">
            Still needs installation for: <strong>{missingOwners.join(", ")}</strong>.
          </p>
        )}
...
function AdvancedGitHubAppSettings({
  attributes,
  hasApp,
  pending,
  busy,
      ...
      <div className="mt-5 space-y-6">
        {hasRegistration && (
          <section className="space-y-3">
            ...
            {pending && (
              <p className="text-sm text-muted-foreground">
                Creating another app supersedes the unfinished setup. It does not delete that registration on GitHub.
              </p>
            )}
        ...
        <details className="border-t pt-4">
          <summary className="cursor-pointer text-sm font-medium">Use an existing GitHub App</summary>
          <ExistingAppForm busy={busy} onSave={onSave} />
        </details>
        <section className="space-y-3 border-t pt-4">
          <h2 className="text-sm font-semibold">Import deployment credentials</h2>
          <p className="text-sm text-muted-foreground">
            Import is a recovery action, not a connection check. It validates the environment credentials and replaces
            any stored app credentials.
          </p>
          {attributes["environment-import-available"] === true ? (
            ...
            <p className="text-sm text-muted-foreground">
              No complete GitHub App credential set is available in this process environment.
            </p>
          )}
        </section>
        {hasRegistration && (
          <section className="space-y-3 border-t pt-4">
            ...
            <p className="text-sm text-muted-foreground">
              Removes stored credentials, not the app on GitHub or workspace configuration. To delete the registration
              itself, use GitHub. Deleting it there uninstalls it everywhere.
            </p>
            <Button variant="destructive" disabled={busy} onClick={onDisconnect}>
              <Unplug data-icon="inline-start" />
              Disconnect from Terrence…
            </Button>
          </section>
        )}
      </div>
      ...
      {!hasApp && !pending && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Create a preconfigured app without copying private keys.</CardDescription>
          </CardHeader>
          <CardContent>
            <CreateAppForm busy={disabled} replacing={false} onCreate={create} />
          </CardContent>
        </Card>
      )}
      <AdvancedGitHubAppSettings
        attributes={attributes}
        hasApp={hasApp}
        pending={pending}
        busy={disabled}
...
export function AdminGitHubApp(): React.JSX.Element {
  ...
  useEffect((): void => {
    void load().catch((caught: unknown): void => {
  ...
  const startFlow = async (flow: "manifest" | "installation", organization = "", publicApp = false): Promise<void> => {
    ...
    try {
      ...
      const path =
        flow === "manifest" ? `/admin/github-app/manifest/setup?${query}` : "/admin/github-app/manifest/resume";
  ...
  const action = async (name: AppAction): Promise<boolean> => {
    ...
    try {
      ...
      setNotice(
        name === "validate"
          ? "GitHub accepted the app credentials."
          : name === "import-environment"
            ? "Environment credentials imported and encrypted in the database."
            : "Disconnected from Terrence. The GitHub registration and workspace configuration have not been deleted.",
      );
      return true;
    } catch (caught: unknown) {
  ...
  const saveManual = async (values: Readonly<Record<string, unknown>>): Promise<boolean> => {
    ...
    try {
      await fetchApi("/admin/github-app", {
        method: "POST",
        body: JSON.stringify({ data: { type: "github-app", attributes: values } }),
      });
      await load();
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        title="GitHub App"
        description="Connect GitHub, manage the app, and check where its credentials are stored."
      />
      {error !== "" && (
        <div role="alert" className="mb-4 rounded-md bg-destructive/15 p-4 text-sm text-destructive">
          {error}
        </div>
      )}
      {notice !== "" && (
        <div role="status" className="mb-4 rounded-md bg-primary/10 p-4 text-sm text-primary">
          {notice}
        </div>
      )}
      <GitHubAppContent
      ...
      <ConfirmDialog
        ...
        description="This removes stored credentials and pending setup. Workspace configuration and GitHub registrations are kept. A restart will not automatically re-import the old environment credentials."
        confirmText="Disconnect from Terrence"
        confirmVariant="destructive"
        requireCheckbox="I understand that GitHub-backed operations will stop until an app is connected again."
        loading={busy === "disconnect"}
        onConfirm={async (): Promise<void> => {
          if (await action("disconnect")) setDisconnectOpen(false);
        }}
      />
      <ConfirmDialog
        open={importOpen}
        ...
        title="Import GitHub App credentials from environment"
        description="This replaces stored credentials and any unfinished setup with the app configured in the deployment environment. It does not migrate installations to a different app."
        confirmText="Validate and import"
        requireCheckbox="I have checked that these are the credentials I want Terrence to use."
        loading={busy === "import-environment"}
        onConfirm={async (): Promise<void> => {
          if (await action("import-environment")) setImportOpen(false);
        }}
      />
    </PageShell>
  );
