...
async function organizationResourceForPrincipal(
  ...
  const [canManageOrganization, orgPermissionFlags, capabilities, costEnabled] = await Promise.all([
    ...
    checkOrganizationPermissionsMany(org.id, userId, tokenOrgId, tokenTeamId, [
      "manage-workspaces",
      "read-projects",
      "manage-projects",
      "manage-vcs-settings",
...
function reservedTagKeyInput(
  body: unknown,
): Readonly<{ key: string; disableOverrides: boolean }> | Readonly<{ error: string }> {
...
function parsePatchAttributes(body: unknown): Record<string, unknown> {
  ...
  if (data !== undefined && "type" in data && data["type"] !== undefined && data["type"] !== "organizations") {
    throw new OrgPatchError(422, {
      errors: [{ status: "422", title: "Unprocessable Entity", detail: "Invalid type" }],
    });
  }
...
function parseOrganizationPatch(attributes: Record<string, unknown>, org: OrgRow) {
  return {
    ...
    defaultTerraformVersion: patchStringOrCurrent(
      attributes["default-terraform-version"],
      org.defaultTerraformVersion ?? "latest",
    ),
    ...
    aggregatedCommitStatusEnabled: patchFlag(
      attributes["aggregated-commit-status-enabled"],
      org.aggregatedCommitStatusEnabled,
    ),
    sendPassingStatusesForUntriggeredSpeculativePlans: patchFlag(
      attributes["send-passing-statuses-for-untriggered-speculative-plans"],
      org.sendPassingStatusesForUntriggeredSpeculativePlans,
    ),
    moduleTestTokenTtl:
      attributes["module-test-token-ttl"] === undefined
    ...
    collaboratorAuthPolicy: patchCollaboratorAuthPolicy(
...
function assertPatchIdentity(fields: OrgPatchFields): void {
  if (
    fields.newName === "" ||
    !["tofu", "terraform"].includes(fields.defaultIacBinary) ||
    fields.defaultTerraformVersion.trim() === ""
  ) {
...
function assertPatchSettings(fields: OrgPatchFields): asserts fields is OrgPatchFields & {
  ...
  if (
    ...
    fields.sessionTimeout === undefined ||
    fields.sessionRemember === undefined ||
    fields.collaboratorAuthPolicy === undefined ||
    fields.defaultAgentPoolId === undefined ||
    typeof fields.defaultExecutionMode !== "string" ||
    (fields.defaultExecutionMode !== "remote" &&
      fields.defaultExecutionMode !== "local" &&
      fields.defaultExecutionMode !== "agent")
  ) {
    throw new OrgPatchError(422, {
      errors: [{ status: "422", title: "Unprocessable Entity", detail: "Invalid organization settings" }],
    });
  }
}

function assertPatchNaming(
  fields: OrgPatchFields,
  org: OrgRow,
): asserts fields is OrgPatchFields & {
  moduleTestTokenTtl: number;
  ownersTeamSamlRoleId: string | null;
} {
      ...
      errors: [
        {
          ...
          detail: `Module test token TTL must be between ${moduleTestTokenTtlBounds.min} and ${moduleTestTokenTtlBounds.max} seconds`,
        },
      ],
    });
  }
  ...
  if (fields.ownersTeamSamlRoleId === undefined) {
    throw new OrgPatchError(422, {
      errors: [
        { status: "422", title: "Unprocessable Entity", detail: "owners-team-saml-role-id must be a string or null" },
      ],
    });
  }
}

async function assertOwnersTeamSamlRoleId(
  ownersTeamSamlRoleId: string | null | undefined,
  orgId: string,
): Promise<void> {
        ...
        errors: [
          {
            status: "422",
            title: "Unprocessable Entity",
            detail: "owners-team-saml-role-id conflicts with an existing team name",
          },
        ],
      });
    }
...
async function assertDefaultAgentPool(defaultAgentPoolId: string | null | undefined, orgId: string): Promise<void> {
  if (defaultAgentPoolId !== null && defaultAgentPoolId !== undefined) {
    const pool = await db.query.agentPools.findFirst({
      where: and(eq(agentPools.id, defaultAgentPoolId), eq(agentPools.orgId, orgId)),
    });
    if (pool === undefined) {
      throw new OrgPatchError(422, {
        errors: [
          {
            status: "422",
            title: "Unprocessable Entity",
            detail: "default-agent-pool-id must reference an agent pool in this organization",
          },
        ],
      });
    }
...
function parseCreateAttributes(body: unknown): Record<string, unknown> {
  ...
  if (data?.["type"] !== "organizations") {
    throw new OrgPatchError(422, {
      errors: [{ status: "422", title: "Unprocessable Entity", detail: "data.type must be organizations" }],
    });
  }
  return typeof data?.["attributes"] === "object" && data["attributes"] !== null
    ? (data["attributes"] as Record<string, unknown>)
...
function parseCreateOrganizationFields(attributes: Record<string, unknown>) {
  return {
    ...
    defaultIacBinary:
      typeof attributes["default-iac-binary"] === "string" ? attributes["default-iac-binary"] : "terraform",
    defaultTerraformVersion:
      typeof attributes["default-terraform-version"] === "string"
        ? attributes["default-terraform-version"].trim()
        : "latest",
    ...
    collaboratorAuthPolicy:
      attributes["collaborator-auth-policy"] === undefined ? "password" : attributes["collaborator-auth-policy"],
    userTokensEnabled:
      attributes["user-tokens-enabled"] === undefined ? true : attributes["user-tokens-enabled"] === true,
  };
}
...
function assertCreateSettings(fields: CreateOrgFields): asserts fields is ValidatedCreateOrgFields {
  if (
    !["tofu", "terraform"].includes(fields.defaultIacBinary) ||
    fields.defaultTerraformVersion === "" ||
    fields.sessionTimeout === undefined ||
    fields.sessionRemember === undefined ||
    (fields.collaboratorAuthPolicy !== "password" && fields.collaboratorAuthPolicy !== "sso") ||
    fields.defaultExecutionMode === undefined
  ) {
    throw new OrgPatchError(422, { errors: [{ status: "422", title: "Unprocessable Entity" }] });
  }
}
...
async function insertOrganization(args: {
  ...
  if ((args.creator as unknown as Record<string, unknown>)["isProvisional"] === true) {
    throw new OrgPatchError(403, {
      errors: [{ status: "403", title: "Forbidden", detail: "Provisional accounts cannot create organizations" }],
    });
  }
...
function parseRetentionPolicyRequest(body: unknown): {
...
async function resolveRetentionPolicyValues(
  ...
  const stateVersionsCount =
    typeof attributes["state-versions-count"] === "number"
...
async function resolveOrganizationIds(user: ParamCtx["user"], orgId: string | null | undefined): Promise<string[]> {
  if (orgId !== null && orgId !== undefined) return [orgId];
  if (user?.isSiteAdmin === true) {
    return (await db.query.organizations.findMany({ columns: { id: true } })).map(
...
function intersectOrgScope(organizationIds: string[], scopes: ReturnType<typeof currentTokenScopes>): string[] {
...
async function resolveTagScope(
  ...
  const org = candidates
    .sort((left, right): number => right.name.length - left.name.length)
    .find((candidate): boolean => tagId.startsWith(`tag-${candidate.name}-`));
...
function parseWorkspaceTagIds(body: unknown): string[] {
  ...
  if (!Array.isArray(data)) {
    throw new OrgPatchError(422, {
      errors: [{ status: "422", title: "Unprocessable Entity", detail: "data must be an array" }],
    });
  }
  ...
  if (parsedIds.length !== data.length) {
    throw new OrgPatchError(422, {
      errors: [
        { status: "422", title: "Unprocessable Entity", detail: "data must contain workspaces resource identifiers" },
      ],
    });
  }
...
async function assertTagWorkspacesAssignable(orgId: string, tagKey: string, ids: string[]): Promise<void> {
  const organizationWorkspaceIds = (
    await db.query.workspaces.findMany({ where: eq(workspaces.orgId, orgId), columns: { id: true } })
  ).map((workspace): string => workspace.id);
  const existingTag =
    organizationWorkspaceIds.length === 0
      ? undefined
      : await db.query.workspaceTags.findFirst({
  ...
  if (targets.length !== ids.length || targets.some((workspace) => workspace.orgId !== orgId)) {
    throw new OrgPatchError(422, {
      errors: [{ status: "422", title: "Unprocessable Entity", detail: "workspaces must belong to the organization" }],
    });
  }
}
...
export const organizationRoutes = new Elysia({ name: "organizations" })
  ...
  .post(
    ...
    async ({ user, orgId: tokenOrgId, teamId: tokenTeamId, body, set }: ParamCtx): Promise<unknown> => {
      try {
  ...
  .get("/api/v2/organizations", async ({ user, orgId, request, set }: ParamCtx): Promise<unknown> => {
    ...
    return {
      data: await Promise.all(orgs.map(organizationResource)),
      ...pagination(request, number, size, totalCount),
    };
  })
  .get(
    "/api/v2/organizations/:org_name/reserved-tag-keys",
    async ({ params, user, orgId, request, set }: ParamCtx): Promise<unknown> => {
      ...
      return {
        data: await Promise.all(
          tags.map(async (tag: ReservedTagKey): Promise<Record<string, unknown>> => reservedTagKeyResource(tag)),
        ),
  ...
  .post(
    "/api/v2/organizations/:org_name/reserved-tag-keys",
    async ({ params, body, user, orgId, set }: ParamCtx): Promise<unknown> => {
      const orgName = params["org_name"] ?? "";
      const org = await cachedOrgByName(orgName);
  ...
  .patch(
    "/api/v2/reserved-tags/:reserved_tag_key_id",
    async ({ params, body, user, orgId, set }: ParamCtx): Promise<unknown> => {
  ...
  .delete(
    "/api/v2/reserved-tags/:reserved_tag_key_id",
    async ({ params, user, orgId, set }: ParamCtx): Promise<unknown> => {
      const tagId = params["reserved_tag_key_id"] ?? "";
      const tag = await db.query.reservedTagKeys.findFirst({ where: eq(reservedTagKeys.id, tagId) });
      if (tag === undefined || !(await checkOrgPermission(user?.id, tag.orgId, "owner", orgId))) {
        (set as { status: number }).status = 404;
  ...
  .get("/api/v2/organizations/:org_name", async ({ params, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
    ...
    if (
      org === undefined ||
      !(await checkOrgPermission(user?.id, org.id, "member", orgId ?? null, teamId ?? null, "settings:read"))
    ) {
      (set as { status: number }).status = 404;
      return { errors: [{ status: "404", title: "Not Found" }] };
    }
  ...
  .get(
    "/api/v2/organizations/:org_name/entitlement-set",
    async ({ params, user, orgId, run, set }: ParamCtx): Promise<unknown> => {
      const orgName = params["org_name"] ?? "";
      const org = await cachedOrgByName(orgName);
      const runScoped = run !== undefined && run !== null && org !== undefined && run.organizationId === org.id;
      if (
        org === undefined ||
        (!runScoped && !(await checkOrgPermission(user?.id, org.id, "member", orgId, null, "settings:read")))
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
  ...
  .get(
    "/api/v2/organizations/:org_name/relationships/data-retention-policy",
    async ({ params, user, orgId, set }: ParamCtx): Promise<unknown> => {
      const orgName = params["org_name"] ?? "";
      const org = await cachedOrgByName(orgName);
      if (org === undefined || !(await checkOrgPermission(user?.id, org.id, "member", orgId, null, "settings:read"))) {
  ...
  .post(
    ...
    async ({ params, body, user, orgId, set }: ParamCtx): Promise<unknown> => {
      ...
      try {
        ...
        const { existing, values } = await resolveRetentionPolicyValues(
          org.id,
          attributes,
          policyType,
          rawDeleteOlderThanNDays,
        ...
        if (existing === undefined) {
          ...
          await db
            .update(organizationDataRetentionPolicies)
            .set(values)
            .where(eq(organizationDataRetentionPolicies.id, existing.id));
        }
  ...
  .get(
    "/api/v2/organizations/:org_name/relationships/module-producers",
    async ({ params, user, request, orgId, set }: ParamCtx): Promise<unknown> => {
      ...
      const partnerships = await db.query.registryPartnerships.findMany({
        where: and(eq(registryPartnerships.consumerOrgId, org.id), eq(registryPartnerships.modules, true)),
      });
      const producerIds = [...new Set(partnerships.map((partnership): string => partnership.producerOrgId))];
      const producers =
        producerIds.length === 0
          ? []
          : await db.query.organizations.findMany({ where: inArray(organizations.id, producerIds) });
          ...
          : [
              {
                id: producer.name,
                type: "organizations",
                attributes: { name: producer.name, "external-id": producer.id },
                links: { self: `/api/v2/organizations/${producer.name}` },
              },
            ];
      });
      const { number, size } = pageRequest(request);
  ...
  .get(
    "/api/v2/organizations/:org_name/tags",
    async ({ params, user, request, orgId, set }: ParamCtx): Promise<unknown> => {
      const orgName = params["org_name"] ?? "";
      const org = await cachedOrgByName(orgName);
      ...
      let items = [...tagCounts.entries()].map(
        ([name, countVal]): {
          id: string;
          type: string;
          attributes: { name: string; "created-at": string; "instance-count": number };
          relationships: { organization: { data: { id: string; type: string } } };
        } => ({
  ...
  .delete(
    "/api/v2/organizations/:org_name/tags",
    async ({ params, user, body, orgId, set }: ParamCtx): Promise<unknown> => {
      const orgName = params["org_name"] ?? "";
      const org = await cachedOrgByName(orgName);
      if (org === undefined || !(await checkOrgPermission(user?.id, org.id, "owner", orgId))) {
        (set as { status: number }).status = 404;
      ...
      const tagIds = dataList
        .map((item) => (item as Record<string, unknown>)?.["id"])
        .filter((id): id is string => typeof id === "string");

      if (tagIds.length > 0) {
        const wsList = await db.query.workspaces.findMany({
        ...
        if (wsIds.length > 0) {
          ...
          await db
            ...
            .where(and(inArray(workspaceTags.workspaceId, wsIds), inArray(workspaceTags.key, tagKeys)));
        }
      }
      (set as { status: number }).status = 204;
      return {};
  ...
  .post(
    "/api/v2/tags/:tag_id/relationships/workspaces",
    async ({ params, user, body, orgId, set }: ParamCtx): Promise<unknown> => {
  ...
  .get(
    "/api/v2/organizations/:org_name/vcs-events",
    async ({ params, user, orgId, request, set }: ParamCtx): Promise<unknown> => {
      const orgName = params["org_name"] ?? "";
      const org = await cachedOrgByName(orgName);
      if (org === undefined || !(await checkOrgPermission(user?.id, org.id, "member", orgId, null, "settings:read"))) {
