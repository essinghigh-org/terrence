...
export const sshKeyRoutes = new Elysia({ name: "sshKeys" })
  .use(authPlugin)
  .get(
    "/api/v2/organizations/:org_name/ssh-keys",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const orgName = params["org_name"] ?? "";
      const org = await cachedOrgByName(orgName);
      if (
        org === undefined ||
        !(await checkOrganizationPermission(org.id, user?.id, tokenOrgId, tokenTeamId ?? null, "read-vcs-settings"))
      ) {
      ...
      if (strictAuditEnabled()) {
        await auditLog("read", "ssh-key", null, user?.id ?? null, org.id, {
      ...
      return {
        data: keyList.map(
          (k: { readonly id: string; readonly name: string }): Record<string, unknown> => ({
            id: k.id,
            type: "ssh-keys",
  ...
  .post(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (name === "" || value === "") {
        ...
        return { errors: [{ status: "422", title: "Unprocessable Entity", detail: "Name and value are required" }] };
      }
      const id = newResourceId("ssh");
      await db
        .insert(sshKeys)
        .values({ id, orgId: org.id, name, value: await encryptSecret(value), createdAt: Date.now() });
      if (strictAuditEnabled()) {
  ...
  .get(
    "/api/v2/ssh-keys/:ssh_key_id",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const sshKeyId = params["ssh_key_id"] ?? "";
      const key = await db.query.sshKeys.findFirst({ where: eq(sshKeys.id, sshKeyId) });
      if (
        key === undefined ||
        !(await checkOrganizationPermission(key.orgId, user?.id, tokenOrgId, tokenTeamId ?? null, "read-vcs-settings"))
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
  ...
  .patch(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (strictAuditEnabled()) {
        await auditLog("update", "ssh-key", sshKeyId, user?.id ?? null, key.orgId, {
          name: key.name,
          "value-replaced": attrs["value"] !== undefined,
        });
      }
