...
export function buildStateSummary(payload: string): StateSummary {
  ...
  const bounded = (value: unknown): string | null => typeof value === "string" && value.length <= 256 ? value : null;
  summary.lineage = bounded(state["lineage"]);
  summary.terraformVersion = bounded(state["terraform_version"]);
...
export function readStateSummary(raw: string | null, digest: string | null): StateSummary | null {
  ...
  try {
    ...
    if (typeof value["md5"] !== "string" || !/^[a-f0-9]{32}$/.test(value["md5"])) return null;
    for (const key of ["lineage", "terraformVersion"]) {
      if (value[key] !== null && (typeof value[key] !== "string" || value[key].length > 256)) return null;
    }
    if (value["stateVersion"] !== null && value["stateVersion"] !== 4) return null;
