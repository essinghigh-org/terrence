...
sqliteTest(
  "plain statements cannot observe an uncommitted async transaction",
  async () => {
...
sqliteTest(
  "nested async sqlite transactions roll back rejected savepoints",
  async () => {
    ...
    try {
      ...
      const outerAfterRow = await db.query.organizations.findFirst({
        where: eq(organizations.id, `${outerOrganizationId}-after`),
      });
