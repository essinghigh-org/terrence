...
function Form({
  initiallyDirty = true,
  secondSection = false,
}: Readonly<{ initiallyDirty?: boolean; secondSection?: boolean }>): React.JSX.Element {
  ...
  return (
    <div>
      ...
      <button
        onClick={() => {
          void navigate("/elsewhere", { replace: true });
        }}
      >
        Replace route
      </button>
        ...
        <UnsavedChangesProvider>
          <Routes>
            <Route
              path="/"
              element={
                <Form initiallyDirty={options.initiallyDirty ?? true} secondSection={options.secondSection ?? false} />
              }
            />
...
for (const control of ["Breadcrumb", "Change tab", "Switch organization", "Replace route"]) {
  test(`${control} stays put when declined and retries the intended transition when confirmed`, async () => {
    ...
    const leave = () =>
      fireEvent.click(
        view.getByRole(control === "Breadcrumb" || control === "Change tab" ? "link" : "button", { name: control }),
      );
    leave();
    await waitFor(() => {
    ...
    await waitFor(() => {
      expect(control === "Change tab" ? view.router.state.location.search : view.router.state.location.pathname).toBe(
        control === "Change tab" ? "?tab=other" : "/elsewhere",
      );
    });
  });
}
