...
describe("bounded committed state summary", () => {
  it("binds safe metadata to exact canonical bytes including whitespace and large numbers", async () => {
    const payload =
      '{ "version":4,"serial":1,"lineage":"test","terraform_version":"1.9.0","resources":[{"mode":"managed","instances":[{"attributes":{"secret":"never-public","number":9007199254740993}}]}],"outputs":{"secret":{"value":"never-public","sensitive":true}} }';
  ...
  it("bounds oversized metadata and identifies opaque and invalid representations", () => {
    const summary = buildStateSummary(
      JSON.stringify({ version: 4, lineage: "x".repeat(10000), terraform_version: "y".repeat(10000) }),
    );
