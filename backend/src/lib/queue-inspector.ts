...
import { db } from "../db";
import { agentJobs, agentPools, agents, runs, workspaces } from "../db/schema";
...
const POSITION_NOTE =
  "Estimate from the scheduler order at this snapshot; new claims, cancellations, locks and worker heartbeats can change it.";
...
function queuePhase(run: RunRow): "plan" | "apply" | null {
  if (run.status === "plan_queued" || run.status === "planning" || run.status === "pending" || run.status === "queuing")
    return "plan";
...
function withPosition(inspection: QueueInspection, position: number | null, qualified = true): QueueInspection {
  return {
    ...inspection,
    position,
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        or(lt(runs.createdAt, run.createdAt), and(eq(runs.createdAt, run.createdAt), lt(runs.id, run.id))),
      ),
    );
...
async function inspectPool(
  ...
  pool: QueueInspection["agentPool"];
  ...
  const claimedAgentIds = new Set(
    claimedJobs.map((job): string | null => job.agentId).filter((id): id is string => id !== null),
  );
...
async function agentJobPosition(
  job: Readonly<{ id: string; agentPoolId: string; iacBinary: string; createdAt: number }>,
): Promise<number | null> {
  const rows = await db
    .select({ count: sql<number>`count(*)` })
    .from(agentJobs)
    ...
    return withPosition(
      {
        ...
        reason:
          workspace.lockedReason === null || workspace.lockedReason === ""
    ...
    return withPosition(
      {
        ...
        reason: maintenance.reason ?? "Run claims are paused while maintenance mode is active.",
        phase,
        constraints: ["maintenance gate must be open"],
      },
      position,
    );
  }
    ...
    return withPosition(
      {
        ...
        reason: "The worker is draining and will not claim new runs.",
        phase,
        constraints: ["worker drain must be inactive"],
      },
      position,
    );
  }
    ...
    return withPosition(
      {
        ...base,
        state: "blocked",
        reasonCode: "storage-degraded",
        reason: `Run claims are paused: ${storageReason}`,
        phase,
        constraints: ["storage health must be normal"],
      },
      position,
    );
  }
    ...
    return withPosition(
      {
        ...
        reason:
          workspace.lockedReason === null || workspace.lockedReason === ""
            ? "The workspace is locked; this run waits for it to be unlocked."
            : `The workspace is locked: ${workspace.lockedReason}`,
        phase,
        constraints: ["workspace must be unlocked"],
        workspace: { ...base.workspace, serializationBlocked: false },
      },
      position,
    );
  }
    ...
    return withPosition(
      {
        ...poolBase,
        state:
          poolInspection.reasonCode === "agent-pool-scope" || poolInspection.reasonCode === "agent-pool-missing"
            ? "blocked"
            : "waiting",
        reasonCode: poolInspection.reasonCode,
        reason: poolInspection.reason ?? "The agent pool is not currently claimable.",
  ...
  return withPosition(
    {
      ...
      state: "ready",
      reasonCode: "agent-capacity-available",
      reason: "The worker can enqueue this run for a compatible agent on its next poll.",
    },
    position,
  );
}
    ...
    return withPosition(
      {
        ...base,
        state: "blocked",
        reasonCode: "local-execution-disabled",
        reason: "This workspace is configured for local execution; the server will not claim this remote run.",
        phase,
        constraints: ["workspace execution mode must not be local"],
      },
      position,
    );
  }
    ...
    return withPosition(
      {
        ...
        reason: `All ${limit} local execution slots are busy.`,
        phase,
        constraints: [`local executions below concurrency limit (${executing}/${limit})`],
      },
      position,
    );
  }
  return withPosition(
    {
      ...base,
      state: "ready",
      reasonCode: "local-capacity-available",
      reason: "A local execution slot is available; the worker can claim this run on its next poll.",
      phase,
      constraints: ["workspace serialization permits this run", "local execution slot available"],
    },
    position,
  );
}
    ...
    return withPosition(
      {
        ...base,
        state: "waiting",
        reasonCode: "workspace-serialization",
        reason: `Another ${serialization.status ?? "active"} run is using this workspace.`,
        phase: "plan",
        constraints: ["workspace serialization permits one blocking run"],
        workspace: { ...base.workspace, serializationBlocked: true },
      },
      position,
    );
  }
...
function confirmedScheduleBlock(base: QueueInspection, run: RunRow, now: number): QueueInspection | null {
  ...
  if (scheduledAt === null || scheduledAt === undefined) {
    return {
      ...base,
      state: "waiting",
      reasonCode: "awaiting-apply",
      reason: "The plan is confirmed and is waiting for an apply action.",
      phase: "apply",
      constraints: ["an apply action or schedule is required"],
    };
  }
  if (scheduledAt > now) {
    return {
      ...
      reason: "The scheduled apply time has not arrived.",
      phase: "apply",
      scheduledAt: iso(scheduledAt),
      constraints: ["scheduled-at must be in the past"],
    };
  }
  return null;
}
...
function confirmedLockBlock(base: QueueInspection, workspace: WorkspaceRow): QueueInspection | null {
  ...
  return {
    ...
    reason:
      workspace.lockedReason === null || workspace.lockedReason === ""
        ? "The scheduled apply is waiting for the workspace lock to clear."
        : `The workspace is locked: ${workspace.lockedReason}`,
    phase: "apply",
    constraints: ["workspace must be unlocked"],
  };
...
function confirmedAgentApply(
  ...
  return {
    ...base,
    state:
      poolInspection.reasonCode === null
        ? "ready"
        : poolInspection.reasonCode === "agent-pool-scope" || poolInspection.reasonCode === "agent-pool-missing"
          ? "blocked"
          : "waiting",
...
function confirmedLocalApply(
  ...
  if (
    context.localConcurrencyLimit !== undefined &&
    context.localExecuting !== undefined &&
    context.localExecuting >= context.localConcurrencyLimit
  ) {
    return {
      ...base,
      state: "waiting",
      reasonCode: "local-capacity",
      reason: `All ${context.localConcurrencyLimit} local execution slots are busy.`,
      phase: "apply",
      constraints: [
        `local executions below concurrency limit (${context.localExecuting}/${context.localConcurrencyLimit})`,
      ],
    };
  }
  return {
    ...
    state: "ready",
    reasonCode: "local-capacity-available",
    reason: "The scheduled apply is due and a local execution slot is available.",
    phase: "apply",
    constraints: ["apply gates must be open", "local execution slot available"],
  };
}
...
async function inspectConfirmedRun(
  ...
  const gateReason =
    context.applyGateReason === undefined ? await applyGateBlockReason(new Date(context.now)) : context.applyGateReason;
  if (gateReason !== null) {
    return {
      ...base,
      state: "waiting",
      reasonCode: "apply-gate",
      reason: gateReason,
      phase: "apply",
      constraints: ["approval, maintenance and storage apply gates must be open"],
    };
  }
  if (workspace.executionMode === "local") {
    return {
      ...
      state: "blocked",
      reasonCode: "local-execution-disabled",
      reason: "This workspace is configured for local execution; the server will not dispatch this scheduled apply.",
      phase: "apply",
      constraints: ["workspace execution mode must not be local"],
    };
  }
...
export async function inspectRunQueue(run: RunRow, options: QueueInspectorContext = {}): Promise<QueueInspection> {
  ...
  if (run.status === "plan_queued" || run.status === "apply_queued") {
    const job = await db.query.agentJobs.findFirst({
      where: and(
        eq(agentJobs.runId, run.id),
        eq(agentJobs.phase, run.status === "plan_queued" ? "plan" : "apply"),
        eq(agentJobs.status, "queued"),
      ),
    });
    if (job === undefined) {
      return {
        ...baseInspection(run, workspace, context),
        state: "unknown",
        reasonCode: "job-missing",
        reason: "The run is queued but its agent job is not present; worker reconciliation will decide its next state.",
      };
    }
  ...
  return {
    ...
    reason: "The scheduler has claimed this run and it is executing.",
    phase: queuePhase(run),
  };
}
...
export async function queueInspectorCapacity(now = Date.now()): Promise<Record<string, unknown>> {
  const [poolRows, agentRows, queuedRows, claimedRows] = await Promise.all([
    ...
    db.query.agents.findMany({
      columns: { id: true, agentPoolId: true, status: true, lastPingAt: true, iacBinaries: true },
    }),
    db.query.agentJobs.findMany({ where: eq(agentJobs.status, "queued"), columns: { agentPoolId: true } }),
    db.query.agentJobs.findMany({
      where: eq(agentJobs.status, "claimed"),
      columns: { agentPoolId: true, agentId: true },
    }),
  ]);
  const claimedByAgent = new Set(
    claimedRows.map((row): string | null => row.agentId).filter((id): id is string => id !== null),
  );
