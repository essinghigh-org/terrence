...
import { findAuthorizedRun, findAuthorizedWorkspace } from "../lib/utils";
...
function explainAuditContext(
  user: Readonly<{ readonly id: string }> | null | undefined,
  orgId: string | null | undefined,
): Readonly<{ userId: string | null; orgId: string | null }> {
...
function validPreflightProbes(raw: unknown): raw is string[] {
  return (
    ...
    raw.length <= 8 &&
    raw.every(
      (probe): probe is string => typeof probe === "string" && (probe === "connectivity" || probe === "identity"),
    )
  );
}
...
function preflightError(
  ...
  status: number,
  detail: string,
): Readonly<{ errors: readonly [{ status: string; title: string; detail: string }] }> {
...
async function resolveExplainerBackend(
  ...
  { backend: Exclude<Awaited<ReturnType<typeof resolvePlanExplainerSettings>>, null> } | { failure: unknown }
> {
  ...
  if (resolvedSettings === null) {
    ...
    return {
      failure: {
        errors: [{ status: "503", title: "Service Unavailable", detail: "Plan explainer is not fully configured" }],
      },
    };
  }
...
async function maybeStreamExplain(
  ...
  if (!refresh) {
    ...
    if (cachedStream !== undefined && cachedStream.content !== "") {
      return {
        response: cachedSseResponse(
          cachedStream.content,
          kind,
          cachedStream.model,
          reasoningEffort,
          cachedStream.createdAt,
        ),
      };
    }
    const dedupeKey = `${runId}:${kind}`;
  ...
  return {
    response: await streamExplainResponse(
      backend,
      source,
      runId,
      kind,
    ...
    if (cachedAfter !== undefined) {
      return {
        response: explanationResource(
          ...
          cachedAfter.model,
          reasoningEffort,
          new Date(cachedAfter.createdAt).toISOString(),
          true,
        ),
      };
    }
...
export const operationsRoutes = new Elysia({ name: "operations" })
  ...
  .get(
    "/api/v2/workspaces/:workspace_id/preflight",
  ...
  .post(
    ...
    async ({ params, body, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
      ...
      return preflightResource(
        await assessWorkspacePreflight(workspace, parsed.probes === undefined ? {} : { probes: parsed.probes }),
      );
    },
  )
  .post(
    "/api/v2/workspaces/:workspace_id/actions/preflight",
    async ({ params, body, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const workspace = await findAuthorizedWorkspace(
        params["workspace_id"] ?? "",
        user?.id,
  ...
  .get(
    "/api/v2/runs/:run_id/explain",
    async ({ params, user, orgId, teamId, set, request }: ParamCtx): Promise<unknown> => {
        ...
        errors: [
          {
            status: "404",
            title: "Not Found",
            detail: "No explanation has been requested for this run yet. POST to this endpoint to generate one.",
          },
        ],
      };
    },
  ...
  .post(
    "/api/v2/runs/:run_id/explain",
    async ({ params, body, user, orgId, teamId, set, request }: ParamCtx): Promise<unknown> => {
      const runId = params["run_id"] ?? "";
      ...
      const streamed = await maybeStreamExplain(
        streamRequested,
        refresh,
        runId,
        kind,
        ...
        backend.backend,
        loaded.source,
        explainAuditContext(user, orgId),
      );
...
function readExplainAttributes(body: unknown): Readonly<Record<string, unknown>> {
  ...
  const attributes =
    data !== null && typeof data === "object" ? (data as Record<string, unknown>)["attributes"] : undefined;
  ...
  return {
    data: {
      ...
      attributes: {
        ...
        ...(job.lastError !== null && job.lastError !== undefined && job.lastError !== ""
          ? { error: job.lastError }
          : {}),
      },
    },
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller: ReadableStreamDefaultController<Uint8Array>) {
      ...
      const controllerSignal = (
        controller as ReadableStreamDefaultController<Uint8Array> & { readonly signal?: AbortSignal }
      ).signal;
