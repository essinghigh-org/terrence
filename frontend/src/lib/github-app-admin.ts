...
export function githubAppSourceLabel(source: string | null | undefined): string {
  switch (source) {
    case "legacy_environment_import":
      return "Imported from environment";
    case "manifest":
      return "Created with GitHub";
    case "manual":
...
export function githubAppStorageNotice(attributes: GitHubAppAttributes): Readonly<{
  ...
  const canRemoveEnvironment = stored && attributes.status === "active" && attributes["environment-removable"] === true;
  if (canRemoveEnvironment)
    return {
      title: "Credentials saved in Terrence",
      description:
        "The private key and webhook secret are encrypted in the database. The four GitHub credential variables below are no longer required, including after a restart.",
      canRemoveEnvironment,
    };
  if (attributes["credential-storage"] === "environment")
    return {
      title: "Still using environment variables",
      description:
        "These credentials have not been saved in the database. Keep the GitHub credential variables until an import succeeds.",
      canRemoveEnvironment,
    };
  if (stored)
    return {
      ...
      description:
        "Terrence has a database record, but cannot confirm a complete, active credential set. Check or reconnect the app before removing deployment credentials.",
      canRemoveEnvironment,
    };
  return {
    title: attributes.status === "disconnected" ? "No active credentials stored" : "Credential storage not confirmed",
    description:
      "Connect an app or import the environment configuration. Do not remove deployment credentials until this page confirms they are saved.",
    canRemoveEnvironment,
  };
}
...
export function githubAppAuthorizationUrl(payload: unknown, origin: string, flow: "manifest" | "installation"): string {
