...
const request = (path: string, method = "GET", attributes?: Record<string, unknown>): Promise<Response> =>
  app.handle(
...
beforeAll(async () => {
  ...
  await db
    ...
    .values({ id: `membership-${suffix}`, userId, orgId, role: "owner", status: "active" });
...
test("artifact limits and deduplication are scoped before selecting tenant records", async () => {
  const first = await createPlatformArtifact({
  ...
  const otherWorkspace = await createPlatformArtifact({
    kind: "import-workbench",
    organizationId: orgId,
  ...
  const otherTenant = await createPlatformArtifact({
    kind: "import-workbench",
  ...
  await db
    ...
    .where(inArray(durableJobs.id, [otherWorkspace.id, otherTenant.id]));
  expect(
    (await listPlatformArtifacts({ kind: "import-workbench", organizationId: orgId, workspaceId, limit: 1 })).map(
      (row) => row.id,
    ),
  ).toEqual([first.id]);
...
test("user tokens can read and export their workbench but cannot read another tenant", async () => {
  const response = await request(`/workspaces/${workspaceId}/import-workbench`, "POST", {
    mappings: [{ address: "example_resource.web", "provider-id": "id-123" }],
  });
  ...
  const foreign = await createPlatformArtifact({
    kind: "import-workbench",
    organizationId: otherOrgId,
    workspaceId,
    payload: {},
...
test("unimplemented fleet execution leaves the preview intact and creates no runs", async () => {
  const preview = await createPlatformArtifact({
    kind: "fleet-operation",
    organizationId: orgId,
    status: "preview",
    payload: { manifest: { "target-ids": [workspaceId], action: "queue-plan", "selection-digest": "digest" } },
  });
  artifacts.push(preview.id);
  const response = await request(`/organizations/${orgId}/fleet-operations`, "POST", {
...
test("promotion advance cannot queue unverified release content", async () => {
  const response = await request(`/organizations/${orgId}/promotions`, "POST", {
    "configuration-digest": "unverified",
    "target-workspace-ids": [workspaceId],
  });
  expect(response.status).toBe(201);
  const body = (await response.json()) as { data: { id: string } };
  artifacts.push(body.data.id);
