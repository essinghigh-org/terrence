...
function CostEstimateGrid({ cost }: Readonly<{ cost: CostModel }>): React.JSX.Element {
  ...
  return (
    <dl
      aria-label="Cost estimate details"
      className="grid grid-cols-2 gap-4 border-t border-border px-5 py-4 text-sm md:grid-cols-4"
    >
      {!cost.costUnavailable && (
        <>
          <div>
            <dt className="text-xs text-muted-foreground">
              {cost.costBaselineComparable ? `Prior ${cost.costTimeBasis}` : "Baseline"}
            </dt>
            <dd className="mt-1 font-medium">
              {cost.costBaselineComparable
                ? formatMonthlyCost(costAttributes["prior-monthly-cost"], cost.costCurrency, cost.costTimeBasis)
                : "Not comparable"}
            </dd>
          </div>
          <div>
            ...
            <dd className="mt-1 font-medium">
              {formatMonthlyCost(costAttributes["proposed-monthly-cost"], cost.costCurrency, cost.costTimeBasis)}
            </dd>
          </div>
          {cost.costBaselineComparable && (
            <div>
              ...
              <dd className="mt-1 font-medium">
                {formatMonthlyCost(costAttributes["delta-monthly-cost"], cost.costCurrency, cost.costTimeBasis)}
              </dd>
            </div>
          )}
          <div>
      ...
      {((costAttributes["error-message"] !== null && costAttributes["error-message"] !== undefined) ||
        cost.costUnavailable) && (
        <div
          className={cost.costUnavailable ? "col-span-full text-muted-foreground" : "col-span-full text-destructive"}
        >
          {costAttributes["error-message"] ?? "Cost estimation is not installed in this image."}
        </div>
      )}
    </dl>
  );
...
function CostProvenanceLine({ cost }: Readonly<{ cost: CostModel }>): React.JSX.Element | null {
  ...
  return (
    <p className="border-t border-border px-5 py-3 text-xs text-muted-foreground">
      Pricing provenance: {costProvenance.tool ?? "estimator"}
      {costProvenance.version === null || costProvenance.version === undefined ? "" : ` ${costProvenance.version}`}
      {costProvenance.currency === null || costProvenance.currency === undefined ? "" : ` · ${costProvenance.currency}`}
      {costProvenance["time-basis"] === null || costProvenance["time-basis"] === undefined
        ? ""
        : ` · ${costProvenance["time-basis"]}`}
      {costProvenance["pricing-date"] === null || costProvenance["pricing-date"] === undefined
        ? ""
        : ` · pricing ${costProvenance["pricing-date"]}`}
    </p>
  );
}

function CostIncreaseRow({
  change,
  costCurrency,
  costTimeBasis,
}: Readonly<{
  ...
  return (
    <li key={`${moduleName}:${address}`}>
      <a href="#plan-heading" className="font-mono text-primary underline-offset-2 hover:underline">
        {moduleName}:{address}
      </a>
      <span className="ml-2">
        +{formatMonthlyCost(change["delta-monthly-cost"] ?? undefined, costCurrency, costTimeBasis)}
      </span>
    </li>
  );
}
    ...
    <section aria-labelledby="cost-heading" className="overflow-hidden rounded-lg border border-border bg-card">
      <div className="flex items-center justify-between gap-4 px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <h3 id="cost-heading" className="font-semibold text-foreground">
            Cost estimation
          </h3>
        </div>
        <Badge variant={cost.costFailed ? "destructive" : "secondary"} className="rounded capitalize">
          {cost.costStatus}
        </Badge>
      </div>
      {cost.costAttributes !== undefined && <CostEstimateGrid cost={cost} />}
      ...
      {cost.costComparison?.baseline?.comparable === false &&
        cost.costComparison.baseline.reason !== null &&
        cost.costComparison.baseline.reason !== undefined && (
          <p className="border-t border-border px-5 py-3 text-xs text-muted-foreground">
            Baseline comparison is unavailable: {cost.costComparison.baseline.reason}
          </p>
        )}
