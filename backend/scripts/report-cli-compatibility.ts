...
import { assertLifecycleEvidence, type LifecycleContract, type LifecycleEvidence } from "../src/lib/provider-lifecycle";
...
export function compatibilityPage(input: readonly unknown[]): string {
  ...
  const reports = input.map((raw): Report => {
    ...
    try {
      ...
      throw new Error(
        `Invalid provider lifecycle contract evidence: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    for (const tier of ["floor", "current"] as const) {
      ...
      const report = reports
        .filter(
          (item): boolean =>
            item.engine === engine &&
            item.versions.terraform_version === version &&
            item.database === "sqlite" &&
            item.sandbox === "disabled" &&
            item.tier === tier,
        )
      ...
      const provider =
        report.versions.provider_selections?.[
          engine === "terraform" ? "registry.terraform.io/hashicorp/tfe" : "registry.opentofu.org/hashicorp/tfe"
        ];
      if (provider !== providerVersion) throw new Error(`Untracked provider in ${engine} ${version} evidence`);
      const hasClaim = (resource: string, behaviors: readonly string[]): boolean =>
        report.claims.some(
          (claim): boolean =>
            claim.resources.includes(resource) &&
            behaviors.every((behavior): boolean => claim.behaviors.includes(behavior)),
        );
      for (const resource of [
        ...
        if (!hasClaim(resource, ["create", "read", "two-unchanged-plans", "unchanged-apply", "destroy"]))
          throw new Error(`Missing ${resource} lifecycle proof for ${engine} ${version}`);
      }
      if (
        !hasClaim("terraform_data.probe", [
          "remote-init",
          "remote-plan",
        ...
        !hasClaim("tfe_variable_set.vs", [
          "set-description",
          "clear-description",
          "restore-description",
          "unchanged-plan-after-each-update",
        ]) ||
        !hasClaim("tfe_team.team", ["import-minimal-config", "unchanged-plan-after-import"])
      )
        throw new Error(`Incomplete CLI journey for ${engine} ${version}`);
      rows.push(
        `| ${engine === "terraform" ? "Terraform" : "OpenTofu"} | ${tier} | ${version} | ${new Date(report.completedAt).toISOString()} | \`${report.binarySha256}\` |`,
      );
    }
  }
  return `---
...
if (import.meta.main) {
  ...
  if (directory === undefined || output === undefined)
    throw new Error("Usage: report-cli-compatibility.ts RESULTS_DIR OUTPUT.md");
  const files = (await readdir(directory)).filter((name): boolean => name.endsWith("-lifecycle.json"));
  const reports = await Promise.all(
    files.map(async (name): Promise<unknown> => JSON.parse(await readFile(join(directory, name), "utf8")) as unknown),
  );
