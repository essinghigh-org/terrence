    ...
    choices: [
      {
        message: {
          ...
          content: "<think>private reasoning</think>Brief answer.<reasoning>more reasoning</reasoning>",
        },
      },
    ],
  });
...
test("keeps inline thinking out of streamed content across chunk boundaries", async () => {
  ...
  const chunks = [`<think`, `>private reasoning</think>Brief answer.`].map((content) =>
  ...
  expect(
    events
      .filter(({ channel }) => channel === "thinking")
      .map(({ text }) => text)
      .join(""),
...
test("plan explanation prompts use the public projection", async () => {
  ...
  try {
    await writePlanJsonArtifact(runId, {
      variables: { password: { value: "REVIEW_SYNTHETIC_SECRET" } },
      resource_changes: [
        {
          address: "test.example",
          change: { actions: ["create"], after: "REVIEW_SYNTHETIC_SECRET", after_sensitive: true },
        },
      ],
    });
