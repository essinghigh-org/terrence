...
function stackResource(stack: StackItem, _projectName: string | null): Record<string, unknown> {
  ...
  relationships["agent-pool"] = {
    data: stack.agentPoolId === null ? null : { id: stack.agentPoolId, type: "agent-pools" },
  };
  relationships["stack-configurations"] = { links: { related: `/api/v2/stacks/${stack.id}/stack-configurations` } };
  relationships["stack-configuration-summaries"] = {
    links: { related: `/api/v2/stacks/${stack.id}/stack-configuration-summaries` },
  };
...
function stackVcsRepoAttributes(attributes: Record<string, unknown>): StackVcsAttributes {
  ...
  if (vcs === null || typeof vcs !== "object" || Array.isArray(vcs)) {
    return {
      vcsIdentifier: null,
      vcsServiceProvider: null,
      vcsBranch: null,
      vcsTagsRegex: null,
      vcsDisplayIdentifier: null,
      vcsRepositoryHttpUrl: null,
      vcsSparseCheckoutPattern: null,
      vcsOAuthTokenId: null,
      vcsGhaInstallationId: null,
      triggerDisabled: attributes["trigger-disabled"] === true,
    };
  }
  ...
  const serviceProvider =
    typeof repo["service-provider"] === "string"
...
const stackServiceProviders = new Set([
  "github",
  "github_enterprise",
  "gitlab_hosted",
...
async function validStackVcs(vcs: StackVcsAttributes, orgId: string): Promise<string | null> {
  if (vcs.vcsServiceProvider !== null && !stackServiceProviders.has(vcs.vcsServiceProvider))
    return "Invalid Stack VCS service provider";
  if (vcs.vcsRepositoryHttpUrl !== null && !validRepositoryHttpUrl(vcs.vcsRepositoryHttpUrl))
  ...
  if (vcs.vcsOAuthTokenId !== null && vcs.vcsGhaInstallationId !== null)
    return "oauth-token-id and github-app-installation-id are mutually exclusive";
  ...
  if (vcs.vcsGhaInstallationId !== null) {
    const installation = await db.query.githubAppInstallations.findFirst({
      where: and(eq(githubAppInstallations.id, vcs.vcsGhaInstallationId), eq(githubAppInstallations.orgId, orgId)),
    });
...
async function enqueueStackConfiguration(configuration: StackRecordItem): Promise<void> {
  const stack = await db.query.stacks.findFirst({
    where: eq(stacks.id, configuration.stackId),
    columns: { orgId: true },
  });
...
async function stackDetails(
  stackId: string,
): Promise<{ stack: StackItem; orgName: string; projectName: string | null } | undefined> {
...
function stackConfigurationResource(
  record: StackRecordItem,
  payload: Record<string, unknown>,
  timestamps: StackRecordTimestamps,
  ...
  return {
    ...
    relationships: {
      ...
      "stack-deployment-groups": {
        links: { related: `/api/v2/stack-configurations/${record.id}/stack-deployment-groups` },
      },
    },
    links: {
      self: `/api/v2/stack-configurations/${record.id}`,
      "json-schemas": `/api/v2/stack-configurations/${record.id}/json-schemas`,
    },
    meta: { beta: false },
  };
}

function stackDeploymentGroupResource(
  record: StackRecordItem,
  payload: Record<string, unknown>,
  timestamps: StackRecordTimestamps,
  approval: StackRecordApproval,
): Record<string, unknown> {
  return {
    id: record.id,
    type: record.recordType,
    attributes: {
      ...
      ...timestamps,
      name: record.name,
      "deployment-group-config": payload["deployment-group-config"] ?? { "auto-approve-checks": [] },
    },
    relationships: {
      "stack-configuration": {
        data: record.parentId === null ? null : { id: record.parentId, type: "stack-configurations" },
      },
      "stack-approvals": { data: approval === null ? [] : [approval] },
      "stack-deployment-runs": {
        links: { related: `/api/v2/stack-deployment-groups/${record.id}/stack-deployment-runs` },
      },
    },
    links: {
      self: `/api/v2/stack-deployment-groups/${record.id}`,
      "stack-deployment-group-summaries":
        record.parentId === null
          ? null
          : `/api/v2/stack-configurations/${record.parentId}/stack-deployment-group-summaries`,
    },
  };
}

function stackDeploymentRunResource(
  record: StackRecordItem,
  payload: Record<string, unknown>,
  timestamps: StackRecordTimestamps,
  approval: StackRecordApproval,
  ...
  return {
    ...
    attributes: {
      ...
      ...timestamps,
      "plan-mode": payload["plan-mode"] ?? "normal",
      component: payload["component"] ?? null,
      "component-index": payload["componentIndex"] ?? 0,
      cycle: payload["cycle"] ?? 0,
      destroy: payload["destroy"] === true,
      "lock-acquired": payload["lockAcquired"] === true,
      error: payload["error"] ?? null,
    },
    relationships: {
      "stack-deployment-group": {
        data: record.parentId === null ? null : { id: record.parentId, type: "stack-deployment-groups" },
      },
      "stack-configuration": {
        data:
          typeof payload["configurationId"] === "string"
            ? { id: payload["configurationId"], type: "stack-configurations" }
            : null,
      },
      "stack-deployment-steps": {
        links: { related: `/api/v2/stack-deployment-runs/${record.id}/stack-deployment-steps` },
      },
...
function stackDeploymentStepResource(
  record: StackRecordItem,
  payload: Record<string, unknown>,
  timestamps: StackRecordTimestamps,
  approval: StackRecordApproval,
  ...
  return {
    ...
    attributes: {
      status: record.status,
      "operation-type": payload["operation-type"] ?? "plan",
      phase: payload["phase"] ?? null,
      "component-index": payload["componentIndex"] ?? 0,
      "requires-state-lock": payload["requires-state-lock"] === true,
      "has-changes": payload["has-changes"] === true || payload["hasChanges"] === true,
      "deferred-changes": payload["deferred-changes"] === true || payload["deferredChanges"] === true,
      output: payload["output"] ?? null,
      ...timestamps,
    },
    relationships: {
      "stack-deployment-run": {
        data: record.parentId === null ? null : { id: record.parentId, type: "stack-deployment-runs" },
      },
      "stack-diagnostics": {
        links: { related: `/api/v2/stack-deployment-steps/${record.id}/stack-diagnostics` },
        meta: { count: 0 },
      },
      "stack-approval": { data: approval },
    },
    links: {
      self: `/api/v2/stack-deployment-steps/${record.id}`,
      "plan-description": `/api/v2/stack-deployment-steps/${record.id}/artifacts?name=plan-description`,
    },
  };
}
...
function stackStateResource(record: StackRecordItem, payload: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    relationships: {
      stack: { data: { id: record.stackId, type: "stacks" } },
      "stack-deployment-run": {
        data: typeof payload["runId"] === "string" ? { id: payload["runId"], type: "stack-deployment-runs" } : null,
      },
    },
...
function stackDiagnosticResource(record: StackRecordItem, payload: Record<string, unknown>): Record<string, unknown> {
  return {
    ...
    attributes: {
      ...
      diags: payload["diags"] ?? null,
      acknowledged: payload["acknowledged"] === true,
      "acknowledged-at": payload["acknowledged-at"] ?? null,
...
function stackApprovalResource(record: StackRecordItem, payload: Record<string, unknown>): Record<string, unknown> {
  return {
    id: record.id,
    type: record.recordType,
    attributes: { reason: payload["reason"] ?? null, "created-at": recordDate(record.createdAt) },
    relationships: {
      user: { data: typeof payload["userId"] === "string" ? { id: payload["userId"], type: "users" } : null },
    },
  };
}
...
function stackRecordResource(record: StackRecordItem): Record<string, unknown> {
  ...
  const approval =
    typeof payload["approvalId"] === "string" ? { id: payload["approvalId"], type: "stack-approvals" } : null;
  const timestamps = {
    "created-at": recordDate(record.createdAt),
  ...
  if (record.recordType === "stack-deployment-groups")
    return stackDeploymentGroupResource(record, payload, timestamps, approval);
  if (record.recordType === "stack-deployment-runs")
    return stackDeploymentRunResource(record, payload, timestamps, approval);
  if (record.recordType === "stack-deployment-steps")
    return stackDeploymentStepResource(record, payload, timestamps, approval);
...
async function authorizedStackRecord(
  recordId: string,
  user: ParamCtx["user"],
  tokenOrgId: string | null | undefined,
  teamId: string | null | undefined,
  expectedType?: string,
): Promise<{ record: StackRecordItem; details: NonNullable<Awaited<ReturnType<typeof stackDetails>>> } | undefined> {
...
function pagedStackRecords(
  records: StackRecordItem[],
  request: ParamCtx["request"],
): { data: Record<string, unknown>[]; pagination: Record<string, unknown> } {
  const { number, size } = pageRequest(request);
  return {
    data: records.slice((number - 1) * size, number * size).map(stackRecordResource),
    pagination: pagination(request, number, size, records.length),
  };
}

async function approveStackRecord(
  record: StackRecordItem,
  ...
  await db.transaction(async (tx): Promise<void> => {
    ...
    const approvedRecord = await tx
      ...
      .returning({ id: stackRecords.id });
    if (approvedRecord.length === 0) return;
    if (record.recordType === "stack-deployment-groups") {
      const runs = await tx.query.stackRecords.findMany({
        where: and(eq(stackRecords.parentId, record.id), eq(stackRecords.recordType, "stack-deployment-runs")),
      });
      for (const run of runs) {
        ...
        const approved = await tx
          ...
          .where(and(eq(stackRecords.id, run.id), notInArray(stackRecords.status, ["succeeded", "failed", "canceled"])))
          .returning({ id: stackRecords.id });
        if (approved.length === 1) runIds.push(run.id);
      }
    } else if (
      record.recordType === "stack-deployment-runs" &&
      !["succeeded", "failed", "canceled"].includes(record.status)
    ) {
      runIds.push(record.id);
    }
  });
  for (const runId of runIds)
    await enqueueStackDeployment(runId, organizationId, `stack-run:${runId}:approval:${approvalId}`);
}

async function createStackConfigurationRecord(
  stack: StackItem,
  source: string,
  attributes: Record<string, unknown>,
): Promise<StackRecordItem | undefined> {
  return db.transaction(async (tx): Promise<StackRecordItem | undefined> => {
    const configurations = await tx
      .select()
      .from(stackRecords)
      .where(and(eq(stackRecords.stackId, stack.id), eq(stackRecords.recordType, "stack-configurations")))
      .orderBy(desc(stackRecords.createdAt));
    const latest = configurations[0];
    if (source === "reuse" && (latest === undefined || ["pending", "preparing"].includes(latest.status)))
      return undefined;
    const latestPayload = latest?.payload ?? {};
    const sequence =
      configurations.length === 0 ? 1 : Number(latestPayload["sequence-number"] ?? configurations.length) + 1;
    const record: typeof stackRecords.$inferInsert = {
      ...
      payload: {
        ...
        components: [],
        archivePath:
          source === "reuse" && typeof latestPayload["archivePath"] === "string" ? latestPayload["archivePath"] : null,
      },
      createdAt: Date.now(),
      updatedAt: Date.now(),
    };
...
function parseStackPatchDocument(body: unknown): {
  attributes: Record<string, unknown>;
  ...
  const relationships =
    data !== null &&
    typeof data === "object" &&
    (data as Record<string, unknown>)["relationships"] !== null &&
    typeof (data as Record<string, unknown>)["relationships"] === "object"
      ? ((data as Record<string, unknown>)["relationships"] as Record<string, unknown>)
      : {};
  return { attributes, relationships };
}

function applyStackScalarUpdates(
  attributes: Record<string, unknown>,
  ...
  if (typeof attributes["speculative-enabled"] === "boolean")
    updates.speculativeEnabled = attributes["speculative-enabled"];
  ...
  if (Array.isArray(attributes["trigger-patterns"]))
    updates.triggerPatterns = (attributes["trigger-patterns"] as unknown[]).filter(
...
async function validateStackAgentPool(nextPoolId: string | null, orgId: string): Promise<string | null> {
  ...
  const pool = await db.query.agentPools.findFirst({
    where: and(eq(agentPools.id, nextPoolId), eq(agentPools.orgId, orgId)),
  });
...
async function applyStackExecutionUpdates(
  ...
  if (
    attributes["execution-mode"] !== undefined &&
    attributes["execution-mode"] !== "remote" &&
    attributes["execution-mode"] !== "agent"
  ) {
  ...
  const nextMode =
    typeof attributes["execution-mode"] === "string"
...
function parseStackCreateFields(data: Record<string, unknown>): StackCreateFields {
  ...
  const rels =
    relationships !== null && typeof relationships === "object" ? (relationships as Record<string, unknown>) : {};
  ...
  return {
    ...
    executionMode:
      attrs["execution-mode"] === undefined
        ? agentPoolId === undefined
          ? "remote"
          : "agent"
        : attrs["execution-mode"],
    speculative: attrs["speculative-enabled"] === true,
    triggerPatterns: Array.isArray(attrs["trigger-patterns"])
      ? (attrs["trigger-patterns"] as unknown[]).filter((item): item is string => typeof item === "string")
      : [],
  };
}
...
async function authorizeStackProject(
  ...
  if (
    project === undefined ||
    !(await checkOrganizationPermission(project.orgId, userId, tokenOrgId, teamId, "manage-projects"))
  ) {
    return { status: 404 };
  }
...
function buildStackInsert(
  ...
  return {
    ...
    agentPoolId: fields.agentPoolId ?? null,
    executionMode,
    name: fields.name,
    description: fields.description === "" ? null : fields.description,
    speculativeEnabled: fields.speculative,
    triggerDisabled: vcs.triggerDisabled,
    debuggingMode: fields.attrs["debugging-mode"] === true,
    workingDirectory: fields.workingDirectory ?? null,
    triggerPatterns: fields.triggerPatterns,
    vcsIdentifier: vcs.vcsIdentifier,
    vcsServiceProvider: vcs.vcsServiceProvider,
    vcsBranch: vcs.vcsBranch,
    vcsTagsRegex: vcs.vcsTagsRegex,
    vcsDisplayIdentifier: vcs.vcsDisplayIdentifier,
    vcsRepositoryHttpUrl: vcs.vcsRepositoryHttpUrl,
    vcsSparseCheckoutPattern: vcs.vcsSparseCheckoutPattern,
    vcsOAuthTokenId: vcs.vcsOAuthTokenId,
    vcsGhaInstallationId: vcs.vcsGhaInstallationId,
    createdAt: now,
    updatedAt: now,
  };
}
...
function stackExecutionModeError(executionMode: unknown, agentPoolId: string | undefined): string | null {
  ...
  if (executionMode === "agent" && agentPoolId === undefined)
    return "agent execution requires an agent-pool relationship";
  return null;
}
...
function validateStackConfigurationRequest(
  source: string,
  vcsIdentifier: string | null,
  attrs: Record<string, unknown>,
): string | null {
...
async function loadUploadableRecord(
  ...
  if (
    ...
    !validSignedApiURL(request, `/api/v2/stack-configurations/${recordId}/upload`, "PUT")
  ) {
    return { error: { status: 404 } };
  }
  const record =
    authorized?.record ??
    (await db.query.stackRecords.findFirst({
      where: and(eq(stackRecords.id, recordId), eq(stackRecords.recordType, "stack-configurations")),
    }));
...
export const stackRoutes = new Elysia({ name: "stacks" })
  ...
  .post("/api/v2/stacks", async ({ body, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
    ...
    if (data === null || typeof data !== "object") {
      ...
      return { errors: [{ status: "422", title: "Unprocessable Entity", detail: "data is required" }] };
    }
    const fields = parseStackCreateFields(data as Record<string, unknown>);
    const { projectId, agentPoolId, executionMode } = fields;
    if (fields.name === "" || projectId === "") {
      (set as { status: number }).status = 422;
      return { errors: [{ status: "422", title: "Unprocessable Entity", detail: "name and project are required" }] };
    }
    const modeError = stackExecutionModeError(executionMode, agentPoolId);
    if (modeError !== null) {
      (set as { status: number }).status = 422;
      return { errors: [{ status: "422", title: "Unprocessable Entity", detail: modeError }] };
    }
    const authorized = await authorizeStackProject(
      projectId,
      agentPoolId,
      user?.id,
      tokenOrgId ?? null,
  ...
  .get(
    "/api/v2/organizations/:org_name/stacks",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const orgName = params["org_name"] ?? "";
      const org = await cachedOrgByName(orgName);
      if (
        org === undefined ||
        !(await checkOrganizationPermission(org.id, user?.id, tokenOrgId ?? null, teamId ?? null, "manage-projects"))
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
  ...
  .get(
    "/api/v2/stacks/:stack_id/stack-configuration-summaries",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const details = await stackDetails(params["stack_id"] ?? "");
      if (
        details === undefined ||
        !(await checkOrganizationPermission(
      ...
      const configurations = await db.query.stackRecords.findMany({
        where: and(eq(stackRecords.stackId, details.stack.id), eq(stackRecords.recordType, "stack-configurations")),
        orderBy: [desc(stackRecords.createdAt)],
      });
      const summaries = configurations.map(
        (configuration): Record<string, unknown> => ({
          id: `scs-${configuration.id.slice(4)}`,
          type: "stack-configuration-summaries",
          attributes: {
            "sequence-number": (configuration.payload ?? {})["sequence-number"] ?? 1,
            status: configuration.status,
            "status-counts": { [configuration.status]: 1 },
          },
      ...
      const { number, size } = pageRequest(request);
      return {
        data: summaries.slice((number - 1) * size, number * size),
        ...pagination(request, number, size, summaries.length),
      };
    },
  )
  .post(
    "/api/v2/stacks/:stack_id/stack-configurations",
    async ({ params, body, request, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (requestError !== null) {
        (set as { status: number }).status = 422;
        return { errors: [{ status: "422", title: "Unprocessable Entity", detail: requestError }] };
      }
      ...
      if (configuration === undefined) {
        ...
        return {
          errors: [
            { status: "422", title: "Unprocessable Entity", detail: "reuse requires a previous stack configuration" },
          ],
        };
      }
  ...
  .get(
    "/api/v2/stack-configurations/:stack_configuration_id",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
  ...
  .get(
    "/api/v2/stack-configurations/:stack_configuration_id/stack-deployment-group-summaries",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_configuration_id"] ?? "",
        user,
      ...
      const groups = await db.query.stackRecords.findMany({
        where: and(
          eq(stackRecords.parentId, authorized.record.id),
          eq(stackRecords.recordType, "stack-deployment-groups"),
        ),
        orderBy: [desc(stackRecords.createdAt)],
      });
      const summaries = groups.map(
        (group): Record<string, unknown> => ({
          id: `sdgs-${group.id.slice(4)}`,
          type: "stack-deployment-group-summaries",
          attributes: { name: group.name, status: group.status, "status-counts": { [group.status]: 1 } },
          relationships: { "stack-deployment-group": { data: { id: group.id, type: "stack-deployment-groups" } } },
        }),
      );
      const { number, size } = pageRequest(request);
      return {
  ...
  .get(
    "/api/v2/stack-configurations/:stack_configuration_id/stack-diagnostics",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_configuration_id"] ?? "",
        user,
      ...
      const records = await db.query.stackRecords.findMany({
        where: and(eq(stackRecords.parentId, authorized.record.id), eq(stackRecords.recordType, "stack-diagnostics")),
        orderBy: [desc(stackRecords.createdAt)],
      });
      const { data, pagination: page } = pagedStackRecords(records, request);
      return { data, ...page };
  ...
  .get(
    "/api/v2/stack-configurations/:stack_configuration_id/upload-url",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_configuration_id"] ?? "",
        user,
      ...
      return {
        data: {
          "source-upload-url": signedApiURL(
            request,
            `/api/v2/stack-configurations/${authorized.record.id}/upload`,
            "PUT",
          ),
        },
        links: { self: `/api/v2/stack-configurations/${authorized.record.id}/upload-url` },
      };
    },
  )
  .put(
    "/api/v2/stack-configurations/:stack_configuration_id/upload",
    async ({ params, body, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if ("error" in loaded) {
        ...
        return {
          errors: [
            { status: String(loaded.error.status), title: loaded.error.status === 404 ? "Not Found" : "Conflict" },
          ],
        };
      }
      ...
      const claimed = await db
        .update(stackRecords)
        .set({ status: "uploading", updatedAt: Date.now() })
        .where(and(eq(stackRecords.id, record.id), eq(stackRecords.status, "pending")))
        .returning({ id: stackRecords.id });
      if (claimed.length !== 1) {
      ...
      try {
        ...
        await db
          .update(stackRecords)
          .set({ status: "ready", payload: { ...recordPayload, archivePath }, updatedAt: Date.now() })
          .where(eq(stackRecords.id, record.id));
        ...
        await db
          ...
          .where(and(eq(stackRecords.id, record.id), eq(stackRecords.status, "uploading")));
        throw error;
      }
  ...
  .get(
    "/api/v2/stack-configurations/:stack_configuration_id/source-bundle",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_configuration_id"] ?? "",
        user,
      ...
      const location = signedApiURL(
        request,
        `/api/v2/stack-configurations/${authorized.record.id}/source-bundle/download`,
        "GET",
      );
  ...
  .get(
    "/api/v2/stack-configurations/:stack_configuration_id/source-bundle/download",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const record = await db.query.stackRecords.findFirst({
        where: and(
          eq(stackRecords.id, params["stack_configuration_id"] ?? ""),
          eq(stackRecords.recordType, "stack-configurations"),
        ),
      });
      ...
      if (
        ...
        (!(await checkOrganizationPermission(
          ...
          !validSignedApiURL(request, `/api/v2/stack-configurations/${record.id}/source-bundle/download`, "GET"))
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
      ...
      if (archivePath === null || !isStackStoragePath(archivePath) || !(await Bun.file(archivePath).exists())) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
  ...
  .get(
    "/api/v2/stacks/:stack_id/stack-deployments",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const details = await stackDetails(params["stack_id"] ?? "");
      if (
        details === undefined ||
      ...
      const groups = await db.query.stackRecords.findMany({
        where: and(eq(stackRecords.stackId, details.stack.id), eq(stackRecords.recordType, "stack-deployment-groups")),
        orderBy: [desc(stackRecords.createdAt)],
      });
      ...
      const deployments = groups.flatMap((group): Record<string, unknown>[] => {
        ...
        return [
          {
            id: `${details.stack.id}-std-${name}`,
            type: "stack-deployments",
            attributes: { name },
            relationships: {
              stack: { data: { id: details.stack.id, type: "stacks" } },
              "latest-deployment-run": {
                data: typeof run === "string" ? { id: run, type: "stack-deployment-runs" } : null,
              },
            },
            links: {
              self: `/api/v2/stacks/${details.stack.id}/stack-deployments/${encodeURIComponent(name)}`,
              "stack-deployment-runs": `/api/v2/stacks/${details.stack.id}/stack-deployments/${encodeURIComponent(name)}/stack-deployment-runs`,
            },
          },
        ];
      });
      const { number, size } = pageRequest(request);
      return {
        data: deployments.slice((number - 1) * size, number * size),
        ...pagination(request, number, size, deployments.length),
      };
    },
  )
  .get(
    "/api/v2/stack-configurations/:stack_configuration_id/stack-deployment-groups",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_configuration_id"] ?? "",
        user,
  ...
  .get(
    "/api/v2/stack-configurations/:stack_configuration_id/stack-deployment-groups/:stack_deployment_group_name",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_configuration_id"] ?? "",
        user,
        ...
        authorized === undefined
          ...
          : await db.query.stackRecords.findFirst({
              where: and(
                eq(stackRecords.parentId, authorized.record.id),
                eq(stackRecords.recordType, "stack-deployment-groups"),
                eq(stackRecords.name, params["stack_deployment_group_name"] ?? ""),
              ),
            });
      if (group === undefined) {
        (set as { status: number }).status = 404;
  ...
  .get(
    "/api/v2/stack-deployment-groups/:stack_deployment_group_id",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_group_id"] ?? "",
        user,
        tokenOrgId,
        teamId,
        "stack-deployment-groups",
  ...
  .get(
    "/api/v2/stack-configurations/:stack_configuration_id/stack-deployment-runs",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_configuration_id"] ?? "",
        user,
      ...
      const records = (
        await db.query.stackRecords.findMany({
          where: and(
            eq(stackRecords.stackId, authorized.record.stackId),
            eq(stackRecords.recordType, "stack-deployment-runs"),
          ),
          orderBy: [desc(stackRecords.createdAt)],
        })
      ).filter((record) => (record.payload ?? {})["configurationId"] === authorized.record.id);
      const { data, pagination: page } = pagedStackRecords(records, request);
      return { data, ...page };
    },
  )
  .get(
    "/api/v2/stack-deployment-groups/:stack_deployment_group_id/stack-deployment-runs",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_group_id"] ?? "",
        user,
  ...
  .get(
    "/api/v2/stack-deployment-runs/:stack_deployment_run_id",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_run_id"] ?? "",
        user,
        tokenOrgId,
        teamId,
        "stack-deployment-runs",
  ...
  .get(
    "/api/v2/stack-deployment-runs/:stack_deployment_run_id/stack-deployment-steps",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_run_id"] ?? "",
        user,
  ...
  .get(
    "/api/v2/stack-deployment-steps/:stack_deployment_step_id",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_step_id"] ?? "",
        user,
        tokenOrgId,
        teamId,
        "stack-deployment-steps",
  ...
  .get(
    "/api/v2/stack-deployment-steps/:stack_deployment_step_id/stack-diagnostics",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_step_id"] ?? "",
        user,
  ...
  .get(
    "/api/v2/stack-deployment-steps/:stack_deployment_step_id/artifacts",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_step_id"] ?? "",
        user,
      ...
      if (
        authorized === undefined ||
        !["plan-description", "plan-debug-log", "apply-description", "apply-debug-log"].includes(name)
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
      const artifact = await db.query.stackRecords.findFirst({
        where: and(
          eq(stackRecords.parentId, authorized.record.id),
          eq(stackRecords.recordType, "stack-artifacts"),
          eq(stackRecords.name, name),
        ),
      });
      const path = (artifact?.payload ?? {})["path"];
      if (typeof path !== "string" || !isStackStoragePath(path) || !(await Bun.file(path).exists())) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
  ...
  .get(
    "/api/v2/stack-approvals/:stack_approval_id",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_approval_id"] ?? "",
        user,
        tokenOrgId,
        teamId,
        "stack-approvals",
  ...
  .post(
    "/api/v2/stack-deployment-groups/:stack_deployment_group_id/approve-all-plans",
    async ({ params, user, body, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_group_id"] ?? "",
        user,
        tokenOrgId,
      ...
      const payload = body !== null && typeof body === "object" ? (body as Record<string, unknown>) : {};
      await approveStackRecord(
        authorized.record,
        authorized.details.stack.orgId,
        user?.id ?? null,
  ...
  .post(
    "/api/v2/stack-deployment-groups/:stack_deployment_group_id/rerun",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_group_id"] ?? "",
        user,
      ...
      await db.transaction(async (tx): Promise<void> => {
        await tx
          ...
          .where(eq(stackRecords.id, authorized.record.id));
        const runs = await tx.query.stackRecords.findMany({
          where: and(
            eq(stackRecords.parentId, authorized.record.id),
            eq(stackRecords.recordType, "stack-deployment-runs"),
            or(eq(stackRecords.status, "failed"), eq(stackRecords.status, "canceled")),
          ),
          columns: { id: true },
        });
        ...
        for (const run of runs) {
          ...
          if (step !== undefined)
            await tx
              .update(stackRecords)
              .set({ status: "queued", payload: { ...(step.payload ?? {}), error: null }, updatedAt: now })
  ...
  .post(
    "/api/v2/stack-deployment-runs/:stack_deployment_run_id/approve-all-plans",
    async ({ params, user, body, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_run_id"] ?? "",
        user,
  ...
  .post(
    "/api/v2/stack-deployment-runs/:stack_deployment_run_id/cancel",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_run_id"] ?? "",
        user,
      ...
      const canceled = await db.transaction(async (tx): Promise<boolean> => {
        ...
        const updated = await tx
          ...
          .where(
            and(
              eq(stackRecords.id, authorized.record.id),
        ...
        await tx
          ...
          .set({ status: "canceled", agentId: null, completedAt: now, updatedAt: now })
          .where(
            and(
              eq(stackAgentJobs.deploymentRunId, authorized.record.id),
              notInArray(stackAgentJobs.status, ["completed", "errored", "canceled"]),
            ),
          );
        await tx
          .update(durableJobs)
          ...
          .where(
            and(
              ...
              eq(durableJobs.dedupeKey, `stack-run:${authorized.record.id}`),
              notInArray(durableJobs.status, ["succeeded", "failed", "canceled"]),
            ),
          );
        await tx
          .update(stackStateLocks)
          .set({ runId: null, leaseExpiresAt: null, releasedAt: now, updatedAt: now })
          .where(
            and(
      ...
      if (!canceled) {
        (set as { status: number }).status = 409;
        return { errors: [{ status: "409", title: "Conflict", detail: "deployment run is already terminal" }] };
      }
      (set as { status: number }).status = 204;
      return {};
    },
  ...
  .post(
    "/api/v2/stack-deployment-steps/:stack_deployment_step_id/advance",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_deployment_step_id"] ?? "",
        user,
      ...
      await db
        ...
        .where(eq(stackRecords.id, authorized.record.id));
      if (authorized.record.parentId !== null)
        await enqueueStackDeployment(
          authorized.record.parentId,
          authorized.details.stack.orgId,
          `stack-run:${authorized.record.parentId}:advance:${authorized.record.id}`,
        );
      (set as { status: number }).status = 204;
      return {};
    },
  ...
  .get(
    "/api/v2/stacks/:stack_id/stack-states",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const details = await stackDetails(params["stack_id"] ?? "");
      if (
        details === undefined ||
      ...
      const records = await db.query.stackRecords.findMany({
        where: and(eq(stackRecords.stackId, details.stack.id), eq(stackRecords.recordType, "stack-states")),
        orderBy: [desc(stackRecords.createdAt)],
      });
      const { data, pagination: page } = pagedStackRecords(records, request);
      return { data, ...page };
  ...
  .get(
    "/api/v2/stack_states/:stack_state_id",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_state_id"] ?? "",
        user,
        tokenOrgId,
        teamId,
        "stack-states",
  ...
  .get(
    "/api/v2/stack-states/:stack_state_id",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_state_id"] ?? "",
        user,
  ...
  .get(
    "/api/v2/stack-states/:stack_state_id/description",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const authorized = await authorizedStackRecord(
        params["stack_state_id"] ?? "",
        user,
      ...
      const location = signedApiURL(
        request,
        `/api/v2/stack-states/${authorized.record.id}/description/download`,
        "GET",
      );
      (set.headers as Record<string, string>)["Location"] = location;
  ...
  .get(
    "/api/v2/stack-states/:stack_state_id/description/download",
    async ({ params, user, request, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const record = await db.query.stackRecords.findFirst({
        where: and(eq(stackRecords.id, params["stack_state_id"] ?? ""), eq(stackRecords.recordType, "stack-states")),
      });
      const details = record === undefined ? undefined : await stackDetails(record.stackId);
      if (
        record === undefined ||
        ...
        (!(await checkOrganizationPermission(
          ...
          !validSignedApiURL(request, `/api/v2/stack-states/${record.id}/description/download`, "GET"))
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
  ...
  .patch(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      ...
      const executionError = await applyStackExecutionUpdates(
        attributes,
        relationships,
        details.stack,
        details.stack.orgId,
      ...
      if (executionError !== null) {
        (set as { status: number }).status = 422;
        return { errors: [{ status: "422", title: "Unprocessable Entity", detail: executionError }] };
      }
  ...
  .post(
    "/api/v2/stacks/:stack_id/fetch-latest-from-vcs",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const details = await stackDetails(params["stack_id"] ?? "");
      if (
        details === undefined ||
      ...
      if (details.stack.vcsIdentifier === null) {
        (set as { status: number }).status = 422;
        return {
          errors: [{ status: "422", title: "Unprocessable Entity", detail: "fetch requires a VCS-backed stack" }],
        };
      }
      ...
      if (configuration === undefined) {
        (set as { status: number }).status = 422;
        return {
          errors: [{ status: "422", title: "Unprocessable Entity", detail: "Unable to create a Stack configuration" }],
        };
      }
