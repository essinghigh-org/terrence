...
import { db } from "../db";
import { agents, stackAgentJobs, stackRecords, stackStateLocks, stacks } from "../db/schema";
...
export function isStackAgentResultValid(result: unknown): boolean {
  ...
  try {
    ...
    const metadata = Object.fromEntries(
      Object.entries(resultRecord).filter(([key]): boolean => key !== "state" && key !== "json_state"),
    );
    const serialized = JSON.stringify(
      Object.keys(metadata).length === Object.keys(resultRecord).length ? resultRecord : metadata,
    );
...
async function jobDetails(job: StackAgentJob): Promise<ClaimedStackAgentJob | undefined> {
  const [stack, step] = await Promise.all([
    ...
    db.query.stackRecords.findFirst({
      where: and(eq(stackRecords.id, job.stepId), eq(stackRecords.recordType, "stack-deployment-steps")),
    }),
  ]);
  if (stack === undefined || step === undefined || step.parentId === null) return undefined;
  const deploymentRun = await db.query.stackRecords.findFirst({
    where: and(eq(stackRecords.id, step.parentId), eq(stackRecords.recordType, "stack-deployment-runs")),
  });
  ...
  const configuration = await db.query.stackRecords.findFirst({
    where: and(
      eq(stackRecords.id, payloadString(deploymentRun, "configurationId") ?? ""),
...
async function claimStackAgentAttempt(
  ...
  const claimed = await db
    ...
    .where(and(eq(stackAgentJobs.id, candidate.id), eq(stackAgentJobs.status, "queued")))
    .returning();
  const job = claimed[0];
  ...
  if (details === undefined) {
    await db
      ...
      .set({
        status: "errored",
        errorMessage: "Stack deployment step no longer exists",
        completedAt: now,
        updatedAt: now,
      })
      .where(eq(stackAgentJobs.id, job.id));
    return { kind: "retry" };
  }
  ...
  await db
    ...
    .where(and(eq(stackRecords.id, details.step.id), inArray(stackRecords.status, ["queued", "approved", "pending"])));
  await db
    .update(stackRecords)
    .set({ status: nextStatus, updatedAt: now })
    .where(
      and(
        eq(stackRecords.id, details.deploymentRun.id),
        inArray(stackRecords.status, ["queued", "approved", "acquiring_lock", "planning", "applying"]),
      ),
    );
  ...
  const fencingToken =
    typeof rawFencingToken === "number" && Number.isInteger(rawFencingToken) ? rawFencingToken : undefined;
  const needsLock = (details.step.payload ?? {})["requires-state-lock"] === true;
  const lockHeld =
    !needsLock ||
    (fencingToken !== undefined &&
      (await refreshStackStateLock(
        details.stack.id,
        details.deploymentRun.name ?? "default",
        details.deploymentRun.id,
        fencingToken,
      )));
  if (!lockHeld) {
    await db
      .update(stackAgentJobs)
      .set({ agentId: null, status: "queued", claimedAt: null, updatedAt: Date.now() })
      .where(eq(stackAgentJobs.id, job.id));
    await db.update(agents).set({ status: "idle", lastPingAt: Date.now() }).where(eq(agents.id, agent.id));
    await enqueueDurableJob(
      "stack-deployment",
      { runId: details.deploymentRun.id },
      { dedupeKey: `stack-run:${details.deploymentRun.id}`, runAfter: Date.now() + 1000, rescheduleRunning: true },
    );
    return { kind: "retry" };
  }
...
export async function findClaimedStackAgentJob(
  agentId: string,
  jobId: string,
  fencingToken?: number,
): Promise<ClaimedStackAgentJob | undefined> {
  const job = await db.query.stackAgentJobs.findFirst({
    where: and(
...
export async function heartbeatStackAgentJob(agentId: string, jobId: string, fencingToken?: number): Promise<boolean> {
  ...
  if (requiresStateLock) {
    ...
    if (
      !(await refreshStackStateLock(
        claimed.stack.id,
        claimed.deploymentRun.name ?? "default",
        claimed.deploymentRun.id,
        rawFencingToken,
      ))
    )
      return false;
...
async function persistCompletedApplyState(outcome: StackAgentCompletionOutcome): Promise<void> {
  ...
  const run = await db.query.stackRecords.findFirst({
    where: and(eq(stackRecords.id, outcome.job.deploymentRunId), eq(stackRecords.recordType, "stack-deployment-runs")),
  });
  if (run === undefined) return;
  const statePayload = stackStatePayload(outcome.job.result);
  try {
    if (run.payload?.["destroy"] === true)
      await removeStackState(outcome.job.stackId, run.name ?? "default", run.id, outcome.fencingToken);
    else {
    ...
    await db.transaction(async (tx): Promise<void> => {
      await tx
        ...
        .set({
          status: "failed",
          payload: { ...outcome.stepPayload, ...(outcome.job.result ?? {}), error: detail },
          updatedAt: now,
        })
        .where(eq(stackRecords.id, outcome.job.stepId));
      await tx
        .update(stackRecords)
        .set({ status: "failed", payload: { error: detail }, updatedAt: now })
...
function validateStackAgentCompletion(completion: StackAgentJobCompletion): void {
  ...
  if (!isStackAgentResultValid(metadata))
    throw new Error(`stack agent result metadata exceeds ${MAX_STACK_AGENT_RESULT_BYTES} bytes or structural limits`);
}
...
async function stackAgentCompletionContext(
  ...
  const job = await tx.query.stackAgentJobs.findFirst({
    where: and(
      eq(stackAgentJobs.id, jobId),
      eq(stackAgentJobs.agentId, agentId),
      eq(stackAgentJobs.status, "claimed"),
  ...
  const step = await tx.query.stackRecords.findFirst({
    where: and(eq(stackRecords.id, job.stepId), eq(stackRecords.recordType, "stack-deployment-steps")),
  });
...
async function stackAgentCompletionRun(tx: StackAgentTransaction, step: StackRecord): Promise<StackRecord | undefined> {
  return step.parentId === null
    ? undefined
    : tx.query.stackRecords.findFirst({
        where: and(eq(stackRecords.id, step.parentId), eq(stackRecords.recordType, "stack-deployment-runs")),
      });
}
...
async function persistSuccessfulStackAgentCompletion(
  ...
  const deferredChanges = completionFlag(completion.result, [
    "deferredChanges",
    "deferred-changes",
    "deferred_changes",
  ]);
  if (job.phase === "plan" && (hasChanges || deferredChanges)) {
    await tx
      .update(stackRecords)
      .set({ status: "pending_operator", payload: result, updatedAt: now })
      .where(eq(stackRecords.id, step.id));
    runStatus = "pre_deploying_pending_operator";
  } else {
    await tx
      .update(stackRecords)
      .set({ status: "completed", payload: result, updatedAt: now })
      .where(eq(stackRecords.id, step.id));
  }
  await tx
    .update(stackRecords)
    .set({ status: runStatus, payload: { ...(run.payload ?? {}), lastAgentJobId: job.id }, updatedAt: now })
    .where(eq(stackRecords.id, run.id));
  await tx.update(agents).set({ status: "idle", lastPingAt: now }).where(eq(agents.id, agentId));
  const fencingToken = payloadFencingToken(step);
...
export async function completeStackAgentJob(
  ...
  const outcome = await db.transaction(async (transaction): Promise<StackAgentCompletionOutcome | undefined> => {
    ...
    if (completion.status === "errored")
      return persistFailedStackAgentCompletion(tx, agentId, completed, context.step, run, completion, now);
  ...
  if (outcome !== undefined && outcome.runStatus === "step_completed") {
    ...
    await enqueueDurableJob(
      "stack-deployment",
      { runId: outcome.job.deploymentRunId },
      { dedupeKey: `stack-run:${outcome.job.deploymentRunId}`, rescheduleRunning: true },
    );
  }
  return outcome;
}
