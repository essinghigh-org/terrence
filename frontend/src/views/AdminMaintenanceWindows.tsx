...
function humanizeDays(days: number[]): string {
  ...
  if (days.length === 5 && DAYS_WEEKDAYS.every((day: number): boolean => days.includes(day))) {
    return "Weekdays";
  }
  if (days.length === 2 && DAYS_WEEKEND.every((day: number): boolean => days.includes(day))) {
    return "Weekend";
  }
  return [...days]
...
export function AdminMaintenanceWindows(): React.JSX.Element {
  ...
  const updateWindow = (index: number, patch: Partial<MaintenanceWindow>): void => {
    setWindows((existing): MaintenanceWindow[] =>
      existing.map((window, i): MaintenanceWindow => (i === index ? { ...window, ...patch } : window)),
    );
  };
    ...
    <PageShell variant="form">
      ...
      <Card>
        <CardHeader variant="section">
          ...
          <CardDescription>Applies are blocked during configured windows. Plans are never affected.</CardDescription>
          <CardAction>
            <div className="flex items-center gap-2">
              ...
              <Switch
                ...
                onCheckedChange={setWindowsEnabled}
        ...
        <CardContent>
          {windows.length === 0 ? (
            ...
            <div className="space-y-3">
              {windows.map(
                (window, index): React.JSX.Element => (
                  <div key={index} className="rounded-lg border bg-card p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        ...
                        onClick={(): void => {
                          setWindows((existing): MaintenanceWindow[] =>
                            existing.filter((_, i): boolean => i !== index),
                          );
                        }}
                        ...
                        {DAY_OPTIONS.map((option): React.JSX.Element => {
                          ...
                          return (
                            <Button
                              ...
                              onClick={(): void => {
                                updateWindow(index, { days: toggleDay(window.days, option.day) });
                              }}
                            >
                              {option.label}
                            </Button>
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      <div className="space-y-1">
                        <label htmlFor={`window-${index}-start`} className="text-xs font-medium text-foreground">
                          Start time
                        </label>
                        <Input
                          ...
                          onChange={(event): void => {
                            updateWindow(index, { "start-time": event.target.value });
                          }}
                        />
                      </div>
                      <div className="space-y-1">
                        <label htmlFor={`window-${index}-end`} className="text-xs font-medium text-foreground">
                          End time
                        </label>
                        <Input
                          id={`window-${index}-end`}
                          ...
                          onChange={(event): void => {
                            updateWindow(index, { "end-time": event.target.value });
                          }}
                        />
                      </div>
                      <div className="space-y-1">
                        <label htmlFor={`window-${index}-timezone`} className="text-xs font-medium text-foreground">
                          Timezone
                        </label>
                        <Input
                          ...
                          placeholder="UTC"
                          onChange={(event): void => {
                            updateWindow(index, { timezone: event.target.value });
                          }}
                        />
                      </div>
                    </div>
                    <p className="text-xs text-muted-foreground">
                      Times are interpreted in the selected timezone (defaults to UTC).
                    </p>
                  </div>
                ),
              )}
