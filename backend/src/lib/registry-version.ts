...
export const MODULE_SEMVER_PATTERN =
...
export function isModuleVersion(value: string): boolean {
  ...
  return (
    ...
    prerelease
      .split(".")
      .every((identifier): boolean => !/^\d+$/u.test(identifier) || identifier === "0" || !identifier.startsWith("0"))
  );
}
...
export function sortModuleVersionsDescending<T extends Readonly<{ version: string }>>(versions: readonly T[]): T[] {
...
>(versions: readonly T[]): T | undefined {
  return (
    sortModuleVersionsDescending(
      versions.filter((version): boolean => version.status === "ok" && version.isRevoked !== true),
    )[0] ?? undefined
  );
}
