...
function Paw({ right = false, inset = false }: Readonly<{ right?: boolean; inset?: boolean }>): React.JSX.Element {
  ...
  return (
    <g
      className="terrence-paw"
      transform={right ? `translate(${322 - offset} 0) scale(-1 1)` : `translate(${offset} 0)`}
    >
...
function HeldProp({ pose, small }: Readonly<{ pose: TerrencePose; small: boolean }>): React.JSX.Element | null {
  switch (pose) {
    case "empty":
      return (
        <g className="terrence-prop" data-prop="box" transform="translate(0 5)" strokeWidth="3">
    ...
    case "healthy":
      return (
        <g className="terrence-prop" data-prop="check">
    ...
    case "failed":
      return (
        <g className="terrence-prop" data-prop="diagnostic">
    ...
    case "lost":
      return (
        <g className="terrence-prop" data-prop="map" strokeWidth="3">
          <path d="m107 192 36-10 36 13 36-10v54l-36 10-36-13-36 10Z" fill="#FFF1D5" />
          <path d="m143 182 36 13v54l-36-13Z" fill="#FAE4B5" />
          {!small && (
            <path
              className="terrence-secondary-detail"
              d="m123 221 12-9 19 13 15-16 27 10"
              stroke="#B38A43"
              strokeDasharray="2 6"
            />
          )}
    ...
    case "blocked":
      return (
        <g className="terrence-prop" data-prop="lock">
    ...
    case "interrupted":
      return (
        <g className="terrence-prop" data-prop="cable">
          ...
          <path
            d="M112 196h33v29h-33a8 8 0 0 1-8-8v-13a8 8 0 0 1 8-8Zm98 0h-31v29h31a8 8 0 0 0 8-8v-13a8 8 0 0 0-8-8Z"
            fill="#E4EAF3"
          />
...
function Wrench(): React.JSX.Element {
  return (
    <g className="terrence-prop" data-prop="wrench">
  ...
  return (
    <g className="terrence-face">
      <g className="terrence-eyes" transform={pose === "guide" || pose === "empty" ? "translate(0 3)" : undefined}>
        {pose === "healthy" ? (
          <path d="M123 143q8-12 16 0m45 0q8-12 16 0" />
        ) : (
          <>
            <ellipse cx="132" cy="142" rx="5" ry="8" fill="#233654" stroke="none" />
            <ellipse cx="190" cy="142" rx="5" ry="8" fill="#233654" stroke="none" />
          </>
        )}
      </g>
      <ellipse cx="115" cy="157" rx="10" ry="5" fill="#739BE8" stroke="none" />
      <ellipse cx="207" cy="157" rx="10" ry="5" fill="#739BE8" stroke="none" />
...
export function Terrence({
  ...
  return (
    ...
    >
      {surface === "paper" && (
        <rect className="terrence-backplate" x="8" y="8" width="304" height="264" rx="28" fill="#EDF3FF" />
      )}
      {!small && (
        <ellipse
          className="terrence-secondary-detail"
      ...
      {pose === "ecosystem" && (
        <path
          className="terrence-orbit terrence-orbit--back"
          d="M14 170a146 62 0 0 1 292 0"
      ...
      <g className="terrence-body" stroke="#233654" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round">
        <g className="terrence-pose" transform={poseTransforms[pose]}>
          ...
          <path
            d="M89 160C89 121 103 97 131 90L137 76 152 85 163 72 174 85 190 78 195 94C224 105 233 131 233 164L229 200C225 226 202 239 161 239 117 239 93 225 90 201Z"
            fill="#96B9F6"
          />
          <path
            d="M113 185C116 165 136 156 161 156 189 156 210 170 210 192 210 216 191 229 161 229 130 229 111 215 113 185Z"
            fill="#DCEAFF"
            stroke="none"
          />
          <Face pose={pose} />
          {pose === "lost" && !small && (
            <path
              className="terrence-secondary-detail"
              d="m238 90 1-4c2-9 18-9 18 2 0 7-10 7-10 15m0 10v1"
              stroke="#739BE8"
              strokeWidth="5"
            />
          )}
          ...
          {centralProp && (
            <>
              <Paw inset={pose === "blocked"} />
              <Paw right inset={pose === "blocked"} />
            </>
          )}
        </g>
      </g>
      {pose === "ecosystem" && (
        <g data-prop="engine-marks">
          <path
            className="terrence-orbit terrence-orbit--front"
            d="M14 170a146 62 0 0 0 292 0"
            transform="rotate(-18 160 170)"
            stroke="#536785"
            strokeWidth="1"
            strokeDasharray="4 6"
          />
          <g transform="rotate(-8 31 133)">
            <svg x="9" y="110" width="44" height="48">
              <TerraformMark />
            </svg>
          </g>
          <g transform="rotate(7 289 207)">
            <svg x="268" y="184" width="42" height="46">
              <OpenTofuMark />
            </svg>
          </g>
        </g>
...
export function TerrenceLogo({
  className,
  wordmark = false,
}: Readonly<{ className?: string; wordmark?: boolean }>): React.JSX.Element {
  return (
    <span className={cn("inline-flex items-center gap-2.5", className)}>
      ...
      {wordmark && (
        <span className="font-heading text-xl font-bold tracking-tight">
          terrence<span className="text-primary">.</span>
        </span>
      )}
    </span>
  );
