...
describe("registry module ingestion", () => {
  ...
  test("keeps discovery bounded by pagination but advances through every import batch", () => {
    ...
    expect(
      selectRegistryModuleVersionBatch(candidates, new Set(candidates.map((candidate): string => candidate.version))),
    ).toEqual([]);
  });
  ...
  test("extracts cached metadata from a realistic module fixture", async () => {
    ...
    expect(metadata.modules).toContainEqual({
      name: "labels",
  ...
  test("rejects traversal, links, oversized uploads, and oversized members", async () => {
    ...
    const traversalTar = Bun.spawn([
      ...
      "-czf",
      traversal,
      "--transform=s|main.tf|../escaped.tf|",
      "-C",
      source,
      "main.tf",
    ]);
    ...
    await expectRejection(async (): Promise<void> => {
      await validateModuleArchive(traversal);
    });
    ...
    await expectRejection(async (): Promise<void> => {
      await validateModuleArchive(links);
    }, "forbidden link");
    ...
    await expectRejection(async (): Promise<void> => {
      await validateModuleArchive(tooLargeUpload);
    }, "upload limit");
    ...
    await expectRejection(async (): Promise<void> => {
      await validateModuleArchive(largeMemberArchive);
    }, "file larger");
  });
  ...
  test("rejects a missing or escaping source directory", async () => {
    ...
    await expectRejection(async (): Promise<void> => {
      await ingestModuleArchive(
        fixtureArchive,
        join(directory, "escaping.tar.gz"),
        "../outside",
        inspectRegistryModule,
      );
    }, "safe relative path");
  });
});
