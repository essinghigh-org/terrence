...
export const oidcConfigRoutes = new Elysia({ name: "oidc-configs" })
  .use(authPlugin)
  .post(
    "/api/v2/organizations/:org_name/oidc-configurations",
          ...
          errors: [
            {
              ...
              detail: "OIDC configuration type and attributes are required",
            },
          ],
        };
      }
  ...
  .get(
    "/api/v2/oidc-configurations/:oidc_id",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
  ...
  .patch(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      ...
      await db
        .update(oidcConfigs)
        .set({ config: updated.config, updatedAt: updated.updatedAt })
        .where(eq(oidcConfigs.id, id));
