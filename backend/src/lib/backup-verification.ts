...
function assertManifestIdentity(
  candidate: Partial<BackupManifest> & { manifestSha256?: unknown },
): Omit<BackupManifest, "manifestSha256"> {
  ...
  if (candidate.manifestSha256 !== expected)
    throw new BackupVerificationError("manifest-tampered", "Backup manifest checksum does not match its contents");
  if (candidate.database === undefined || candidate.storage === undefined || candidate.keys === undefined) {
    throw new BackupVerificationError(
      "manifest-invalid",
      "Backup manifest is missing database, storage, or key metadata",
    );
  }
...
function validDatabaseSection(database: unknown): boolean {
  ...
  return (
    (section.driver === "sqlite" || section.driver === "postgres") &&
...
function assertStorageFileMetadata(
  storage: Readonly<{ fileCount?: unknown; totalBytes?: unknown; files: readonly unknown[] }>,
): void {
  if (
    !Number.isSafeInteger(storage.fileCount) ||
  ...
  if (
    ...
    storage.totalBytes !==
      storage.files.reduce((sum: number, file: unknown): number => sum + (file as { sizeBytes: number }).sizeBytes, 0)
  ) {
    throw new BackupVerificationError(
      "manifest-invalid",
      "Backup manifest storage totals do not match its file entries",
    );
  }
}
...
function parseManifest(raw: unknown): BackupManifest {
  if (raw === null || typeof raw !== "object" || Array.isArray(raw))
    throw new BackupVerificationError("manifest-invalid", "Backup manifest must be a JSON object");
  ...
  if (!validKeyMetadata(keys, keyNames))
    throw new BackupVerificationError("manifest-invalid", "Backup manifest has invalid key metadata");
...
async function digestFile(path: string): Promise<BackupFileDigest> {
  ...
  if (!info.isFile())
    throw new BackupVerificationError("storage-entry-invalid", "Backup storage contains a non-file entry");
...
async function collectFiles(root: string): Promise<readonly BackupFileDigest[]> {
  ...
  const visit = async (directory: string): Promise<void> => {
    ...
    for (const entry of names) {
      ...
      if (!entry.isFile())
        throw new BackupVerificationError("storage-entry-invalid", "Backup storage contains a link or special file");
      if (entries.length >= MAX_MANIFEST_FILES)
        throw new BackupVerificationError("storage-too-large", "Backup storage contains too many files");
      const info = await stat(full);
      totalBytes += info.size;
      if (totalBytes > MAX_MANIFEST_BYTES)
        throw new BackupVerificationError("storage-too-large", "Backup storage exceeds the manifest size limit");
...
async function queryTableCounts(): Promise<Readonly<Record<string, number>>> {
  ...
  try {
    ...
    throw new BackupVerificationError(
      "schema-incompatible",
      `Unable to count the configured schema tables: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}

async function queryEncryptedRecordCounts(): Promise<Readonly<Record<string, number>>> {
  const clauses = ENCRYPTED_COLUMN_CANDIDATES.map(
    ({ table, column }): string =>
  ...
  try {
    ...
    return Object.fromEntries(
      rows.map((row): [string, number] => [row.key, Number(row.count)]).filter(([, count]) => count > 0),
    );
  } catch {
...
async function schemaDigestForLiveDatabase(): Promise<string> {
  try {
    const rows = await rawQueryAll<Record<string, unknown>>(
      sql.raw(
        isPostgres
    ...
    throw new BackupVerificationError(
      "schema-unavailable",
      `Unable to read the active database schema: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
export async function createBackupManifest(
  ...
  const directory = resolve(
    options.outputDirectory ?? join(options.storagePath ?? defaultStoragePath(), BACKUP_MANIFEST_DIRECTORY),
  );
...
export async function createBackupManifestForSource(
  ...
  if (!sourceInfo.isDirectory() && sourceInfo.isFile() && sourceLooksLikeArchive(sourcePath)) {
    ...
    try {
      await runBoundedProcess(["tar", "-xf", sourcePath, "-C", temporaryRoot], {
        timeoutMs: 60_000,
      ...
      throw new BackupVerificationError(
        "archive-invalid",
        `Unable to extract backup archive: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  } else if (
    !sourceInfo.isDirectory() &&
    sourceInfo.isFile() &&
    [".db", ".sqlite", ".json"].includes(extname(sourcePath).toLowerCase())
  ) {
    root = dirname(sourcePath);
  } else if (!sourceInfo.isDirectory()) {
    throw new BackupVerificationError(
      "source-invalid",
      "Backup source must be a directory, manifest, SQLite file, or tar archive",
    );
  }
  try {
    ...
    const databasePath = await discoverDatabase(
      root,
      source.databasePath ??
        (sourceInfo.isFile() && !sourceLooksLikeArchive(sourcePath) && extname(sourcePath).toLowerCase() !== ".json"
          ? sourcePath
          : undefined),
    );
    if (databasePath === null)
      throw new BackupVerificationError("database-input", "Backup does not contain a SQLite database file");
    ...
    try {
      ...
      const journal = database.query("SELECT hash FROM __drizzle_migrations ORDER BY id DESC LIMIT 1").get() as {
        hash?: unknown;
      } | null;
      const files = await collectFiles(storage);
      const body: Omit<BackupManifest, "manifestSha256"> = {
...
export async function readBackupStatus(storagePath: string = defaultStoragePath()): Promise<BackupStatus> {
  try {
    const parsed = JSON.parse(
      await readFile(join(resolve(storagePath), BACKUP_STATUS_FILE), "utf8"),
    ) as Partial<BackupStatus>;
    return {
...
async function recordSuccessfulRestore(
  storagePath: string,
  manifest: BackupManifest,
  rehearsalId: string,
...
function safeArchiveMemberName(line: string): string | null {
  ...
  if (tarMemberPathUnsafe(member) || member.includes("\\") || member.startsWith("/") || /^[A-Za-z]:/.test(member))
    return null;
...
async function assertSafeBackupArchive(path: string): Promise<void> {
  ...
  if (info === null || !info.isFile() || info.size === 0)
    throw new BackupVerificationError("archive-invalid", "Backup archive is missing or empty");
  if (info.size > MAX_ARCHIVE_BYTES)
    throw new BackupVerificationError("archive-too-large", "Backup archive exceeds the size limit");
  let listing: string;
  try {
    listing = (
      await runBoundedProcess(["tar", "--numeric-owner", "-tvf", path], {
        timeoutMs: 30_000,
    ...
    throw new BackupVerificationError(
      "archive-invalid",
      `Unable to inspect backup archive: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  if (members.length > MAX_ARCHIVE_MEMBERS)
    throw new BackupVerificationError("archive-too-large", "Backup archive contains too many members");
  ...
  for (const line of members) {
    ...
    if (member === null)
      throw new BackupVerificationError(
        "archive-unsafe",
        "Backup archive contains a link, special file, or unsafe path",
      );
    ...
    if (seen.has(canonical))
      throw new BackupVerificationError("archive-unsafe", "Backup archive contains duplicate members");
    seen.add(canonical);
    const sizeMatch = /^(\S+)\s+\S+\s+(\d+)\s+\S+\s+\S+\s+(.+)$/.exec(line);
    logicalBytes += Number(sizeMatch?.[2] ?? 0);
    if (logicalBytes > MAX_ARCHIVE_BYTES)
      throw new BackupVerificationError("archive-too-large", "Backup archive expands beyond the size limit");
  }
}
...
async function findFile(root: string, names: readonly string[]): Promise<string | null> {
  ...
  while (queue.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && !entry.name.startsWith(".") && current.level < 5)
        queue.push({ path, level: current.level + 1 });
    }
  }
  return null;
}
...
async function discoverDatabase(root: string, explicit: string | undefined): Promise<string | null> {
  ...
  if (dbFiles.length > 1)
    throw new BackupVerificationError(
      "database-ambiguous",
      "Backup contains more than one SQLite database; specify database-path",
    );
  return null;
}
...
function explicitDatabasePath(
  source: BackupSourceOptions,
  sourcePath: string,
  sourceInfo: Readonly<Pick<Stats, "isFile">>,
): string | undefined {
  if (source.databasePath !== undefined) return source.databasePath;
  if (sourceInfo.isFile() && !sourceLooksLikeArchive(sourcePath) && extname(sourcePath).toLowerCase() !== ".json")
    return sourcePath;
  return undefined;
}
...
async function verifyStorageFiles(manifest: BackupManifest, storagePath: string): Promise<BackupCheck> {
  ...
  if (mismatched.length > 0)
    return check("fail", "storage-digests", `${mismatched.length} stored file(s) have a size or digest mismatch`);
...
async function verifyArtifactReferences(
  ...
  for (const reference of references) {
    const hasColumn = (
      database.query(`PRAGMA table_info(${quoteIdentifier(reference.table)})`).all() as readonly { name?: unknown }[]
    ).some((column): boolean => column.name === reference.column);
    if (!hasColumn) continue;
    const rows = database
      .query(
        `SELECT ${quoteIdentifier(reference.column)} AS "path" FROM ${quoteIdentifier(reference.table)} WHERE ${quoteIdentifier(reference.column)} IS NOT NULL`,
      )
      .all() as readonly { path?: unknown }[];
...
function sqliteTableNames(database: Readonly<Database>): Set<string> {
  const rows = database.query("SELECT name FROM sqlite_master WHERE type = 'table'").all() as readonly {
    name?: unknown;
  }[];
  return new Set(
    rows
      .map((row): string | null => (typeof row.name === "string" ? row.name : null))
      .filter((name): name is string => name !== null),
  );
}

function sqliteTableCounts(
  database: Readonly<Database>,
  ...
  if (missing.length > 0)
    throw new BackupVerificationError(
      "schema-incompatible",
      `Backup database is missing ${missing.length} required schema table(s)`,
    );
  return Object.fromEntries(
    Object.keys(expectedTables).map((table): [string, number] => {
      const row = database.query(`SELECT COUNT(*) AS "count" FROM ${quoteIdentifier(table)}`).get() as {
        count?: number | bigint;
      } | null;
...
function sqliteEncryptedRecordCounts(database: Readonly<Database>): Readonly<Record<string, number>> {
  ...
  for (const { table, column } of ENCRYPTED_COLUMN_CANDIDATES) {
    ...
    const hasColumn = (
      database.query(`PRAGMA table_info(${quoteIdentifier(table)})`).all() as readonly { name?: unknown }[]
    ).some((row): boolean => row.name === column);
    if (!hasColumn) continue;
    const row = database
      .query(
...
async function checkKeyIdentifiers(storagePath: string, manifest: BackupManifest): Promise<BackupCheck> {
  ...
  if (manifest.keys.passwordConfigured && process.env["ENCRYPTION_PASSWORD"] === undefined) {
    ...
    return check(
      mismatches.length === 0 ? "warning" : "fail",
      "key-identifiers",
      mismatches.length === 0
        ? "Key fingerprints match; ENCRYPTION_PASSWORD is not configured in this rehearsal process"
        : `${mismatches.length} key fingerprint(s) do not match`,
    );
  }
...
async function decryptSelectedRecords(
  database: Readonly<Database>,
  storagePath: string,
  manifest: BackupManifest,
): Promise<BackupCheck> {
  ...
  for (const [key, expected] of Object.entries(expectedCounts)) {
    if ((actualCounts[key] ?? 0) !== expected)
      return check("fail", "encrypted-records", `${key} count does not match the manifest`);
  }
  let selected = 0;
  try {
    for (const { table, column } of ENCRYPTED_COLUMN_CANDIDATES) {
  ...
  return check(
    ...
    "encrypted-records",
    selected === 0
      ? "No encrypted records were present to sample"
      : `${selected} selected encrypted record(s) decrypted`,
  );
}
...
async function verifySqliteDatabase(
  ...
  try {
    ...
    if (manifest.database.sha256 === null) {
      ...
      checks.push(
        digest === manifest.database.sha256
    ...
    const quickValue =
      typeof quickRaw === "string" ? quickRaw : typeof quickRaw === "number" ? quickRaw.toString() : "";
    checks.push(
      quickValue === "ok"
        ? check("pass", "database-integrity", "SQLite quick_check passed")
        : check("fail", "database-integrity", "SQLite quick_check reported corruption"),
    );
    const schemaDigest = await schemaDigestForSqlite(database);
    checks.push(
      schemaDigest === manifest.database.schemaSha256
        ? check("pass", "schema", "Database schema digest matches the manifest")
        : check("fail", "schema", "Database schema digest does not match the manifest"),
    );
    const actualCounts = sqliteTableCounts(database, manifest.database.tables);
    const countMismatch = Object.entries(manifest.database.tables).filter(
      ([table, count]): boolean => actualCounts[table] !== count,
    );
    checks.push(
      countMismatch.length === 0
        ? check("pass", "table-counts", `${Object.keys(actualCounts).length} table count(s) match`)
        : check("fail", "table-counts", `${countMismatch.length} table count(s) do not match`),
    );
    const actualEncrypted = sqliteEncryptedRecordCounts(database);
    const encryptedMismatch = Object.entries(manifest.encryptedRecords).filter(
      ([key, count]): boolean => actualEncrypted[key] !== count,
    );
    checks.push(
      encryptedMismatch.length === 0
        ? check("pass", "encrypted-counts", "Encrypted-record counts match")
        : check("fail", "encrypted-counts", `${encryptedMismatch.length} encrypted-record count(s) do not match`),
    );
    ...
    if (error instanceof BackupVerificationError) checks.push(check("fail", error.code, error.message));
      ...
      checks.push(
        check(
          "fail",
          "database-integrity",
          `Unable to inspect backup database: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
  } finally {
    database?.close();
...
async function verifyPrepared(prepared: PreparedSource, decrypt: boolean): Promise<BackupIntegrityReport> {
  ...
  return {
    passed: checks.every((item): boolean => item.status !== "fail"),
    manifest,
    checks,
...
async function copyPreparedSource(
...
): Promise<Readonly<{ root: string; storagePath: string; databasePath: string | null; cleanup: () => Promise<void> }>> {
  ...
  try {
    ...
    const copiedStorage = storageInsideSource
      ? prepared.storagePath === prepared.root
        ? destination
        : join(destination, storageRelative)
      : join(destination, "storage");
    if (!storageInsideSource)
      await cp(prepared.storagePath, copiedStorage, { recursive: true, force: true, preserveTimestamps: true });
    ...
    if (copiedDatabase !== null && !(await pathExists(copiedDatabase)) && sourceDatabase !== null)
      await cp(sourceDatabase, copiedDatabase, { force: true, preserveTimestamps: true });
    return {
      root: destination,
...
async function runMigrationsOnCopy(databasePath: string): Promise<BackupCheck> {
  ...
  try {
    ...
    return check(
      "fail",
      "schema-migration",
      `Disposable restore copy could not apply the bundled schema: ${error instanceof Error ? error.message : String(error)}`,
    );
  } finally {
    database?.close();
  }
...
async function runCliWorkflow(
  ...
  if (selected === undefined || selected === "") {
    return requireCli
      ...
      : check(
          "warning",
          "cli-workflow",
          "No tofu or terraform executable is configured; isolated database and artifact checks still completed",
        );
  }
  ...
  try {
    ...
    return check(
      "fail",
      "cli-workflow",
      `Isolated CLI workflow failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  } finally {
...
export async function runRestoreRehearsal(options: RestoreRehearsalOptions): Promise<BackupRehearsalReport> {
  ...
  try {
    ...
    const report = await verifyPrepared(
      {
        ...prepared,
        root: copied.root,
