...
describe("runtime deployment configuration", (): void => {
  test("rejects invalid explicit deployment secrets without echoing them", (): void => {
    for (const environment of [
      ...
      throws(
        ...
        (error: unknown): boolean => error instanceof Error && !error.message.includes("private-marker"),
      );
  ...
  test("rejects ignored or storage-exposing sandbox write-path configurations", (): void => {
    throws((): void => {
      parseRuntimeConfiguration({ TERRENCE_SANDBOX_EXTRA_RW_PATHS: "/opt/cache" });
    }, /requires TERRENCE_SANDBOX_EXTRA_RW_ALLOWED/);
    for (const path of ["relative", "", "/tmp", "/tmp/storage"]) {
      throws((): void => {
        parseRuntimeConfiguration({
          STORAGE_DIR: "/tmp/storage",
          TERRENCE_SANDBOX_EXTRA_RW_ALLOWED: "true",
          TERRENCE_SANDBOX_EXTRA_RW_PATHS: path,
        });
      });
    }
    expect(
      parseRuntimeConfiguration({
        STORAGE_DIR: "/tmp/storage",
        TERRENCE_SANDBOX_EXTRA_RW_ALLOWED: "true",
        TERRENCE_SANDBOX_EXTRA_RW_PATHS: "/opt/cache",
  ...
  test("requires a complete agent update and a supported node status", (): void => {
    for (const environment of [
      ...
      {
        TERRENCE_AGENT_UPDATE_VERSION: "1.0.0",
        TERRENCE_AGENT_UPDATE_URL: "https://example.com/agent",
        TERRENCE_AGENT_UPDATE_SHA256: "invalid",
      },
      {
        TERRENCE_AGENT_UPDATE_VERSION: "1.0.0",
        TERRENCE_AGENT_UPDATE_URL: "file:///private-marker",
        TERRENCE_AGENT_UPDATE_SHA256: "a".repeat(64),
      },
    ])
      throws(
        (): void => {
    ...
    const config = parseRuntimeConfiguration({
      TERRENCE_AGENT_UPDATE_VERSION: "1.0.0",
      TERRENCE_AGENT_UPDATE_URL: "https://example.com/agent?token=secret",
      TERRENCE_AGENT_UPDATE_SHA256: "A".repeat(64),
    });
  ...
  test("validates GitHub URLs while preserving explicit App override precedence", (): void => {
    for (const environment of [
      { GITHUB_API_URL: "" },
      { GITHUB_APP_HTTP_URL: "ftp://example.com" },
    ...
    expect(parseRuntimeConfiguration({ GITHUB_API_URL: "https://enterprise.example/api/v3" }).GITHUB_APP_API_URL).toBe(
      "https://enterprise.example/api/v3",
    );
    expect(
      parseRuntimeConfiguration({
        GITHUB_API_URL: "https://general.example",
        GITHUB_APP_API_URL: "https://app.example",
      }).GITHUB_APP_API_URL,
  ...
  test("rejects misspelled application-owned names but allows unrelated process variables", (): void => {
    for (const name of ["TERRENCE_RUN_CONCURENCY", "SYSTEM_API_TLS_CRET", "RATE_LIMIT_MXA", "GITHUB_APP_PRIVATE_KYE"]) {
      throws(
        (): void => {
          parseRuntimeConfiguration({ [name]: "private-marker" });
        },
        (error: unknown): boolean =>
          error instanceof Error && error.message.startsWith("Unknown ") && !error.message.includes("private-marker"),
      );
    }
    expect(
      parseRuntimeConfiguration({
        HOME: "/tmp",
        PATH: "/usr/bin",
        CUSTOM_SERVICE_VALUE: "allowed",
        TERRENCE_NODE_ID: "node-1",
      }).PORT,
    ).toBe(3000);
  });
  test("rejects invalid network policies instead of ignoring entries", (): void => {
    ...
    const config = parseRuntimeConfiguration({
      TERRENCE_TRUSTED_PROXY_CIDRS: "10.0.0.0/8,127.0.0.1",
      TERRENCE_OUTBOUND_ALLOW_HOSTS: "Example.COM.",
    });
  ...
  test("validates listener TLS pairs and explicit storage paths", (): void => {
    ...
    const config = parseRuntimeConfiguration({
      SYSTEM_API_HOST: "admin.example.com",
      SYSTEM_API_TLS_CERT: "/tmp/cert",
      SYSTEM_API_TLS_KEY: "/tmp/key",
    });
  ...
  test("keeps the generated documentation and examples aligned with the schema", (): void => {
    expect(readFileSync(new URL("../../docs/configuration-contract.md", import.meta.url), "utf8")).toBe(
      configurationReference(),
    );
    throws((): void => {
      parseRuntimeConfiguration({ TERRENCE_RUN_CONCURENCY: "5" }, true);
    }, /Unsupported setting/);
  });
  ...
  test("validates every numeric setting without exposing the input", (): void => {
    for (const [name, rule] of Object.entries(integerConfiguration)) {
      for (const raw of [
        ...
        throws(
          (): void => {
            parseRuntimeConfiguration({ [name]: raw });
          },
          (error: unknown): boolean => {
            return (
      ...
      if (rule.min > 0)
        throws((): void => {
          parseRuntimeConfiguration({ [name]: "0" });
        });
      expect(parseRuntimeConfiguration({ [name]: String(rule.min) })[name as keyof typeof integerConfiguration]).toBe(
        rule.min,
      );
      expect(parseRuntimeConfiguration({ [name]: String(rule.max) })[name as keyof typeof integerConfiguration]).toBe(
        rule.max,
      );
    }
  });
  ...
  test("rejects conflicting listeners", (): void => {
    throws((): void => {
      parseRuntimeConfiguration({ PORT: "8443" });
  ...
  test("freezes the startup values and reports their origin without copying unrelated secrets", (): void => {
    ...
    const decoded = JSON.parse(output) as {
      port: number;
      report: { name: string; value: unknown; origin: string; restartRequired: boolean }[];
    };
    expect(decoded.port).toBe(4567);
    expect(decoded.report.find((entry): boolean => entry.name === "PORT")).toEqual({
      name: "PORT",
      value: 4567,
      origin: "environment",
  ...
  test("rejects invalid security URLs and policies without echoing secrets", (): void => {
    ...
    const values = parseRuntimeConfiguration({
