...
import {
  ...
  projectRelationshipIds,
  stackRelationshipIds,
  variableRelationshipResources,
} from "../lib/resource-relationships";
...
async function projectVarsetIds(projectFilter: string): Promise<string[] | null> {
  ...
  const ids = new Set<string>([...owned.map((v): string => v.id), ...applied.map((l): string => l.variableSetId)]);
  if (ids.size === 0) return null;
  return [...ids];
}

async function hydrateVarsetRecords(records: VarSetItem[], orgName: string): Promise<Record<string, unknown>[]> {
  ...
  const [workspaceLinkRows, projectLinkRows, variableRows] =
    setIds.length === 0
      ? [[], [], []]
      : await Promise.all([
  ...
  return Promise.all(
    records.map(
      async (r: VarSetItem): Promise<Record<string, unknown>> =>
...
function parseVarsetAttributes(
  ...
  if (
    ...
    attributes === null ||
    !validVariableSetAttributes(attributes, partial)
  ) {
    (set as { status: number }).status = 422;
    return {
      error: { errors: [{ status: "422", title: "Unprocessable Entity", detail: "Invalid variable set attributes" }] },
    };
  }
  return { attributes };
}
...
async function checkVarsetParentProject(
  ...
  if (parent === undefined || parent.orgId !== orgId) {
    ...
    return {
      errors: [
        { status: "422", title: "Unprocessable Entity", detail: "Parent project must belong to the organization" },
      ],
    };
  }
  ...
  if (global === true) {
    ...
    return {
      errors: [
        { status: "422", title: "Unprocessable Entity", detail: "Project-owned variable sets cannot be global" },
      ],
    };
  }
  return null;
...
function parseVarsetVariableAttributes(
  ...
  if (data?.["type"] !== "vars" || !validVariableSetVariableAttributes(attributes, partial)) {
    (set as { status: number }).status = 422;
    return {
      error: { errors: [{ status: "422", title: "Unprocessable Entity", detail: "Invalid variable attributes" }] },
    };
  }
  return { attributes };
}
...
async function refreshVarsetVariableEncryption(values: Record<string, unknown>, current: VarItem): Promise<void> {
...
export const varsetRoutes = new Elysia({ name: "varsets" })
  .use(authPlugin)
  .get(
    "/api/v2/organizations/:org_name/varsets",
  ...
  .post(
    "/api/v2/organizations/:org_name/varsets",
    async ({ params, user, orgId, teamId, body, set }: ParamCtx): Promise<unknown> => {
      const orgName = params["org_name"] ?? "";
      const org = await cachedOrgByName(orgName);
      if (
        org === undefined ||
        !(await checkOrganizationPermission(org.id, user?.id, orgId, teamId, "manage-varsets"))
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
      ...
      const parentProjectId =
        typeof attributes["parent-project-id"] === "string" ? attributes["parent-project-id"] : null;
          ...
          errors: [
            {
              ...
              detail: "The owning project of a variable set cannot be changed",
            },
          ],
        };
      }
  ...
  .post(
    "/api/v2/varsets/:varset_id/relationships/workspaces",
    async ({
      params,
      user,
      orgId,
      ...
      if (workspaceIds === undefined) {
        (set as { status: number }).status = 422;
        return {
          errors: [{ status: "422", title: "Unprocessable Entity", detail: "Invalid workspace relationships" }],
        };
      }
      ...
      if (
        targets.length !== workspaceIds.length ||
        targets.some((w: Readonly<{ readonly orgId: string }>): boolean => w.orgId !== record.orgId)
      ) {
        (set as { status: number }).status = 422;
        return {
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "Workspaces must belong to the variable set organization",
            },
          ],
        };
      }
      await db
        ...
        .values(
          workspaceIds.map((wid: string): typeof variableSetWorkspaces.$inferInsert => ({
  ...
  .get(
    ...
    async ({ params, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
      ...
      return {
        data: links.map(
          (link: { readonly workspaceId: string }): Record<string, string> => ({
            id: link.workspaceId,
            type: "workspaces",
          }),
        ),
  ...
  .post(
    "/api/v2/varsets/:varset_id/relationships/projects",
    async ({
      params,
      user,
      orgId,
      ...
      if (projectIds === undefined) {
        (set as { status: number }).status = 422;
        return { errors: [{ status: "422", title: "Unprocessable Entity", detail: "Invalid project relationships" }] };
      }
      // JSON:API no-op: an explicit empty add/remove does nothing (the reference format accepts it).
      ...
      if (
        targets.length !== projectIds.length ||
        targets.some((p: Readonly<{ readonly orgId: string }>): boolean => p.orgId !== record.orgId)
      ) {
        (set as { status: number }).status = 422;
        return {
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "Projects must belong to the variable set organization",
            },
          ],
        };
      }
      await db
        .insert(variableSetProjects)
        .values(
          projectIds.map((pid: string): typeof variableSetProjects.$inferInsert => ({
            id: newResourceId("vsp"),
            variableSetId: record.id,
            projectId: pid,
          })),
        )
  ...
  .delete(
    ...
    async ({
      ...
      await db
        .delete(variableSetProjects)
        .where(
          and(eq(variableSetProjects.variableSetId, record.id), inArray(variableSetProjects.projectId, projectIds)),
        );
      (set as { status: number }).status = 204;
      return {};
    },
  ...
  .post(
    "/api/v2/varsets/:varset_id/relationships/stacks",
    async ({
      params,
      user,
      orgId,
      ...
      if (stackIds === undefined) {
        (set as { status: number }).status = 422;
        return { errors: [{ status: "422", title: "Unprocessable Entity", detail: "Invalid stack relationships" }] };
      }
      // JSON:API no-op: an explicit empty add/remove does nothing (the reference format accepts it).
      ...
      if (
        targets.length !== stackIds.length ||
        targets.some((s: Readonly<{ readonly orgId: string }>): boolean => s.orgId !== record.orgId)
      ) {
        (set as { status: number }).status = 422;
        return {
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "Stacks must belong to the variable set organization",
            },
          ],
        };
      }
      await db
        .insert(stackVariableSets)
        .values(
          stackIds.map((sid: string): typeof stackVariableSets.$inferInsert => ({
            stackId: sid,
            variableSetId: record.id,
          })),
        )
  ...
  .delete(
    ...
    async ({
      ...
      await db
        .delete(stackVariableSets)
        .where(and(eq(stackVariableSets.variableSetId, record.id), inArray(stackVariableSets.stackId, stackIds)));
      (set as { status: number }).status = 204;
      return {};
    },
  )
  // relationships/vars
  .get(
    "/api/v2/varsets/:varset_id/relationships/vars",
    async ({ params, user, orgId, teamId, request, set }: ParamCtx): Promise<unknown> => {
      const varsetId = params["varset_id"] ?? "";
      const record = await findAuthorizedVariableSet(varsetId, user?.id, orgId, teamId);
      if (record === undefined) {
      ...
      const totalCount = countRows[0]?.total ?? 0;
      return {
        data: variables.map((v: VarItem): Record<string, unknown> => variableSetVariableResource(v)),
        ...pagination(request, number, size, totalCount),
      };
    },
  )
  .get(
    "/api/v2/varsets/:varset_id/relationships/vars/:var_id",
    async ({ params, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const varsetId = params["varset_id"] ?? "";
      ...
      const variable =
        record !== undefined
          ? await db.query.variableSetVariables.findFirst({
              where: and(eq(variableSetVariables.id, varId), eq(variableSetVariables.variableSetId, record.id)),
            })
          : undefined;
    ...
    async ({ params, user, orgId, teamId, body, set }: ParamCtx): Promise<unknown> => {
      ...
      try {
        ...
        if (isUniqueConstraintError(error)) {
          ...
          return {
            errors: [
              { status: "422", title: "Unprocessable Entity", detail: "Variable key already exists in this set" },
            ],
          };
        }
        throw error;
  ...
  .patch(
    ...
    async ({ params, user, orgId, teamId, body, set }: ParamCtx): Promise<unknown> => {
      ...
      if (relationship.resources.length === 0) {
        (set as { status: number }).status = 204;
        return {};
      }
      if (
        (relationship.resources as ResItem[]).some(
          (item: ResItem): boolean => !validVariableSetVariableAttributes(item.attributes, true),
        )
      ) {
        (set as { status: number }).status = 422;
        return { errors: [{ status: "422", title: "Unprocessable Entity", detail: "Invalid variable resources" }] };
      ...
      const variables = await db.query.variableSetVariables.findMany({
        where: and(eq(variableSetVariables.variableSetId, record.id), inArray(variableSetVariables.id, ids)),
      });
      ...
      const updates = await Promise.all(
        (relationship.resources as ResItem[]).map(
          async (item: ResItem): Promise<{ variable: VarItem; values: Record<string, unknown> }> => {
            const v = byId.get(item.id);
            if (v === undefined) throw new Error("Variable not found");
            const base = variableSetVariableUpdate(
              v,
              item.attributes as Parameters<typeof variableSetVariableUpdate>[1],
            );
      ...
      try {
        await db.transaction(async (tx: unknown): Promise<void> => {
          ...
          await Promise.all(
            updates.map(
              async (u): Promise<unknown> =>
                t.update(variableSetVariables).set(u.values).where(eq(variableSetVariables.id, u.variable.id)),
            ),
          );
        });
      } catch (error: unknown) {
      ...
      const resources = updates.map(
        (
          u: Readonly<{ readonly variable: VarItem; readonly values: Readonly<Record<string, unknown>> }>,
        ): Record<string, unknown> => variableSetVariableResource({ ...u.variable, ...u.values }),
      );
    ...
    async ({ params, user, orgId, teamId, body, set }: ParamCtx): Promise<unknown> => {
      ...
      const variable =
        record !== undefined
          ...
          : undefined;
      if (record === undefined || variable === undefined) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
      const parsed = parseVarsetVariableAttributes(body, set, true);
      if ("error" in parsed) return parsed.error;
      const updated = variableSetVariableUpdate(
        variable,
        parsed.attributes as Parameters<typeof variableSetVariableUpdate>[1],
      );
