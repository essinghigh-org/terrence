...
test("compatibility publication requires every pinned journey and verified digest", () => {
  const reports = (["terraform", "tofu"] as const).flatMap((engine) =>
    (["floor", "current"] as const).map((tier) => ({
      fixture: "backend/tests/e2e/provider_e2e.test.ts",
      engine,
      tier,
      versions: {
        terraform_version: matrix[engine][tier],
        provider_selections: {
          [engine === "terraform" ? "registry.terraform.io/hashicorp/tfe" : "registry.opentofu.org/hashicorp/tfe"]:
            surface.provider.split(" v")[1],
        },
      },
      database: "sqlite",
      sandbox: "disabled",
      ...
      claims: [
        {
          ...
          behaviors: ["create", "read", "two-unchanged-plans", "unchanged-apply", "destroy"],
        },
        {
