...
const storageDir =
  typeof rawStorageDir === "string" && rawStorageDir !== "" ? rawStorageDir : join(import.meta.dir, "../storage");
...
export function configurationVersionResource(
  ...
  return {
    ...
    relationships: {
      "ingress-attributes": {
        data: hasIngressData(cv) ? { id: cv.id, type: "ingress-attributes" } : null,
...
export function configurationVersionIngressResource(cv: DeepReadonly<ConfigurationVersion>): Record<string, unknown> {
  const ingress = (cv.ingressAttributes ?? {}) as Record<string, unknown>;
  return {
    id: cv.id,
...
async function beginConfigVersionIdempotency(
  ...
  const idempotencyBegin = await beginIdempotency(idempotency, "configuration-versions", set);
  if (idempotencyBegin.kind === "replay") {
    const replayed =
      idempotencyBegin.resourceId === null
        ? undefined
        : await db.query.configurationVersions.findFirst({
            where: and(
              eq(configurationVersions.id, idempotencyBegin.resourceId),
...
async function authorizeConfigUpload(
  ...
  if (request.headers.get("authorization") === null && !signedUpload) {
    ...
    return { error: { errors: [{ status: "401", title: "Unauthorized" }] } };
  }
  ...
  if (cv.status !== "pending" || cv.archivePath !== null) {
    ...
    return {
      error: { errors: [{ status: "409", title: "Conflict", detail: "Configuration content was already uploaded" }] },
    };
  }
  return { cv };
}
...
async function claimConfigUpload(
  ...
  const claimFilter = and(
    ...
    or(isNull(configurationVersions.uploadClaimExpiresAt), lt(configurationVersions.uploadClaimExpiresAt, Date.now())),
  );
        ...
        errors: [
          {
            status: "409",
            title: "Conflict",
            detail: "An upload for this configuration version is already in progress",
          },
        ],
      },
    };
...
async function releaseUploadClaim(cvId: string, claimToken: string): Promise<void> {
  await db
    .update(configurationVersions)
    .set({ uploadClaimExpiresAt: null, uploadClaimToken: null })
    .where(and(eq(configurationVersions.id, cvId), eq(configurationVersions.uploadClaimToken, claimToken)));
}
  ...
  try {
    ...
    return {
      error: {
        errors: [
          {
            status: String(tooLarge ? 413 : 400),
            title: tooLarge ? "Payload Too Large" : "Bad Request",
            detail: tooLarge
              ? "Configuration archive exceeds 100 MiB maximum"
              : "Could not read configuration archive body",
          },
        ],
      },
    };
        ...
        errors: [
          {
            ...
            detail: expanded
              ? "Configuration archive expands beyond the permitted size"
              : "Configuration archive is not a valid gzip tar archive",
          },
        ],
      },
    };
...
export const configurationVersionRoutes = new Elysia({ name: "configurationVersions" })
  .use(authPlugin)
  .get(
    "/api/v2/workspaces/:workspace_id/configuration-versions",
  ...
  .post(
    ...
    async ({ params, body, user, orgId, teamId, request, set }: ParamCtx): Promise<unknown> => {
      ...
      if (orgId !== null && orgId !== undefined) {
        ...
        return {
          errors: [
            { status: "403", title: "Forbidden", detail: "Organization tokens cannot create configuration versions" },
          ],
        };
      }
      const parsed = parseConfigVersionCreate(body, request, set);
      if ("error" in parsed) return parsed.error;
      const begun = await beginConfigVersionIdempotency(
        request,
        workspaceId,
        user?.id,
        orgId,
  ...
  .get(
    "/api/v2/configuration-versions/:cv_id",
    async ({ params, user, orgId, teamId, request, set }: ParamCtx): Promise<unknown> => {
  ...
  .put(
    "/api/v2/configuration-versions/:cv_id/upload",
    async ({ params, body, user, orgId, teamId, request, set }: ParamCtx): Promise<unknown> => {
      const cvId = params["cv_id"] ?? "";
  ...
  .post(
    "/api/v2/configuration-versions/:cv_id/actions/archive",
    async ({ params, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
      ...
      const archived = await db.transaction(async (tx) => {
        ...
        if (
          ...
          ["github", "gitlab", "bitbucket"].includes(current.source ?? "")
        )
          return undefined;
        ...
        const rows = await tx
          ...
          .where(and(eq(configurationVersions.id, current.id), eq(configurationVersions.status, "uploaded")))
          .returning({ id: configurationVersions.id });
          ...
          errors: [
            {
              status: "409",
              title: "Conflict",
              detail: "Configuration version cannot be archived in its current state",
            },
          ],
        };
      }
  ...
  .get(
    "/api/v2/configuration-versions/:cv_id/download",
    async ({ params, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const cvId = params["cv_id"] ?? "";
      const cv = await db.query.configurationVersions.findFirst({ where: eq(configurationVersions.id, cvId) });
      if (cv === undefined) {
  ...
  .get(
    "/api/v2/runs/:run_id/configuration-version/download",
    async ({ params, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const run = await db.query.runs.findFirst({ where: eq(runs.id, params["run_id"] ?? "") });
      if (run === undefined || run.configurationVersionId === null) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
      ...
      if (
        ws === undefined ||
        cv === undefined ||
        cv.workspaceId !== run.workspaceId ||
        cv.archivePath === null ||
        cv.status !== "uploaded" ||
        !(await Bun.file(cv.archivePath).exists())
      ) {
  ...
  .post(
    "/api/v2/configuration-versions/:cv_id/actions/soft_delete_backing_data",
    async ({ params, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const cvId = params["cv_id"] ?? "";
      const cv = await db.query.configurationVersions.findFirst({ where: eq(configurationVersions.id, cvId) });
      if (cv === undefined) {
      ...
      const activeRun = await db.query.runs.findFirst({
        where: and(eq(runs.configurationVersionId, cv.id), notInArray(runs.status, FINAL_RUN_STATUSES)),
        columns: { id: true },
      });
  ...
  .post(
    "/api/v2/configuration-versions/:cv_id/actions/restore_backing_data",
    async ({ params, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const cvId = params["cv_id"] ?? "";
      const cv = await db.query.configurationVersions.findFirst({ where: eq(configurationVersions.id, cvId) });
      if (cv === undefined) {
      ...
      if (
        cv.status !== "backing_data_soft_deleted" ||
        cv.archivePath === null ||
        !(await Bun.file(cv.archivePath).exists())
      ) {
        (set as { status: number }).status = 400;
      ...
      await db
        .update(configurationVersions)
        .set({ status: "uploaded", softDeletedAt: null })
        .where(eq(configurationVersions.id, cv.id));
      return {};
    },
  )
  .post(
    "/api/v2/configuration-versions/:cv_id/actions/permanently_delete_backing_data",
    async ({ params, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const cvId = params["cv_id"] ?? "";
      const cv = await db.query.configurationVersions.findFirst({ where: eq(configurationVersions.id, cvId) });
      if (cv === undefined) {
  ...
  .get(
    "/api/v2/configuration-versions/:cv_id/ingress-attributes",
