...
test.skipIf(process.env["WORKSPACE_SCALE"] !== "1")(
  "measures a real 10000-workspace list without loading state or providers",
  async () => {
    ...
    try {
      ...
      for (let offset = 0; offset < 10000; offset += 100) {
        const batch = Array.from({ length: 100 }, (_, j) => ({
          id: `ws-${seed.suffix}-${offset + j}`,
          orgId: seed.orgId,
          name: `workspace-${String(offset + j).padStart(5, "0")}`,
        }));
        await db.insert(workspaces).values(batch);
        await db
          .insert(workspaceTags)
          .values(batch.map((workspace) => ({ id: `tag-${workspace.id}`, workspaceId: workspace.id, key: "scale" })));
        await db.insert(runs).values(
          batch.flatMap((workspace) => [
      ...
      api = Bun.serve({
        ...
        async fetch(req: Request): Promise<Response> {
          ...
          if (url.pathname === "/api/v2/users/refresh")
            return Response.json({
              data: {
                type: "sessions",
                id: "scale-session",
                attributes: { token: seed.token, "expired-at": new Date(Date.now() + 3600000).toISOString() },
              },
            });
      ...
      const heap = (await browser.webview.cdp("Runtime.getHeapUsage")) as {
        usedSize: number;
      ...
      const report = {
        ...
        workspaceRequests: workspaceTimes.length,
        workspaceQueries,
        responseBytes: bytes,
        firstUsefulMs: Math.round(firstUsefulMs),
        serverRequestMs: {
          total: workspaceTimes.reduce((a, b) => a + b, 0),
          p50: sorted[Math.floor(sorted.length * 0.5)],
          p95: sorted[Math.min(sorted.length - 1, Math.floor(sorted.length * 0.95))],
        },
        uiDatabaseQueries: getQueryCount(),
        browserJsHeapBytes: heap.usedSize,
