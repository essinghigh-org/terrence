...
const preflightCache = new Map<
  string,
  Readonly<{ expiresAtMs: number; generation: string; assessment: WorkspacePreflightAssessment }>
>();
...
async function archiveVariableScan(
  ...
  Readonly<{ variables: readonly Readonly<{ name: string; hasDefault: boolean }>[]; skipped: number; failed: boolean }>
> {
  ...
  try {
    for (const [index, member] of terraformMembers.entries()) {
      ...
      const source = await readBoundedProcessOutput(
        process,
        Math.max(1, ARCHIVE_SOURCE_LIMIT - bytes),
        ARCHIVE_FILE_TIMEOUT_MS,
      );
...
async function vcsConfiguration(workspace: WorkspaceRow): Promise<boolean | null> {
  ...
  const [token, installation] = await Promise.all([
    tokenId === null
      ...
      : db.query.oauthTokens.findFirst({ where: eq(oauthTokens.id, tokenId) }),
    installationId === null
      ? Promise.resolve(undefined)
      : db.query.githubAppInstallations.findFirst({ where: eq(githubAppInstallations.id, installationId) }),
  ]);
  if (token !== undefined) {
...
async function loadSnapshot(workspace: WorkspaceRow, now: number): Promise<Snapshot> {
  ...
  const archiveMembers =
    configuration?.archivePath === null || configuration?.archivePath === undefined
      ? null
  ...
  return hashValue({
    ...
    organization:
      organization === undefined
        ? null
        : {
            defaultIacBinary: organization.defaultIacBinary,
            defaultTerraformVersion: organization.defaultTerraformVersion,
          },
    configuration:
    ...
    variables: variables.map((entry) => ({
      ...
      value: hashValue(entry.variable.valueEncrypted ?? entry.variable.value),
    })),
    agents: poolAgents
      .map((agent) => ({
        id: agent.id,
        status: agent.status,
        iacBinaries: agent.iacBinaries,
...
async function configurationCheck(snapshot: Snapshot): Promise<WorkspacePreflightCheck> {
  ...
  if (configuration === undefined || configuration.archivePath === null) {
    if (workspace.executionMode === "local")
      return check(
        ...
        "unknown",
        false,
        "Local execution supplies configuration from the client context.",
        "Run the preflight from the CLI or upload a configuration version.",
        context,
        true,
      );
    return check(
      ...
      "failed",
      true,
      "No uploaded configuration version is available.",
      "Upload a configuration version for this workspace.",
      context,
    );
  }
  if (!(await Bun.file(configuration.archivePath).exists())) {
    return check(
      ...
      "failed",
      true,
      "The latest configuration archive is unavailable.",
  ...
  if (archiveMembers === null)
    return check(
      ...
      "Upload a valid configuration archive.",
      context,
    );
  ...
  if (workingDirectory !== "" && !archiveContainsWorkingDir(archiveMembers, workingDirectory)) {
    return check(
      ...
      "failed",
      true,
      "The configured working directory is not present in the archive.",
      "Set the working directory to a directory in the uploaded configuration or upload the matching source.",
      context,
    );
  }
  return check(
    ...
    "usable",
    true,
    "The latest configuration archive is available and its working directory is present.",
    undefined,
    context,
  );
}
...
async function inputCheck(snapshot: Snapshot): Promise<WorkspacePreflightCheck> {
  ...
  if (configuration?.archivePath === null || configuration?.archivePath === undefined || archiveMembers === null) {
    return check(
      ...
      "unknown",
      false,
      "Input completeness will be checked in the execution context.",
      "Upload configuration or run the preflight from the client context.",
      workspace.executionMode === "local" ? "client" : "worker",
      true,
    );
  }
  ...
  if (scan.failed)
    return check(
      ...
      "unknown",
      true,
      "Terraform variable declarations could not be inspected safely.",
      "Run a plan to let the selected engine validate the configuration inputs.",
      "control-plane",
    );
  const provided = new Set(
    variables.filter((entry) => entry.variable.category === "terraform").map((entry) => entry.variable.key),
  );
  const missing = scan.variables
    .filter((variable) => !variable.hasDefault && !provided.has(variable.name))
  ...
  if (missing.length > 0) {
    return check(
      ...
      "failed",
      true,
      `Required Terraform inputs are missing (${missing.slice(0, 20).join(", ")}${missing.length > 20 ? ", …" : ""}).`,
      "Add the missing workspace or variable-set values before starting the run.",
      "control-plane",
    );
  }
  if (scan.skipped > 0)
    return check(
      ...
      "unknown",
      false,
      "Some Terraform variable declarations could not be parsed by the advisory scanner.",
      "Run a plan to let the selected engine validate the configuration inputs.",
      "control-plane",
      true,
    );
  return check(
    ...
    "usable",
    true,
    "All scanned required Terraform inputs have values or defaults.",
    undefined,
    "control-plane",
  );
}
...
async function engineCheck(snapshot: Snapshot): Promise<WorkspacePreflightCheck> {
  ...
  if (tool !== "terraform" && tool !== "tofu")
    return check(
      ...
      "failed",
      true,
      "The workspace selects an unsupported IaC engine.",
      "Choose terraform or tofu in workspace settings.",
      "control-plane",
    );
  if (!validateVersion(version))
    return check(
      ...
      "failed",
      true,
      "The configured IaC version is invalid.",
      "Choose a supported exact version, latest, or version constraint.",
      "control-plane",
    );
  if (workspace.executionMode === "local")
    return check(
      ...
      "unknown",
      false,
      "The selected engine will be resolved in the client execution context.",
      "Run the preflight from the client context to verify the installed engine.",
      "client",
      true,
    );
  if (workspace.executionMode === "agent")
    return check(
      ...
      "unknown",
      false,
      "The selected engine version will be resolved by the matching agent.",
      "Run the preflight in the agent context to verify the installed engine version.",
      "agent",
      true,
    );
  ...
  if (!availability.ok)
    return check(
      ...
      true,
      availability.detail,
      "Install or select a supported version of the configured IaC engine.",
      "worker",
    );
  const installed = await installedBinaryVersions(tool);
  const exact = /^v?([0-9]+\.[0-9]+\.[0-9]+(?:-[A-Za-z0-9.]+)?)$/.exec(version.trim())?.[1];
  if (exact !== undefined && installed.includes(exact))
    return check("engine", "usable", true, `${tool} ${exact} is installed and ready.`, undefined, "worker");
  if (
    knownAvailableVersions(tool).length > 0 ||
    version.trim() === "latest" ||
    version.includes(">") ||
    version.includes("<") ||
    version.includes("=")
    ...
    return check(
      ...
      "reachable",
      true,
      `${tool} can be resolved when the run starts; binary resolution remains in the worker execution context.`,
      undefined,
      "worker",
      true,
    );
  ...
  return check(
    ...
    "unknown",
    true,
    `No local ${tool} binary or release metadata is available to confirm this version.`,
    "Run the preflight where the worker can resolve the selected engine or install it in the binary cache.",
    "worker",
  );
}
...
function targetCheck(snapshot: Snapshot): WorkspacePreflightCheck {
  ...
  if (workspace.executionMode === "local")
    return check(
      ...
      "configured",
      true,
      "The run will execute in the client context.",
      undefined,
      "client",
    );
  if (workspace.executionMode === "remote") {
    if (envFlag("TERRENCE_DISABLE_WORKER"))
      return check(
        ...
        "failed",
        true,
        "The Terrence worker is disabled.",
        "Enable the worker before starting a remote run.",
        "worker",
      );
  ...
  if (workspace.executionMode !== "agent")
    return check(
      ...
      "failed",
      true,
      "The workspace execution mode is unsupported.",
      "Choose local, remote, or agent execution.",
      "control-plane",
    );
  if (workspace.agentPoolId === null)
    return check(
      "execution-target",
      "failed",
      true,
      "No agent pool is attached to this workspace.",
      "Attach an agent pool before starting an agent run.",
      "agent",
    );
  const timeout = configuredHeartbeatTimeoutMs();
  const live = poolAgents.filter(
    (agent) =>
      agent.lastPingAt !== null &&
      now - agent.lastPingAt <= timeout &&
      !["exited", "errored", "unknown"].includes(agent.status),
  );
  ...
  if (matching.some((agent) => agent.status === "idle"))
    return check(
      "execution-target",
      "usable",
      true,
      "An online idle agent supports the selected IaC engine.",
      undefined,
      "agent",
    );
  if (matching.length > 0)
    return check(
      ...
      "reachable",
      true,
      "A matching agent is online but currently busy; the run may wait in the queue.",
      undefined,
      "agent",
      true,
    );
  if (live.length > 0)
    return check(
      ...
      "failed",
      true,
      "Online agents do not advertise the selected IaC engine.",
      "Connect an agent with the selected engine or change the workspace engine.",
      "agent",
    );
  return check(
    "execution-target",
    "failed",
    true,
    "No healthy agent is currently available in the attached pool.",
    "Start an agent in the attached pool and retry the preflight.",
    "agent",
  );
}
...
function healthCheck(): WorkspacePreflightCheck {
  ...
  if (degraded !== null)
    return check(
      ...
      "failed",
      true,
      `The control plane reports degraded storage: ${degraded}`,
      "Recover the Terrence storage volume before starting a run.",
      "control-plane",
    );
  return check(
    "health",
    "usable",
    true,
    "The control plane is responding and has no latched storage failure.",
    undefined,
    "control-plane",
  );
}
...
function sandboxCheck(snapshot: Snapshot): WorkspacePreflightCheck {
  ...
  if (workspace.executionMode === "agent")
    return check(
      ...
      "unknown",
      false,
      "Agent sandbox capability is verified when the agent claims the run.",
      "Run the preflight in the agent execution context to verify sandbox policy.",
      "agent",
      true,
    );
  if (workspace.executionMode === "local")
    return check(
      ...
      "unknown",
      false,
      "Local sandbox capability belongs to the client execution context.",
      "Run the preflight from the client context to verify sandbox policy.",
      "client",
      true,
    );
  try {
    if (!runSandboxRequired())
      return check(
        "sandbox",
        "configured",
        false,
        "The deployment explicitly disables the run sandbox.",
        "Enable the run sandbox for isolated execution.",
        "worker",
        true,
      );
    ...
    if (abi < 1)
      return check(
        ...
        "failed",
        true,
        "The required Landlock sandbox is unavailable to the worker.",
        "Install the landlock runner and enable Landlock on the worker host.",
        "worker",
      );
    if (runNetPolicy() === "deny" && abi < 4)
      return check(
        "sandbox",
        "failed",
        true,
        "The worker requires network isolation but its Landlock ABI is too old.",
        "Upgrade the worker kernel or use a Landlock ABI that supports network rules.",
        "worker",
      );
    return check(
      "sandbox",
      "usable",
      true,
      `The worker sandbox is available (Landlock ABI ${abi}).`,
      undefined,
      "worker",
    );
  } catch {
    return check(
      ...
      "unknown",
      true,
      "Sandbox policy could not be verified by the control plane.",
      "Run the preflight in the worker execution context.",
      "worker",
    );
  }
}
...
async function storageCheck(): Promise<WorkspacePreflightCheck> {
  ...
  if (degraded !== null)
    return check(
      ...
      "failed",
      true,
      `Terrence storage is degraded: ${degraded}`,
      "Free storage and restart or recover the Terrence instance.",
      "control-plane",
    );
  ...
  if (headroom === null)
    return check(
      ...
      "unknown",
      true,
      "Storage headroom could not be inspected.",
      "Check the storage volume from the worker execution context.",
      "control-plane",
    );
  if (headroom.availableBytes < headroom.minimumBytes)
    return check(
      "storage",
      "failed",
      true,
      "Terrence storage has insufficient free space for a run.",
      "Free space on the Terrence storage volume before starting the run.",
      "control-plane",
    );
  return check(
    "storage",
    "usable",
    true,
    "Terrence storage has enough free space for a run.",
    undefined,
    "control-plane",
  );
}
    ...
    checks.push(
      snapshot.vcsConfigured
        ? check(
            ...
            "configured",
            true,
            "The configured VCS integration belongs to this organization.",
            undefined,
            "control-plane",
          )
        : check(
            ...
            "failed",
            true,
            "The configured VCS integration is missing or belongs to another organization.",
            "Reconnect the VCS integration for this organization.",
            "control-plane",
          ),
    );
  }
  ...
  if (identity.configured) {
    checks.push(
      check(
        ...
        "configured",
        false,
        "Workload identity inputs are present; provider connectivity is deferred to the execution context.",
        "Run an identity probe in the worker or agent context to validate provider access.",
        snapshot.workspace.executionMode === "agent"
          ? "agent"
          : snapshot.workspace.executionMode === "local"
            ? "client"
            : "worker",
        true,
      ),
...
function optionalProbeChecks(snapshot: Snapshot, probes: readonly string[] | undefined): WorkspacePreflightCheck[] {
  ...
  return [...new Set(probes)]
    .filter((probe): boolean => probe === "connectivity" || probe === "identity")
    .map((probe) =>
      check(
        `probe-${probe}`,
        "unknown",
...
export function preflightResource(
  assessment: WorkspacePreflightAssessment,
