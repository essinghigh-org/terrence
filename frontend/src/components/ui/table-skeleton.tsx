...
export function TableSkeleton({
  ...
  return (
    <div role="status" aria-label={label} className={cn("w-full", className)}>
      ...
      {Array.from(
        { length: rows },
        (_unused, rowIndex): React.JSX.Element => (
          <div key={rowIndex} className="flex items-center gap-4 border-b border-border px-4 py-3 last:border-b-0">
            {Array.from(
              { length: cols },
              (_unused, colIndex): React.JSX.Element => (
                <div
                  key={colIndex}
