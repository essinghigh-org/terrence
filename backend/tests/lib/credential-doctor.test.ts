...
describe("credential doctor", () => {
  test("runs the AWS read-only exchange and never returns bearer material", async () => {
    ...
    const aws = {
      provider: "aws",
      values: { "role-arn": "arn:aws:iam::123456789012:role/doctor" },
    } as const satisfies CredentialDoctorConfiguration;
    const result = await runCredentialDoctor(
      aws,
      token,
      requesterFor((request) => {
        if (request.method === "HEAD") return response(200);
        if (request.body?.includes("AssumeRoleWithWebIdentity") === true) {
          return response(
            200,
            "<AssumeRoleWithWebIdentityResponse><Credentials><AccessKeyId>AKIA</AccessKeyId><SecretAccessKey>secret</SecretAccessKey><SessionToken>session</SessionToken></Credentials></AssumeRoleWithWebIdentityResponse>",
          );
        }
        return response(
          200,
          "<GetCallerIdentityResponse><GetCallerIdentityResult><Account>123456789012</Account><Arn>arn:aws:iam::123456789012:role/doctor</Arn><UserId>doctor</UserId></GetCallerIdentityResult></GetCallerIdentityResponse>",
        );
      }, calls),
    );
    ...
    expect(result.identity).toEqual({
      ...
      arn: "arn:aws:iam::123456789012:role/doctor",
      user_id: "doctor",
    });
  ...
  test("keeps provider permission denial separate from trust failure", async () => {
    ...
    const gcp = {
      provider: "gcp",
      values: {
        "workload-identity-provider-id": "projects/123/locations/global/workloadIdentityPools/pool/providers/provider",
      },
    } as const satisfies CredentialDoctorConfiguration;
    ...
    const result = await runCredentialDoctor(
      ...
      requesterFor((request) => {
        ...
        if (request.url === CREDENTIAL_DOCTOR_ENDPOINTS.gcpSts)
          return response(200, JSON.stringify({ access_token: "provider-access-token" }));
        return response(403, JSON.stringify({ error: { code: "PERMISSION_DENIED" } }));
      }, calls),
    );

    expect(result.status).toBe("failed");
    expect(result.checks.find((check) => check.name === "provider_access")).toMatchObject({
      status: "failed",
  ...
  test("reports DNS failures at reachability and does not attempt provider access", async () => {
    ...
    const azure = {
      provider: "azure",
      values: { identity: "client-id", "tenant-id": "tenant", "subscription-id": "subscription" },
    } as const satisfies CredentialDoctorConfiguration;
    const result = await runCredentialDoctor(
      azure,
      { ...token, claims: { ...token.claims, aud: "azure.workload.identity" } },
      requesterFor(() => {
    ...
    expect(result.status).toBe("failed");
    expect(result.checks.find((check) => check.name === "network_reachability")).toMatchObject({
      status: "failed",
      code: "dns_failure",
    });
    expect(result.checks.find((check) => check.name === "provider_access")).toMatchObject({
  ...
  test("classifies an Azure federated trust rejection distinctly", async () => {
    ...
    const result = await runCredentialDoctor(
      ...
      requesterFor((request) => {
        ...
        return response(
          400,
          JSON.stringify({ error: "invalid_client", error_description: "No matching federated identity record found" }),
        );
      }, []),
    );
  ...
  test("does not permit a subject or expired claim to look healthy", async () => {
    const aws = {
      provider: "aws",
      values: { "role-arn": "role", "expected-subject": "expected" },
    } as const satisfies CredentialDoctorConfiguration;
    const expired = { ...token, claims: { ...token.claims, exp: Math.floor(Date.now() / 1000) - 1, sub: "wrong" } };
    const result = await runCredentialDoctor(
      aws,
      expired,
      requesterFor(() => response(200), []),
    );
    expect(result.checks.find((check) => check.name === "trust")).toMatchObject({
      status: "failed",
  ...
  test("constructs only a health endpoint from the configured Vault address", () => {
    expect(credentialDoctorNetworkEndpoint("vault", { address: "https://vault.example.test/team/" })).toEqual({
      url: "https://vault.example.test/team/v1/sys/health",
    });
    expect(credentialDoctorNetworkEndpoint("vault", { address: "https://user:secret@vault.example.test" })).toEqual({
      error: "Vault address must be an HTTP(S) URL without embedded credentials",
    });
  });
});
