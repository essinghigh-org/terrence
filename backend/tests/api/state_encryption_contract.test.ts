...
test("real OpenTofu encrypted state is rejected without losing the current state or recovery bytes", async () => {
  ...
  ${
    ...
    ${
      mode === "new"
        ? ""
        : `key_provider "pbkdf2" "old" { passphrase = "synthetic-${mode === "wrong" ? "wrong" : "old"}-key-0001" }
    method "aes_gcm" "old" { keys = key_provider.pbkdf2.old }`
    }
    ${
      mode === "new" || mode === "migrate"
        ? `key_provider "pbkdf2" "new" { passphrase = "synthetic-new-key-0002" }
...
`,
    ...
    await config("new");
    expect(JSON.parse(await successful("state", "pull"))).toMatchObject({
      outputs: { probe: { value: "synthetic-encrypted-value" } },
    });
    await config("old");
    ...
    const create = (serial: number, payload: string, inline: boolean): Promise<Response> =>
    ...
    for (const encrypted of [original, migrated]) {
      ...
      expect(
        parseTerraformStatePayload(JSON.stringify({ ...JSON.parse(plaintext), ...JSON.parse(encrypted) })),
      ).toBeNull();
      ...
      const reservation = await expectSuccessResponse(
        await create(parsed.serial + 1, encrypted, false),
        201,
        "state-versions",
      );
      await rejected(
        await request(reservation.attributes["hosted-state-upload-url"] as string, { method: "PUT", body: encrypted }),
      );
      expect(await db.query.stateVersions.findFirst({ where: eq(stateVersions.id, reservation.id) })).toMatchObject({
        status: "pending",
        statePayload: null,
      });
      ...
      try {
        ...
        await insertStateVersionWithSerialRetry({
          ...
          workspaceId,
          statePayload: await encryptStatePayload(encrypted),
        });
      } catch (error) {
        failure = error;
      }
      ...
      expect(await (await request(`/api/v2/state-versions/${baseline.id}/download`, { headers })).text()).toBe(
        plaintext,
      );
    }
    ...
    expect(run.attributes).toMatchObject({
      ...
      "recovery-state-representation": "opentofu-encrypted",
    });
    ...
    await db.insert(stateVersions).values({
      id: legacyId,
      workspaceId,
      serial: parsed.serial + 10,
      statePayload: await encryptStatePayload(migrated),
      jsonState: await encryptStatePayload(migrated),
    });
    const legacy = await expectSuccessResponse(
      await request(`/api/v2/state-versions/${legacyId}`, { headers }),
      200,
      "state-versions",
    );
    ...
    for (const path of [
      ...
      `/workspaces/${workspaceId}/dependency-graph`,
      `/workspaces/${workspaceId}/current-state-version-outputs`,
      `/state-versions/${legacyId}/outputs`,
      `/state-versions/${legacyId}/state-version-outputs`,
      `/state-versions/${legacyId}/json-download`,
    ]) {
      await rejected(await request(`/api/v2${path}`, { headers }), 422);
    }
    await rejected(
      await request(`/api/v2/state-versions/${legacyId}/actions/rollback`, { method: "POST", headers }),
      422,
    );
    const workspace = await expectSuccessResponse(
      await request(`/api/v2/workspaces/${workspaceId}?include=outputs`, { headers }),
      200,
      "workspaces",
    );
    expect(workspace.relationships?.["outputs"]).toMatchObject({
      data: null,
      meta: { "unavailable-reason": CLIENT_ENCRYPTED_STATE_ERROR },
    });
    const stateTool = stateTools.find((tool) => tool.name === "get_workspace_state");
    expect(stateTool).toBeDefined();
    const mcpState = await stateTool!.handler(
      { userId: seed.userId, orgId: null, teamId: null, tokenId: seed.tokenId, scopes: null },
