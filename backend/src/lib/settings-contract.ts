...
export const oidcSigningAlgorithms = [
  "HS256",
  "HS384",
  "HS512",
  "RS256",
...
function validHttpUrl(value: unknown, rule: Rule): boolean {
  ...
  try {
    ...
    return (
      ...
      url.password === "" &&
      url.hash === "" &&
      (rule.allowQuery === true || url.search === "")
    );
  } catch {
    return false;
  }
...
function validWindow(value: unknown): boolean {
  ...
  if (Object.keys(window).some((key): boolean => !["days", "start-time", "end-time", "timezone"].includes(key)))
    return false;
...
function validCidrs(value: unknown): boolean {
  return (
    Array.isArray(value) &&
    value.length <= 256 &&
    value.every((entry: unknown): boolean => typeof entry === "string" && validConfiguredCidr(entry))
  );
}
...
function matchesRule(value: unknown, rule: Rule): boolean {
  ...
  switch (rule.kind) {
    case "url":
      return validHttpUrl(value, rule);
    case "cidrs":
      return validCidrs(value);
    case "windows":
      return validWindows(value);
    case "json":
      return validJson(value);
    case "issuer":
      return typeof value === "string" && validOidcIssuer(value);
    case "boolean":
      return typeof value === "boolean";
    case "string":
      return validString(value, rule);
    case "enum":
      return validEnum(value, rule);
    case "syslog-targets":
      return validSyslogTargets(value);
    case "ascii":
      return validAscii(value, rule);
    case "integer":
      return validInteger(value, rule);
    case "headers":
      return validHeaders(value);
  }
}
...
function validateEnabledAuthentication(group: string, values: Readonly<Record<string, unknown>>, input: boolean): void {
  ...
  if (group === "oidc") {
    if (!present("issuer") || !present("client-id"))
      throw new SettingsValidationError("Enabled OIDC requires issuer and client-id", input);
    const algorithm = values["signing-alg"];
    if (
      (values["pkce-method"] !== "S256" || (typeof algorithm === "string" && algorithm.startsWith("HS"))) &&
      !present("client-secret")
    ) {
      throw new SettingsValidationError(
        "OIDC requires a client secret unless using S256 with asymmetric signing",
        input,
      );
    }
  }
  if (group === "ldap") {
    if (!present("host") || !present("base-dn"))
      throw new SettingsValidationError("Enabled LDAP requires host and base-dn", input);
    if (present("bind-dn") && !present("bind-password"))
      throw new SettingsValidationError("LDAP service account requires a bind password", input);
  }
}
