  ...
  test("uniqueness and missing relationships produce portable, redacted conflicts", async () => {
    ...
    try {
      const unique = await failure(() =>
        db.insert(users).values({ id: `${id}-second`, username: id, passwordHash: "fixture-only" }),
      );
      const foreign = await failure(() =>
        db.insert(organizationMemberships).values({ id: `${id}-membership`, userId: id, orgId: `${id}-missing` }),
      );
      ...
      for (const error of [unique, foreign]) {
        ...
        const body = handleAppError({
          code: "UNKNOWN",
          error,
          set,
          request: { url: "http://localhost/api/v2/invariant" },
        });
