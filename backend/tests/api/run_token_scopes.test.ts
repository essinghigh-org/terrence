...
const STATE_A = JSON.stringify({
  version: 4,
  terraform_version: "1.7.0",
  outputs: { a: { value: "a", type: "string" } },
});
const STATE_B = JSON.stringify({
  version: 4,
  terraform_version: "1.7.0",
  outputs: { b: { value: "b", type: "string" } },
});
...
describe("run-scoped token scope/lifetime/revocation (RUN-022)", () => {
  ...
  beforeAll(async () => {
    ...
    await db
      ...
      .values({ id: `mem-rtok-${suffix}`, userId: `user-rtok-${suffix}`, orgId, role: "owner" });
    await db.insert(apiTokens).values({
      id: userTokenId,
      token: hashAuthenticationToken(`rtok-user-${suffix}`),
      userId: `user-rtok-${suffix}`,
    });
    ...
    const post = await app.handle(
      new Request(`http://terrence.test/api/v2/workspaces/${wsA}/state-versions`, {
        method: "POST",
        headers: { Authorization: `Bearer ${userToken}`, "Content-Type": "application/vnd.api+json" },
        body: JSON.stringify({
          data: {
            type: "state-versions",
            attributes: { serial: 1, state: stateA, md5: createHash("md5").update(stateA).digest("base64") },
          },
        }),
      }),
    );
    ...
    const postB = await app.handle(
      new Request(`http://terrence.test/api/v2/workspaces/${wsB}/state-versions`, {
        method: "POST",
        headers: { Authorization: `Bearer ${userToken}`, "Content-Type": "application/vnd.api+json" },
        body: JSON.stringify({
          data: {
            type: "state-versions",
            attributes: { serial: 1, state: stateB, md5: createHash("md5").update(stateB).digest("base64") },
          },
        }),
      }),
    );
