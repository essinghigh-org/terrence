 ...
 *      binary package.
 *
 * The guards are pure predicates, so this suite fuzzes them directly with
 * fast-check over path-shaped strings plus curated adversarial corpora, and
 * asserts the predicates never disagree with the spec they encode.
 */
...
const pathArbitrary = (maxLength = 24): fc.Arbitrary<string> =>
  fc.array(fc.constantFrom(...PATH_ALPHABET), { maxLength }).map((parts) => parts.join(""));
...
describe("zipEntryEscapes (binaryManager)", () => {
  ...
  test("fuzz: implementation never disagrees with the spec", () => {
    fc.assert(
      fc.property(pathArbitrary(), (entry) => {
        expect(zipEntryEscapes(entry)).toBe(zipEscapesSpec(entry));
      }),
      { numRuns: 20_000, seed: 0x2a62 },
    );
  });
...
describe("unexpectedZipMembers (binaryManager)", () => {
  ...
  test("property: classification equals normalize-then-compare", () => {
    ...
    fc.assert(
      fc.property(pathArbitrary(8), fc.array(pathArbitrary(12), { maxLength: 7 }), (candidate, entries) => {
        const bin = candidate || "tofu";
        const expected = entries.filter((e) => normalize(e) !== bin);
        expect(unexpectedZipMembers(entries, bin).sort()).toEqual(expected.sort());
      }),
      { numRuns: 5000, seed: 0xbeef },
    );
  });
  ...
  test("property: a member whose normalized form equals the binary is never unexpected", () => {
    ...
    const bin = "tofu";
    fc.assert(
      fc.property(fc.array(pathArbitrary(8), { maxLength: 3 }), (tail) => {
        const unexpected = unexpectedZipMembers([bin, ...tail], bin);
...
describe("tarMemberPathUnsafe (worker)", () => {
  ...
  test("fuzz: implementation never disagrees with its spec", () => {
    ...
    fc.assert(
      fc.property(pathArbitrary(), (member) => {
        expect(tarMemberPathUnsafe(member)).toBe(spec(member));
      }),
      { numRuns: 20_000, seed: 0x7a7a },
    );
  });
});
...
describe("tarMemberIsForbiddenSpecial (worker)", () => {
  ...
  test("fuzz: rejection is exactly the six-char set", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...alphabet), (c) => {
        expect(tarMemberIsForbiddenSpecial(c)).toBe(forbidden.has(c));
      }),
      { numRuns: 10_000, seed: 0x51ec },
    );
  });
});
