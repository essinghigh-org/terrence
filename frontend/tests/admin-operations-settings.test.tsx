...
test("uses provider defaults and saves an optional base URL without an endpoint URL", async () => {
  ...
  await waitFor((): void => {
    expect(view.getByLabelText("Base URL (optional)")).toBeTruthy();
  });
  ...
  const order = ["Provider", "Model", "Reasoning effort", "Base URL (optional)"].map((label): number =>
    labels.indexOf(label),
  );
  expect(
    order.every((index, position): boolean => index >= 0 && (position === 0 || index > order[position - 1]!)),
  ).toBe(true);
});
...
test("AdminLoggingSettings loads and saves logging configuration independently", async () => {
  ...
  globalThis.fetch = mock(async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
    ...
    if (url === "/api/v2/admin/logging-settings") {
      return json({
        data: { attributes: { enabled: true, "log-level": "info", "syslog-targets": ["udp://syslog.local:514"] } },
      });
    }
    throw new Error(`Unexpected request: ${url}`);
  ...
  await waitFor((): void => {
    expect(view.getByLabelText("Remote destinations")).toBeTruthy();
  });
...
test("AdminApprovalWebhook loads and saves webhook configuration independently", async () => {
  ...
  await waitFor((): void => {
    expect(view.getByLabelText("Callback URL (optional)")).toBeTruthy();
  });
...
test("AdminMaintenanceWindows loads and saves maintenance windows independently", async () => {
  ...
  await waitFor((): void => {
    expect(view.getByText("No maintenance windows yet")).toBeTruthy();
  });
  fireEvent.click(view.getByRole("button", { name: "Add maintenance window" }));
  await waitFor((): void => {
    expect(view.getByText("Window 1")).toBeTruthy();
  });
  fireEvent.click(view.getByRole("button", { name: "Save changes" }));
  await waitFor((): void => {
    expect(patchBody).toBeDefined();
