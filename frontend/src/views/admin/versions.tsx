    ...
    <div className="space-y-6">
      <Card>
        <CardHeader variant="section">
          ...
          <CardDescription>
            Register a pinned Terraform or OpenTofu binary for workspace execution. A SHA256 checksum is strongly
            recommended when using a custom download URL.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleAddVersion} className="flex gap-4 items-end">
            <div className="space-y-1 flex-1">
              <label className="text-xs font-medium text-foreground/85" htmlFor="admin-version">
                Version
              </label>
              <Input
                id="admin-version"
            ...
            <div className="space-y-1 flex-1">
              <label className="text-xs font-medium text-foreground/85" htmlFor="admin-version-url">
            ...
            <div className="space-y-1 flex-1">
              <label className="text-xs font-medium text-foreground/85" htmlFor="admin-version-sha">
                SHA256 (optional)
              </label>
              <Input
      ...
      <Card>
        <CardHeader variant="section">
          ...
          <CardDescription>
            Workspace settings choose which engine and version constraint to run. Custom binaries are downloaded and
            verified when a URL and checksum are provided.
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  tfVersions.map(
                    (v): React.JSX.Element => (
                      <TableRow key={v.id} className="hover:bg-muted/50">
                        ...
                        <TableCell className="px-4 py-3 text-xs text-muted-foreground truncate max-w-xs">
                          {v.attributes.url ?? "Default download"}
                        </TableCell>
                        <TableCell className="px-4 py-3 text-xs font-mono text-muted-foreground/70">
                          {v.attributes.sha != null ? v.attributes.sha.slice(0, 12) + "…" : "—"}
                        </TableCell>
                        <TableCell className="px-4 py-3">
                          <Button
                            ...
                            onClick={(): void => {
                              setVersionToDelete({ id: v.id, label: v.attributes.version ?? v.id });
                            }}
                          >
