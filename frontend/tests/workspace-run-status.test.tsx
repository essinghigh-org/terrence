...
test("shows the latest run status instead of treating an unlocked workspace as a status", async () => {
  ...
  await waitFor((): void => {
    expect(view.getByText("Policy override required")).toBeTruthy();
  });
        ...
        data: [
          {
            ...
            attributes: { name: "production", locked: false },
            relationships: {
              project: { data: { id: "project-1", type: "projects" } },
              ...currentRunRelationship(null),
            },
          },
        ],
        included: [],
...
test("KPI totals stay org-wide on a bounded filtered page", async () => {
        ...
        data: [
          {
            ...
            relationships: currentRunRelationship("run-1"),
          },
        ],
        included: [includedRun("run-1", "post_plan_running", "ws-1")],
        meta: {
          pagination: { "total-count": 80, "total-pages": 2 },
          "workspace-summary": { total: 10000, locked: 120, "run-statuses": { post_plan_running: 80, errored: 15 } },
        },
      });
  ...
  const view = render(
    <MemoryRouter initialEntries={["/app/acme?status=running"]}>
      <Routes>
        <Route path="/app/:orgName" element={<Workspaces />} />
      </Routes>
    </MemoryRouter>,
  ...
  await waitFor((): void => {
    expect(view.getByText("Running post-plan tasks")).toBeTruthy();
  });
...
test("missing summary is unavailable rather than misrepresented by the page count", async () => {
  globalThis.fetch = mock(async (input: string | URL | Request): Promise<Response> => {
    const url = urlOf(input);
    if (url.includes("/workspaces?"))
      return json({ data: [{ id: "ws-1", attributes: { name: "visible", locked: true } }] });
    if (url.includes("/projects?")) return json({ data: [] });
  ...
  await waitFor((): void => {
    expect(view.getByText("visible")).toBeTruthy();
  });
...
test("Attention tile counts errored runs and its filter includes them (issue #612)", async () => {
  ...
  await waitFor((): void => {
    expect(view.getByText("broken")).toBeTruthy();
  });
  ...
  await waitFor((): void => {
    expect(seen.some((entry: string): boolean => entry.includes("current-run") && entry.includes("errored"))).toBe(
      true,
    );
  });
});
