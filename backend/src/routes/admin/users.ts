...
async function checkAdminUserCreate(
  ...
  if (!adminPolicy.ok) {
    (set as { status: number }).status = 422;
    return {
      error: { errors: [{ status: "422", title: "Unprocessable Entity", detail: adminPolicy.errors.join(" ") }] },
    };
  }
  const existing = await db.query.users.findFirst({
    where: or(eq(users.username, username), ...(email === null ? [] : [eq(users.email, email)])),
  });
...
async function insertAdminUser(
  ...
  if (created === undefined) {
    ...
    return { error: { errors: [{ status: "500", title: "Internal Server Error" }] } };
  }
  return { created };
}
...
async function parseAdminEmailUpdate(
  ...
  if (email !== null) {
    ...
    if (claimant !== undefined && claimant.id !== userId) {
      (set as { status: number }).status = 409;
      return {
        error: { errors: [{ status: "409", title: "Conflict", detail: "That email address is already in use" }] },
      };
    }
  }
        ...
        errors: [
          {
            status: "422",
            title: "Unprocessable Entity",
            detail: !policy.ok ? policy.errors.join(" ") : "Passwords do not match.",
          },
        ],
      },
    };
...
async function endImpersonationSession(
  ...
  const impersonationToken =
    tokenId !== undefined && isImpersonationTokenId(tokenId)
...
export const usersRoutes = new Elysia({ name: "admin-users" })
  ...
  .get("/api/v2/admin/users", async ({ user, request, set }: ParamCtx): Promise<unknown> => {
    ...
    if (q !== "") {
      const pattern = `%${q}%`;
      const usernameOrEmail = or(
        caseInsensitiveLike(users.username, pattern),
        caseInsensitiveLike(users.email, pattern),
      );
    ...
    return {
      data: allUsers.map((u: UserItem) => adminUserResource(u)),
  ...
  .delete(
    ...
    async ({
      ...
      const now = Date.now();
      const emailHash = (targetUser as unknown as { email?: string | null }).email
        ? sensitiveIdentifierHash(String((targetUser as unknown as { email?: string | null }).email).toLowerCase())
        : null;
      ...
      await auditLog("delete", "users", userId, user?.id ?? null, null, {
        username: (targetUser as unknown as { username?: string }).username,
      });
      publish("authz.changed", { "user-id": userId });
      (set as { status: number }).status = 204;
      return {};
  ...
  // --- User Actions ---
  .post(
    "/api/v2/admin/users/:user_id/actions/reset_password",
          ...
          errors: [
            {
              status: "409",
              title: "Conflict",
              detail: "This account changed while resetting its password. Reload and try again.",
            },
          ],
        };
      }
  ...
  .post("/api/v2/admin/users/:user_id/actions/suspend", async ({ params, user, set }: ParamCtx): Promise<unknown> => {
    ...
    if ((target as Record<string, unknown>)["isSuspended"] === true) {
      ...
      return { errors: [{ status: "400", title: "Bad Request", detail: "User is already suspended" }] };
    }
    await db.transaction(async (tx: unknown): Promise<void> => {
      const t = tx as typeof db;
    ...
    try {
      const { auditLog: al } = await import("../../lib/utils");
      await al("suspend", "users", userId, user?.id ?? null, null, {
        username: (target as unknown as { username?: string }).username,
      });
    } catch {}
  ...
  .post("/api/v2/admin/users/:user_id/actions/unsuspend", async ({ params, user, set }: ParamCtx): Promise<unknown> => {
    ...
    if ((target as Record<string, unknown>)["isSuspended"] !== true) {
      (set as { status: number }).status = 400;
      return { errors: [{ status: "400", title: "Bad Request", detail: "User is not suspended" }] };
    }
    await db.update(users).set({ isSuspended: false }).where(eq(users.id, userId));
    try {
      const { auditLog: al } = await import("../../lib/utils");
      await al("unsuspend", "users", userId, user?.id ?? null, null, {
        username: (target as unknown as { username?: string }).username,
      });
    } catch {}
    publish("authz.changed", { "user-id": userId });
  ...
  .post(
    "/api/v2/admin/users/:user_id/actions/grant_admin",
    async ({ params, user, set }: ParamCtx): Promise<unknown> => {
      ...
      if (target.isSiteAdmin === true) {
        (set as { status: number }).status = 400;
        return { errors: [{ status: "400", title: "Bad Request", detail: "User is already a site admin" }] };
      }
  ...
  .post(
    "/api/v2/admin/users/:user_id/actions/revoke_admin",
    async ({ params, user, set }: ParamCtx): Promise<unknown> => {
      if (user?.isSiteAdmin !== true) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      ...
      if (target.isSiteAdmin !== true) {
        (set as { status: number }).status = 400;
        return { errors: [{ status: "400", title: "Bad Request", detail: "User is not a site admin" }] };
      }
      if (target.scimSiteAdmin === true) {
        (set as { status: number }).status = 409;
        return {
          errors: [
            { status: "409", title: "Conflict", detail: "Site-admin access is managed by the configured SCIM group" },
          ],
        };
      }
  ...
  .post(
    "/api/v2/admin/users/:user_id/actions/grant_site_auditor",
    async ({ params, user, set }: ParamCtx): Promise<unknown> => {
      if (user?.isSiteAdmin !== true) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      ...
      if ((target as Record<string, unknown>)["isSiteAuditor"] === true) {
        (set as { status: number }).status = 400;
        return { errors: [{ status: "400", title: "Bad Request", detail: "User is already a site auditor" }] };
      }
  ...
  .post(
    "/api/v2/admin/users/:user_id/actions/revoke_site_auditor",
    async ({ params, user, set }: ParamCtx): Promise<unknown> => {
      if (user?.isSiteAdmin !== true) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
  ...
  .post(
    "/api/v2/admin/users/:user_id/actions/disable_two_factor",
    async ({ params, user, set }: ParamCtx): Promise<unknown> => {
      if (user?.isSiteAdmin !== true) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      ...
      if (mfa === undefined || mfa.enabled !== true) {
        ...
        return {
          errors: [
            { status: "400", title: "Bad Request", detail: "User does not have two-factor authentication enabled" },
          ],
        };
      }
  ...
  .post(
    "/api/v2/admin/users/:user_id/actions/impersonate",
    async ({ params, user, set }: ParamCtx): Promise<unknown> => {
      if (user?.isSiteAdmin !== true) {
        await auditLog(
          "impersonate",
      ...
      if (target.id === user.id || target.isSiteAdmin === true) {
        ...
        (set as { status: number }).status = 422;
        return {
          errors: [{ status: "422", title: "Unprocessable Entity", detail: "This user cannot be impersonated" }],
        };
      }
      if ((target as Record<string, unknown>)["isSuspended"] === true) {
        (set as { status: number }).status = 404;
      ...
      return {
        data: {
          ...
          attributes: { token: rawToken, "expires-at": new Date(expiresAt).toISOString(), "user-id": target.id },
        },
      };
    },
  )
  .post("/api/v2/admin/users/actions/unimpersonate", async ({ user, token, set }: ParamCtx): Promise<unknown> => {
    ...
    await auditLog(
      ...
      user?.id ?? null,
      null,
      { reason: "not-an-impersonation-session" },
      { result: "denied", immutable: true },
    );
    (set as { status: number }).status = 400;
    return {
      errors: [{ status: "400", title: "Bad Request", detail: "The current session is not an impersonation session" }],
    };
  });
