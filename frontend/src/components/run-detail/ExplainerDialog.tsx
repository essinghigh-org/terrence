    ...
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-5 py-3">
      <p className="min-w-0 truncate text-xs text-muted-foreground">
        {explainer.explainerModel !== ""
          ? `Generated by ${explainer.explainerModel}${explainer.explainerReasoningEffort !== null ? ` - ${explainer.explainerReasoningEffort}` : ""}`
          : ""}
      </p>
      <div className="flex items-center gap-2">
        {explaining ? (
          ...
          <Button
            ...
            size="sm"
            onClick={(): void => {
              void explainer.handleExplain(explainer.explainerKind, true);
            }}
      ...
      <DialogContent className="flex max-h-[80dvh] flex-col gap-0 overflow-hidden overscroll-contain p-0 sm:max-w-2xl">
        <DialogHeader className="border-b border-border px-5 py-4 pr-16 text-left">
          ...
          <DialogDescription className="sr-only">
            A plain-language explanation generated from the stored{" "}
            {explainer.explainerKind === "apply" ? "apply failure" : "plan"} data.
          </DialogDescription>
        </DialogHeader>
        <div
        ...
        >
            ...
            >
              ...
              <div>
                ...
                <p className="mt-2 text-xs text-destructive/90">
                  Check the explainer endpoint, model, and API key, then try again.
                </p>
              </div>
            </div>
          )}
          {explaining && (
            <details
              ...
              onToggle={(event): void => {
                explainer.setExplainerThinkingOpen(event.currentTarget.open);
              }}
            >
              ...
              <div className="border-t border-border px-3 py-2 text-xs leading-5 text-muted-foreground">
                <p className="break-words whitespace-pre-wrap">
                  {explainerThinking !== ""
                    ? explainerThinking
                    : "Reasoning will appear here if the model provides it."}
                </p>
              </div>
            </details>
          )}
