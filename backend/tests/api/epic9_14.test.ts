...
describe("Epics 9-14: Runs Comments, Tasks, Tokens, Entitlements & Audit Logs", () => {
  ...
  it("manages run apply comments, comments API, and plan JSON output", async () => {
    ...
    // Comment resources carry body + a run-event relationship (the reference format contract);
    // actor attribution lives on the run-event resource, not the comment.
    expect(listBody.data[0].relationships["run-event"]).toBeDefined();
