...
import { db } from "../db";
import { configurationVersions, workspaces } from "../db/schema";
...
type CvParams = { workspace_id: string };
type CvIdParams = { cv_id: string };

export const configurationVersionRoutes = new Elysia({ name: "configurationVersions" })
  .use(authPlugin)
  .get("/api/v2/workspaces/:workspace_id/configuration-versions", async ({ params: { workspace_id }, user, orgId, request, set }): Promise<unknown> => {
    const ws = await findAuthorizedWorkspace(workspace_id, user?.id, orgId);
    if (ws === null || ws === undefined) { set.status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
  ...
  .post("/api/v2/workspaces/:workspace_id/configuration-versions", async ({ params: { workspace_id }, body, user, orgId, request, set }): Promise<unknown> => {
    const ws = await findAuthorizedWorkspace(workspace_id, user?.id, orgId);
    if (ws === null || ws === undefined) { set.status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
    ...
    await db.insert(configurationVersions).values({ id, workspaceId: workspace_id, status: "pending", speculative: (attributes.speculative as boolean) ?? false, source: (attributes.source as string) ?? "tfe-api" });
    set.status = 201;
    return { data: { id, type: "configuration-versions", attributes: { "auto-queue-runs": (attributes["auto-queue-runs"] as boolean) ?? false, speculative: (attributes.speculative as boolean) ?? false, status: "pending", source: (attributes.source as string) ?? "tfe-api", "upload-url": `/api/v2/configuration-versions/${id}/upload`, "download-url": `/api/v2/configuration-versions/${id}/download` } } };
  })
  .get("/api/v2/configuration-versions/:cv_id", async ({ params: { cv_id }, user, orgId, request, set }): Promise<unknown> => {
    const cv = await db.query.configurationVersions.findFirst({ where: eq(configurationVersions.id, cv_id) });
    if (cv === null || cv === undefined) { set.status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
    const ws = await db.query.workspaces.findFirst({ where: eq(workspaces.id, cv.workspaceId) });
    if (ws === null || ws === undefined || !(await checkOrgPermission(user?.id, ws.orgId, "member", orgId))) { set.status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
  ...
  .put("/api/v2/configuration-versions/:cv_id/upload", async ({ params: { cv_id }, request, set }): Promise<unknown> => {
    const cv = await db.query.configurationVersions.findFirst({ where: eq(configurationVersions.id, cv_id) });
    if (cv === null || cv === undefined) { set.status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
  ...
  .get("/api/v2/configuration-versions/:cv_id/download", async ({ params: { cv_id }, user, orgId, set }): Promise<unknown> => {
    const cv = await db.query.configurationVersions.findFirst({ where: eq(configurationVersions.id, cv_id) });
    if (cv === null || cv === undefined) { set.status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
    const ws = await db.query.workspaces.findFirst({ where: eq(workspaces.id, cv.workspaceId) });
    if (ws === null || ws === undefined || !(await checkOrgPermission(user?.id, ws.orgId, "member", orgId))) { set.status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
  ...
  .get("/api/v2/configuration-versions/:cv_id/ingress-attributes", async ({ params: { cv_id }, user, orgId: tokenOrgId, set }): Promise<unknown> => {
    const cv = await db.query.configurationVersions.findFirst({ where: eq(configurationVersions.id, cv_id) });
    if (cv === null || cv === undefined) { set.status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
    const ws = await db.query.workspaces.findFirst({ where: eq(workspaces.id, cv.workspaceId) });
    if (ws === null || ws === undefined || !(await checkOrgPermission(user?.id, ws.orgId, "member", tokenOrgId))) { set.status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
