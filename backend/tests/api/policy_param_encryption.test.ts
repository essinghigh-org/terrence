...
describe("policy-set parameter encryption (#577)", () => {
  ...
  const createParam = (key: string, value: string, sensitive: boolean) =>
    request(`/api/v2/policy-sets/${setId}/parameters`, "POST", {
  ...
  afterAll(async () => {
    await db
      ...
      .where(eq(policySetParameters.policySetId, setId))
      .catch((): void => undefined);
    await db
      .delete(policySets)
      .where(eq(policySets.id, setId))
      .catch((): void => undefined);
    await db
      .delete(apiTokens)
  ...
  it("rotates sensitive values through updates", async () => {
    ...
    expect(
      await variableValueForRead({ value: after?.value ?? "", valueEncrypted: after?.valueEncrypted ?? null }),
    ).toBe("second");
  });
  ...
  it("refuses to downgrade a sensitive parameter with an explicit null value", async () => {
    const created = (await (await createParam("NO_NULL_DOWNGRADE", "nullsecret", true)).json()) as {
      data: { id: string };
    };
    const patchRes = await request(`/api/v2/policy-sets/${setId}/parameters/${created.data.id}`, "PATCH", {
