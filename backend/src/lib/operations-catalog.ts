...
export const WORKSPACE_BLUEPRINTS: readonly BlueprintDefinition[] = [
  {
    ...
    parameters: [
      ...
      {
        name: "variable-set-ids",
        required: false,
        description: "Approved variable-set IDs; values are never embedded",
      },
    ],
  ...
  },
  {
    ...
    parameters: [
      ...
      {
        name: "assessment-interval",
        required: false,
        description: "Existing assessment interval accepted by the workspace API",
      },
    ],
...
export function renderWorkspaceAdoptionHcl(input: WorkspaceAdoptionExport): string {
  ...
  const lines: string[] = [
    ...
    ...(input.workspace.terraformVersion === null
      ? []
      : [`  terraform_version = ${hclString(input.workspace.terraformVersion)}`]),
  ...
  if (input.variableSets.length > 0) {
    ...
    for (const variableSet of [...input.variableSets].sort((a, b) => a.id.localeCompare(b.id))) {
      lines.push(
        `# variable-set ${variableSet.name} (${variableSet.id})`,
        `# import { to = tfe_workspace_variable_set.${stableConfigName(variableSet.name, "variable_set")} id = ${hclString(variableSet.id)} }`,
      );
    }
  }
  ...
  lines.push(
    "",
    "# Unsupported or operator-reviewed fields:",
    "# - credentials and sensitive variable values",
    "# - provider-side notification delivery state",
    "# - state snapshots and run history",
  );
  return `${lines.join("\n")}\n`;
}
...
export function webhookRepository(payload: Readonly<Record<string, unknown>>): string | null {
  ...
  if (repository !== null && typeof repository === "object" && !Array.isArray(repository)) {
    const value =
      ...
      (repository as Record<string, unknown>)["path_with_namespace"];
