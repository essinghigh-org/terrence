...
function parseCidrRangeBody(body: unknown): { attributes: Record<string, unknown>; listId: string } {
  ...
  const listId =
    typeof (listRel?.["data"] as Record<string, unknown>)?.["id"] === "string"
      ? ((listRel?.["data"] as Record<string, unknown>)["id"] as string)
      : "";
        ...
        errors: [
          {
            ...
            detail: range === undefined ? "cidr-block is required" : "cidr-block must be a valid CIDR block",
          },
        ],
      },
    };
...
function parseCidrRangeUpdates(
  ...
  const cidrBlock =
    typeof attributes["cidr-block"] === "string"
  ...
  if (cidrBlock !== "") {
    if (!isCidrBlock(cidrBlock)) {
      ...
      return {
        error: {
          errors: [{ status: "422", title: "Unprocessable Entity", detail: "cidr-block must be a valid CIDR block" }],
        },
      };
    }
...
async function createCidrRange(
  listId: string,
  attributes: Record<string, unknown>,
): Promise<CidrRangeItem | "invalid" | undefined> {
  const rawValue =
    typeof attributes["cidr-block"] === "string"
      ? attributes["cidr-block"].trim()
      : typeof attributes["value"] === "string"
...
export const cidrRangeRoutes = new Elysia({ name: "cidr-ranges" })
  ...
  // CIDR Range Lists
  .get(
    "/api/v2/organizations/:org_name/cidr-range-lists",
    async ({ params, user, request, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      const total =
        (await db.select({ value: count() }).from(cidrRangeLists).where(eq(cidrRangeLists.orgId, org.id)))[0]?.value ??
        0;
      ...
      return {
        data: await Promise.all(lists.map(async (l) => cidrRangeListResource(l))),
  ...
  .post(
    ...
    async ({ params, user, body, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      const enforcementScope =
        typeof attributes["enforcement-scope"] === "string" ? attributes["enforcement-scope"] : "organization";
  ...
  .get(
    ...
    async ({ request, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (typeof listId !== "string" || listId === "") {
        ...
        return {
          errors: [
            { status: "422", title: "Unprocessable Entity", detail: "filter[cidr-range-list][id] filter is required" },
          ],
        };
      }
      const list = await db.query.cidrRangeLists.findFirst({ where: eq(cidrRangeLists.id, listId) });
  ...
  .get(
    "/api/v2/cidr-range-lists/:id/relationships/cidr-ranges",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const list = await db.query.cidrRangeLists.findFirst({ where: eq(cidrRangeLists.id, params["id"] ?? "") });
      if (list === undefined || !(await checkOrgPermission(user?.id, list.orgId, "member", tokenOrgId, tokenTeamId))) {
        (set as { status: number }).status = 404;
  ...
  .post(
    "/api/v2/cidr-range-lists/:id/relationships/agent-pools",
    async ({ params, user, body, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const list = await db.query.cidrRangeLists.findFirst({ where: eq(cidrRangeLists.id, params["id"] ?? "") });
      if (list === undefined || !(await checkOrgPermission(user?.id, list.orgId, "owner", tokenOrgId, tokenTeamId))) {
        (set as { status: number }).status = 404;
          ...
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "agent pools are only valid for selected_agent_pools lists",
            },
          ],
        };
      }
          ...
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "data must contain agent-pools resource identifiers",
            },
          ],
        };
      }
      ...
      const pools =
        poolIds.length === 0
          ? []
          : await db.query.agentPools.findMany({
              where: and(eq(agentPools.orgId, list.orgId), inArray(agentPools.id, poolIds)),
            });
      if (pools.length !== poolIds.length) {
        (set as { status: number }).status = 422;
        return {
          errors: [
            { status: "422", title: "Unprocessable Entity", detail: "agent pools must belong to the organization" },
          ],
        };
      }
      const relationResult = await db.transaction(async (tx): Promise<"missing" | "invalid" | "ok"> => {
        ...
        if (poolIds.length > 0) {
          await tx
            .insert(cidrRangeListAgentPools)
            .values(
              poolIds.map((agentPoolId) => ({
  ...
  .delete(
    ...
    async ({ params, user, body, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (poolIds.length > 0) {
        await db
          .delete(cidrRangeListAgentPools)
          .where(
            and(
              eq(cidrRangeListAgentPools.cidrRangeListId, list.id),
              inArray(cidrRangeListAgentPools.agentPoolId, poolIds),
            ),
          );
      }
      (set as { status: number }).status = 204;
