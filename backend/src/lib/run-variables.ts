...
export async function runVariablesForWrite(
  variables: readonly {
    readonly key: string;
  ...
  { key: string; value: string; category?: "terraform" | "env"; sensitive?: boolean; valueEncrypted?: string }[]
> {
  return Promise.all(
    variables.map(async ({ key, value, category, sensitive }) => {
      if (category !== undefined && category !== "terraform" && category !== "env")
        throw new Error("Invalid run variable category");
      return {
        key,
...
export function normalizeRunVariables(
  variables: unknown,
): { key: string; value: string; category: string; sensitive: boolean }[] {
  ...
  for (const item of variables) {
    ...
    if (
      ...
      (typeof record["valueEncrypted"] !== "string" || !isEncryptedSecret(record["valueEncrypted"]))
    ) {
    ...
    normalized.push({
      key: record["key"],
      value:
        typeof record["valueEncrypted"] === "string"
