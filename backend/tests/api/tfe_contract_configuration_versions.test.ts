...
describe("TFE configuration versions contract", () => {
  const seed = seedTfeOrg("cv");
  ...
  it("creates a configuration version with the documented shape", async () => {
    ...
    cvId = resource.id;
        // TFE emits ids prefixed with "cv-"; Terrence uses bare UUIDs (opaque to clients).
    ...
    expect(resource.attributes["upload-url"]).toBeTypeOf("string");
  ...
  it("uploads and shows a configuration version", async () => {
    ...
    const uploadUrl = pending.data.attributes["upload-url"] as string;
