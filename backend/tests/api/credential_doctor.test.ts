...
describe("credential doctor API", () => {
  ...
  beforeAll(async () => {
    ...
    setExternalUrlTransportForTests(async (target, init): Promise<Response> => {
      ...
      if (new URL(target.url).hostname === "login.microsoftonline.com")
        return Response.json({ access_token: "provider-access-token" });
      return Response.json({
        subscriptionId: "subscription",
        tenantId: "tenant",
        displayName: "Doctor subscription",
        state: "Enabled",
      });
    });
  });
  ...
  test("runs from the local worker and exposes only safe claims and result metadata", async () => {
    ...
    expect(body.data.attributes["claims"]).toMatchObject({
      aud: "azure.workload.identity",
      sub: `organization:${orgName}:credential-doctor`,
    });
    expect(body.data.attributes["checks"]).toEqual(
    ...
    expect(
      await db.query.workloadIdentityTokens.findFirst({ where: eq(workloadIdentityTokens.jti, claims.jti!) }),
    ).toBeUndefined();
  });

  test("requires organization provider-management access", async () => {
    const response = await app.handle(
      new Request(`http://terrence.test/api/v2/oidc-configurations/${configId}/credential-doctor`, { method: "POST" }),
    );
