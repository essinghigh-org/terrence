...
async function variableAuthorizationWhere(
  ...
  if (currentTokenScopes() === null && tokenOrgId === null && tokenTeamId === null && user?.isSiteAdmin === true)
    return sql`true`;
  ...
  const accessConditions = (
    await Promise.all(
      [...new Set(organizationIds)].map(async (organizationId): Promise<SQL | null> => {
 ...
 */
async function durableWebhookEnqueue(
  input: Readonly<{
...
async function auditLogResources(logsList: readonly AuditLogItem[]): Promise<Record<string, unknown>[]> {
  const actorIds = [
    ...new Set(logsList.map((log): string | null => log.userId).filter((id): id is string => id !== null)),
  ];
  const actors =
    actorIds.length === 0
      ? []
  ...
  const actorsById = new Map(
    actors.map((actor): [string, { username: string; email: string | null }] => [actor.id, actor]),
  );
...
async function auditLogOrgIdsForUser(user: Readonly<typeof users.$inferSelect>): Promise<string[]> {
  ...
  if (scopes !== null) {
    ...
    const candidateOrgIds =
      user.isSiteAdmin === true || user.isSiteAuditor === true
...
async function auditLogsForPrincipal(
  user: Readonly<typeof users.$inferSelect> | null | undefined,
  token:
    | Readonly<{ id: string; orgId: string | null; teamId: string | null; tokenType?: string; scopes?: string | null }>
    | null
    | undefined,
  page: AuditLogPage,
...
function parseVariableAttributes(body: unknown): {
...
function parseVariableWorkspaceId(data: Record<string, unknown>): {
  workspaceId: string;
  ...
  const relationships =
    data["relationships"] !== null && typeof data["relationships"] === "object"
      ...
      : {};
  const workspaceRelationship =
    relationships["workspace"] !== null && typeof relationships["workspace"] === "object"
      ? (relationships["workspace"] as Record<string, unknown>)
      : {};
  const workspaceData =
    workspaceRelationship["data"] !== null && typeof workspaceRelationship["data"] === "object"
...
function parseRunTriggerSourceable(body: unknown): { srcId: string; srcType: unknown } {
  ...
  const srcData =
    typeof sourceable?.["data"] === "object" && sourceable["data"] !== null
      ? (sourceable["data"] as Record<string, unknown>)
      : undefined;
  return { srcId: typeof srcData?.["id"] === "string" ? srcData["id"] : "", srcType: srcData?.["type"] };
}

async function resolveRunTriggerSource(
  srcId: string,
  orgId: string,
  workspaceId: string,
): Promise<Readonly<{ name: string }> | Readonly<{ error: string }>> {
  const srcWs = await db.query.workspaces.findFirst({ where: eq(workspaces.id, srcId) });
  if (srcWs === undefined || srcWs.orgId !== orgId)
    return { error: "Sourceable workspace must belong to the same organization" };
...
function parseApprovalWebhookPayload(
  rawBody: string,
  set: SetObj,
  ...
  const runId =
    typeof parsed["run"] === "string" ? parsed["run"] : typeof parsed["run_id"] === "string" ? parsed["run_id"] : "";
...
async function confirmRunApplyForWebhook(runId: string, set: SetObj): Promise<unknown> {
  ...
  if (!outcome.ok) {
    await auditLog(
      ...
      null,
      null,
      { source: "approval-webhook", reason: outcome.reason ?? "apply could not be started" },
...
async function insertRunTriggerSources(
  orgId: string,
  workspaceId: string,
  sourceIds: string[],
): Promise<string | null> {
  ...
  const sourceWorkspaces =
    uniqueSourceIds.length === 0
      ...
      : await db.query.workspaces.findMany({
          where: inArray(workspaces.id, uniqueSourceIds),
          columns: { id: true, orgId: true },
        });
  const validSources = new Set(
    sourceWorkspaces
      .filter((source): boolean => source.orgId === orgId && source.id !== workspaceId)
  ...
  if (uniqueSourceIds.length > 0) {
    await db
      .insert(runTriggers)
      .values(
        uniqueSourceIds.map((sourceWorkspaceId: string): typeof runTriggers.$inferInsert => ({
          id: newResourceId("rt"),
          workspaceId,
          sourceWorkspaceId,
        })),
...
export const miscRoutes = new Elysia({ name: "misc" })
  ...
  .post(
    "/api/webhooks/github",
    async ({ request, body, set }: Readonly<{ request: Request; body: unknown; set: SetObj }>): Promise<unknown> => {
    ...
    async ({ request, body, set }: Readonly<{ request: Request; body: unknown; set: SetObj }>): Promise<unknown> => {
      ...
      await durableWebhookEnqueue({
        ...
        deliveryId: vcsWebhookDeliveryId(
          ...
          eventName,
          parsed.payload,
          firstWebhookHeader(request, ["x-request-uuid"]),
        ),
      });
      return webhookAcknowledged;
    },
  ...
  .get("/api/v2/vars", async ({ user, orgId, teamId, request, set }: ParamCtx): Promise<unknown> => {
        ...
        errors: [
          {
            ...
            detail: "Organization and workspace filters must be provided together",
          },
        ],
      };
    }
    ...
    const [rows, countRows] = await Promise.all([
      db
        .select({ variable: workspaceVariables })
        .from(workspaceVariables)
  ...
  .post("/api/v2/vars", async ({ body, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
    ...
    if (
      ...
      return {
        errors: [
          {
            status: String(workspace === undefined ? 404 : 422),
            title: workspace === undefined ? "Not Found" : "Unprocessable Entity",
          },
        ],
      };
    }
  ...
  .get(
    "/api/v2/organizations/:org_name/audit-logs",
    async ({ params, user, orgId: tokenOrgId, request, set }: ParamCtx): Promise<unknown> => {
      if ((user === null || user === undefined) && tokenOrgId === null) {
        (set as { status: number }).status = 401;
        return { errors: [{ status: "401", title: "Unauthorized" }] };
      }
      ...
      if (
        org === undefined ||
        !(await checkOrgPermission(user?.id, org.id, "owner", tokenOrgId, null, "audit-logs:read"))
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
  ...
  .get(
    "/api/v2/organizations/:org_name/audit-configuration",
  ...
  // --- Cost Estimation ---
  .get(
    "/api/v2/runs/:run_id/cost-estimate",
    async ({ params, user, orgId, teamId, set }: ParamCtx): Promise<unknown> => {
      ...
      return {
        data: costEstimateResource(
          authorized.run,
          await readCostEstimateArtifact(runId),
          await costEstimationEnabledForOrganization(authorized.workspace.orgId),
        ),
      };
    },
  )
  ...
  // --- Run Triggers ---
  .get(
    "/api/v2/workspaces/:workspace_id/run-triggers",
    async ({ params, request, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const workspaceId = params["workspace_id"] ?? "";
      const ws = await db.query.workspaces.findFirst({ where: eq(workspaces.id, workspaceId) });
      if (ws === undefined || (await findAuthorizedWorkspace(ws.id, user?.id, tokenOrgId, tokenTeamId)) === undefined) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
      const rawFilterType =
        request !== undefined ? new URL(request.url).searchParams.get("filter[run-trigger][type]") : null;
      if (
        rawFilterType !== null &&
        rawFilterType !== "" &&
        rawFilterType !== "inbound" &&
        rawFilterType !== "outbound"
      ) {
        (set as { status: number }).status = 422;
        return {
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "filter[run-trigger][type] must be 'inbound' or 'outbound'",
            },
          ],
        };
      }
      ...
      const relatedWorkspaceIds = [
        ...new Set(
          triggers.flatMap((t: Readonly<typeof runTriggers.$inferSelect>): string[] => [
            t.workspaceId,
            t.sourceWorkspaceId,
          ]),
        ),
      ...
      const wsNames = new Map(
        relatedWorkspaces.map((w: Readonly<{ readonly id: string; readonly name: string }>): [string, string] => [
          w.id,
          w.name,
        ]),
      );
      return {
        data: triggers.map(
          (t: Readonly<typeof runTriggers.$inferSelect>): Record<string, unknown> => ({
            id: t.id,
            type: "run-triggers",
            attributes: {
  ...
  .post(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (
        ws === undefined ||
        (await findAuthorizedWorkspace(ws.id, user?.id, tokenOrgId, tokenTeamId, "admin")) === undefined
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
          ...
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "Sourceable workspace must be a workspace resource identifier",
            },
          ],
        };
      }
      ...
      return {
        data: {
          ...
          attributes: {
            ...
            "sourceable-name": resolved.name,
            "workspace-name": ws.name,
          },
          relationships: {
            sourceable: { data: { id: srcId, type: "workspaces" } },
            "sourceable-workspace": { data: { id: srcId, type: "workspaces" } },
  ...
  .get(
    "/api/v2/run-triggers/:run_trigger_id",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const triggerId = params["run_trigger_id"] ?? "";
      const trigger =
        triggerId !== "" ? await db.query.runTriggers.findFirst({ where: eq(runTriggers.id, triggerId) }) : undefined;
      if (
        trigger === undefined ||
        (await findAuthorizedWorkspace(trigger.workspaceId, user?.id, tokenOrgId, tokenTeamId)) === undefined
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
      ...
      return {
        data: {
          ...
          type: "run-triggers",
          attributes: {
            "created-at": new Date(trigger.createdAt).toISOString(),
            "sourceable-name": sw?.name ?? "",
            "workspace-name": tw?.name ?? "",
          },
          relationships: {
            sourceable: { data: { id: trigger.sourceWorkspaceId, type: "workspaces" } },
            "sourceable-workspace": { data: { id: trigger.sourceWorkspaceId, type: "workspaces" } },
            workspace: { data: { id: trigger.workspaceId, type: "workspaces" } },
          },
        },
      };
    },
  ...
  .delete(
    ...
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (
        trigger === undefined ||
        (await findAuthorizedWorkspace(trigger.workspaceId, user?.id, tokenOrgId, tokenTeamId, "admin")) === undefined
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
  ...
  .post(
    "/api/v2/workspaces/:workspace_id/relationships/run-triggers",
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const workspaceId = params["workspace_id"] ?? "";
      const ws = await db.query.workspaces.findFirst({ where: eq(workspaces.id, workspaceId) });
      if (
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (Array.isArray(items)) {
        ...
        if (srcIds.length > 0) {
          await db
            .delete(runTriggers)
            .where(and(eq(runTriggers.workspaceId, workspaceId), inArray(runTriggers.sourceWorkspaceId, srcIds)));
        }
      }
      (set as { status: number }).status = 204;
      return new Response(null, { status: 204 });
