...
describe("TFE state versions contract", () => {
  const seed = seedTfeOrg("sv");
  ...
  it("creates a state version with the documented shape", async () => {
    ...
    // TFE emits ids prefixed with "sv-"; Terrence uses bare UUIDs (opaque to clients).
    expect(stateVersionId).toBeTypeOf("string");
    expect(stateVersionId).not.toBe("");
    expect(resource.attributes.serial).toBe(1);
    // TFE returns the MD5 digest of the state payload.
