      ...
      <Card>
        ...
        <CardContent>
          <FieldGroup>
            <Field data-disabled={loading}>
              ...
              <Select id="workspace-ssh-key" value={selectedKeyId} onValueChange={setSelectedKeyId} disabled={loading}>
                <SelectItem value="">No SSH key</SelectItem>
                {keys.map(
                  (key: SshKey): React.JSX.Element => (
              ...
              <FieldDescription>Private key material remains write-only and is never displayed here.</FieldDescription>
            </Field>
        ...
        <CardFooter className="justify-between">
          <span role="status" className="text-sm text-muted-foreground">
            {saved
              ? "SSH key assignment saved."
              : keys.length === 0 && !loading
                ? "No organization SSH keys are available."
                : ""}
          </span>
...
export function WorkspaceRunTriggers({
  ...
  const attachedIds = useMemo(
    (): Set<string> =>
      new Set(triggers.map((trigger: RunTrigger): string => trigger.relationships["sourceable-workspace"].data.id)),
    [triggers],
  );
  const availableWorkspaces = workspaces.filter(
    (workspace: WorkspaceSummary): boolean => workspace.id !== workspaceId && !attachedIds.has(workspace.id),
  );
  const namesById = useMemo(
    (): Map<string, string> =>
      new Map(
        workspaces.map((workspace: WorkspaceSummary): [string, string] => [workspace.id, workspace.attributes.name]),
      ),
    [workspaces],
  );
  ...
  const detach = async (sourceWorkspaceId: string): Promise<void> => {
    ...
    try {
      ...
      setTriggers((current: RunTrigger[]): RunTrigger[] =>
        current.filter(
          (trigger: RunTrigger): boolean => trigger.relationships["sourceable-workspace"].data.id !== sourceWorkspaceId,
        ),
      );
    } catch (caught: unknown) {
  ...
  return (
    <>
              ...
              <Field orientation="responsive" data-disabled={loading || saving}>
                ...
                >
                  ...
                  {availableWorkspaces.map(
                    (workspace: WorkspaceSummary): React.JSX.Element => (
          ...
          <div className="rounded-md border">
            <Table>
              ...
              <TableBody>
                ...
                {!loading &&
                  triggers.map((trigger: RunTrigger): React.JSX.Element => {
                      ...
                      <TableRow key={trigger.id}>
                        ...
                        <TableCell className="text-right">
                          <Button
                            ...
                            variant="destructive"
                            onClick={(): void => {
                              setPendingDetach({ sourceId, sourceName: namesById.get(sourceId) ?? sourceId });
                            }}
                          >
                            Remove
                          </Button>
      ...
      <ConfirmDialog
        ...
        description={
          pendingDetach === null ? undefined : (
            <>
              Successful applies in <strong>{pendingDetach.sourceName}</strong> will stop queueing runs in this
              workspace.
            </>
          )
        }
        ...
        onConfirm={(): void => {
          if (pendingDetach !== null) void detach(pendingDetach.sourceId);
        }}
      />
    </>
  );
    ...
    <div className="space-y-5">
      {isFullyHealthy(attrs, checksFailed) && (
        <div className="flex items-center gap-4 rounded-lg border border-success/20 bg-success/5 px-4 py-2">
          ...
          <div>
            <h3 className="font-heading text-lg font-semibold">Everything healthy</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              The latest assessment found no drift or failing checks.
            </p>
          </div>
        </div>
      )}
      <dl className="grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <dd className="mt-1 font-medium capitalize">{attrs.status.replace(/_/g, " ")}</dd>
        </div>
        <div>
          <dt className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Drift</dt>
          <dd className="mt-1 font-medium">{driftLabel(attrs)}</dd>
        </div>
        <div>
          <dt className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Validation</dt>
          <dd className="mt-1 font-medium">
            {attrs["checks-passed"] ?? 0} passed{checksFailed > 0 ? ` · ${checksFailed} failed` : ""}
          </dd>
        </div>
        <div>
          <dt className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Completed</dt>
          <dd className="mt-1 font-medium">{completedLabel(attrs["completed-at"])}</dd>
        </div>
        {attrs["error-msg"] !== null && attrs["error-msg"] !== undefined && (
          <div className="sm:col-span-2 lg:col-span-4">
            <dt className="text-xs font-semibold uppercase tracking-wide text-destructive">Error</dt>
            <dd className="mt-1 text-sm text-destructive">{attrs["error-msg"]}</dd>
          </div>
        )}
      </dl>
    </div>
...
export function WorkspaceHealth({
  ...
  useEffect((): (() => void) => {
    ...
    void fetchApi<{ data?: HealthAssessment[] }>(`/workspaces/${encodeURIComponent(workspace.id)}/assessment-results`, {
        ...
        <CardContent>
          ...
          {!assessmentsLoading && assessmentError && (
            <p role="alert" className="text-sm text-destructive">
              Health results could not be loaded. Reload the page to try again.
            </p>
          )}
          {!assessmentsLoading && !assessmentError && assessments[0] === undefined && (
            <div className="flex flex-col items-center gap-4 py-6 text-center sm:flex-row sm:text-left">
              ...
              <div>
                <h3 className="font-heading font-semibold">No health assessment has run yet.</h3>
                <p className="mt-1 max-w-md text-sm text-muted-foreground">
                  Health assessments check for infrastructure drift and validation failures. Configure the schedule
                  below to get started.
                </p>
              </div>
            </div>
          )}
          {!assessmentsLoading && assessments[0] !== undefined && <LatestAssessmentCard assessment={assessments[0]} />}
        </CardContent>
      </Card>
      <form onSubmit={save}>
        <Card>
