...
for (const engine of ["terraform", "tofu"]) {
  test(`${engine} serialized engine plan redacts sensitive outputs and retains public output values`, async () => {
    ...
    for (const rawOnly of ["variables", "configuration", "prior_state", "planned_values"])
      expect(projected).not.toHaveProperty(rawOnly);
  });
}
...
test("public projection excludes secrets in every raw representation and fails closed without masks", () => {
  ...
  const raw = {
    ...
    resource_changes: [
      {
        address: "module.child.test.example",
        change: {
          actions: ["update"],
          before: { password: secret, visible: "old" },
          after: { password: secret, visible: "new", nested: [secret, "public"] },
          before_sensitive: { password: true },
          after_sensitive: { password: true, nested: [true, false] },
    ...
    output_changes: {
      password: { actions: ["update"], before: secret, after: secret, before_sensitive: true, after_sensitive: true },
    },
  };
  ...
  expect(publicPlan["resource_changes"]).toMatchObject([
    {
      change: { after: { password: null, visible: "new", nested: [null, "public"] }, after_unknown: { pending: true } },
    },
    { change: { after: null } },
  ]);
...
test("run artifact cleanup removes agent side artifacts even without a raw plan", async () => {
  ...
  const paths = ["sanitized.json", "redacted.json", "provider-schemas.json", "description.txt"].map((suffix) =>
...
test("public plan preserves move, replacement and action summaries without action secrets", () => {
  const projected = sanitizePlanJson({
    ...
    action_invocations: [
      {
        address: "action.test.restart",
        config: { password: "hidden-canary" },
        lifecycle_action_trigger: {
          triggering_resource_address: "test.new",
  ...
  expect(projected["resource_changes"]).toMatchObject([
    {
      previous_address: "test.old",
      change: {
        replace_paths: [
  ...
  expect(projected["action_invocations"]).toEqual([
    {
      address: "action.test.restart",
      lifecycle_action_trigger: { triggering_resource_address: "test.new", action_trigger_event: "after_create" },
...
test("public plan version preserves absent/null values and marks unsupported actions without copying extensions", () => {
  const projected = sanitizePlanJson({
    resource_changes: [
      {
        address: 'test.item["a.b"]',
        change: {
          actions: ["create", "secret-extension"],
          after: null,
          before_sensitive: false,
          after_sensitive: false,
        },
  ...
  expect(projected["resource_changes"]).toEqual([
    {
      address: 'test.item["a.b"]',
      change: {
        actions: ["create", "unsupported"],
        after: null,
        before_sensitive: false,
        after_sensitive: false,
      },
