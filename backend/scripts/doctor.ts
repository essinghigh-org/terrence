...
function checkAdminBootstrap(db: DatabaseTarget): void {
  ...
  if (db.kind !== "sqlite-file") {
    record(
      "admin-bootstrap",
      "warn",
      "ADMIN_PASSWORD is unset and the admin account cannot be verified on a non-sqlite database from here",
    );
    return;
  }
  try {
    ...
    if (total > 0) {
      record(
        "admin-bootstrap",
        "ok",
        `${total} local user(s) present; bootstrap complete (ADMIN_PASSWORD unset, as expected after first boot)`,
      );
    } else {
      record(
        "admin-bootstrap",
        "warn",
        "no local users yet; set ADMIN_PASSWORD or enable local signup to bootstrap an admin",
      );
    }
  } catch {
    record(
      "admin-bootstrap",
      "warn",
      "ADMIN_PASSWORD is unset and the users table could not be read (fresh install or unreadable database)",
    );
  }
}
