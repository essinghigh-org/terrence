...
export const runsRoutes = new Elysia({ name: "admin-runs" })
  ...
  .get("/api/v2/admin/runs", async ({ user, set }: ParamCtx): Promise<unknown> => {
    ...
    const [capacity, inspections] = await Promise.all([
      ...
      Promise.all(
        activeRuns.map(
          async (run): Promise<readonly [string, Record<string, unknown>]> => [
            run.id,
    ...
    return {
      ...
      meta: {
        "queue-inspector": {
          ...
          "position-note":
            "Queue positions are qualified snapshot estimates. Worker claims, cancellations, locks and heartbeats can change them.",
          capacity,
          controls: {
            "cancel-supported": true,
            "reprioritize-supported": false,
            "reprioritize-reason":
              "Queued runs do not have a persisted priority field; cancellation preserves scheduler gates and serialization.",
          },
          "truncated-at": 200,
        },
      },
  ...
  .post("/api/v2/admin/runs/:run_id/actions/cancel", async ({ params, user, set }: ParamCtx): Promise<unknown> => {
    ...
    const updated = await db
      .update(runs)
      .set({ status: "canceled" })
      .where(and(eq(runs.id, runId), notInArray(runs.status, FINAL_RUN_STATUSES)))
      .returning();
  ...
  .post(
    "/api/v2/admin/runs/:run_id/actions/force-cancel",
    async ({ params, user, set }: ParamCtx): Promise<unknown> => {
      ...
      if (updated.length === 0 || updated[0] === undefined) {
        (set as { status: number }).status = 409;
        return { errors: [{ status: "409", title: "Conflict", detail: "Run is not force-cancelable" }] };
      }
