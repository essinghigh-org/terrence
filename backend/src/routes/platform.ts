...
function stringArrayAttribute(attrs: Record<string, unknown>, ...keys: string[]): string[] {
  for (const key of keys) {
    if (Array.isArray(attrs[key]))
      return attrs[key].filter((value): value is string => typeof value === "string" && value.trim() !== "");
  }
  return [];
}

function errorDocument(
  set: ParamContext["set"],
  status: number,
  detail: string,
...
async function workspaceByPermission(
  ...
  return findAuthorizedWorkspace(
    workspaceId,
    context.user?.id,
    context.orgId ?? null,
    context.teamId ?? null,
...
async function authorizedStatePair(
  ...
  | Readonly<{
      ...
      before: typeof stateVersions.$inferSelect;
      after: typeof stateVersions.$inferSelect;
    }>
...
> {
  ...
  if (beforeId === null || afterId === null || beforeId === afterId) {
    return {
      error: errorDocument(context.set, 422, "before and after state-version IDs must identify two different versions"),
    };
  }
  ...
  if (before === undefined || after === undefined) {
    return {
      error: errorDocument(
        context.set,
        410,
        "One or both state versions are unavailable under the retention policy",
...
async function createStateComparison(
  context: ParamContext,
  workspaceId: string,
  attrs: Record<string, unknown>,
): Promise<unknown> {
...
async function authorizedPlanBefore(
  ...
  const before = await findAuthorizedRun(
    beforeId,
    context.user?.id,
    context.orgId ?? null,
    context.teamId ?? null,
...
async function createPlanComparison(
  context: ParamContext,
  runId: string,
  attrs: Record<string, unknown>,
): Promise<unknown> {
  const current = await findAuthorizedRun(
    runId,
    context.user?.id,
    context.orgId ?? null,
    context.teamId ?? null,
  ...
  if (beforeId === null || beforeId === runId)
    return errorDocument(context.set, 422, "before-run-id must identify a different run");
  ...
  if (beforePlan === undefined || afterPlan === undefined)
    return errorDocument(
      context.set,
      409,
      "Both runs need a retained public plan artifact before they can be compared",
...
async function inventoryForWorkspace(
  workspace: WorkspaceRow,
  query: string | null,
): Promise<readonly InventoryObservation[]> {
  const versions = await db.query.stateVersions.findMany({
  ...
  const observations = versions.flatMap((version): readonly InventoryObservation[] =>
    stateInventoryObservations(comparisonStateInput(version)),
  );
  if (query === null || query === "") return observations;
  const lower = query.toLocaleLowerCase();
  return observations.filter((observation): boolean =>
    [observation.identity, observation.address, observation.provider, observation.type].some((value): boolean =>
...
async function latestConfigurationVersion(
  workspaceId: string,
...
async function driftCheckFingerprint(
  ...
  const checks = await db.query.assessmentCheckResults.findMany({
    where: eq(assessmentCheckResults.assessmentResultId, assessment.id),
    columns: { address: true, kind: true, status: true },
  });
  return driftFingerprint({
    ...
    checks: checks.map(
      (check): Record<string, unknown> => ({ address: check.address, kind: check.kind, status: check.status }),
    ),
  });
}
...
function driftIncidentPayload(
  ...
  return existing === undefined
    ...
    : {
        ...
        history: [
          ...(Array.isArray(existing.payload["history"]) ? (existing.payload["history"] as unknown[]) : []),
          historyEntry,
        ].slice(-100),
      };
}
...
async function persistDriftIncident(
  ...
  return existing === undefined
    ? createPlatformArtifact({
        ...
        actorId,
        dedupeKey: `drift:${workspace.id}:${fingerprint}`,
        payload,
        status: "open",
      })
    : updatePlatformArtifact(existing.id, "drift-incident", workspace.orgId, {
        status:
          existing.status === "snoozed" &&
          typeof existing.payload["snooze-until"] === "number" &&
          existing.payload["snooze-until"] > now
            ? "snoozed"
            : "open",
        payload,
      });
...
function incidentResolutionError(attrs: Record<string, unknown>, status: string): { detail: string } | null {
  ...
  if (
    stringAttribute(attrs, "evidence-assessment-id", "evidenceAssessmentId") === null &&
    attrs["acknowledged-exception"] !== true
  ) {
...
function parseIncidentSnooze(
  attrs: Record<string, unknown>,
  status: string,
): { snoozeUntil: number | null } | { error: { detail: string } } {
  ...
  if (
    ...
    (snoozeUntil === null ||
      ...
      snoozeUntil > Date.now() + 30 * 86_400_000)
  ) {
...
async function queueDependencyRuns(
  ...
  for (const targetId of targets.slice(0, 100)) {
    ...
    await db.insert(runs).values({
      ...
      planOnly: true,
      autoApply: false,
      message: `Dependency preview ${rowId} from ${workspaceId}`,
...
export const platformRoutes = new Elysia({ name: "platform" })
  .use(authPlugin)
  .post(
    "/api/v2/workspaces/:workspace_id/state-comparisons",
    async (context: ParamContext): Promise<unknown> =>
  ...
  .get("/api/v2/state-comparisons/:comparison_id", async (context: ParamContext): Promise<unknown> => {
    ...
    const row = await db.query.durableJobs.findFirst({
      where: and(eq(durableJobs.id, id), eq(durableJobs.kind, "state-comparison")),
    });
  ...
  .post(
    "/api/v2/runs/:run_id/plan-comparisons",
    async (context: ParamContext): Promise<unknown> =>
  ...
  .get("/api/v2/plan-comparisons/:comparison_id", async (context: ParamContext): Promise<unknown> => {
    const row = await getPlatformArtifact(
      context.params["comparison_id"] ?? "",
      "plan-comparison",
      context.orgId ?? "",
    );
  ...
  .get("/api/v2/workspaces/:workspace_id/inventory-history", async (context: ParamContext): Promise<unknown> => {
    ...
    const query =
      new URL(context.request.url).searchParams.get("q") ?? new URL(context.request.url).searchParams.get("address");
    const observations = await inventoryForWorkspace(workspace, query);
    return {
      data: observations.map(inventoryResource),
  ...
  .post("/api/v2/workspaces/:workspace_id/import-workbench", async (context: ParamContext): Promise<unknown> => {
    ...
    if (engine !== "terraform" && engine !== "tofu")
      return errorDocument(context.set, 422, "engine must be terraform or tofu");
    ...
    const conflicts = mappings.mappings
      .filter((mapping): boolean => existingIds.has(mapping.providerId))
      .map((mapping): string => mapping.address);
  ...
  .get("/api/v2/workspaces/:workspace_id/import-workbenches", async (context: ParamContext): Promise<unknown> => {
    const workspace = await workspaceByPermission(context.params["workspace_id"] ?? "", context, "plan");
    if (workspace === undefined) return notFound(context.set);
    const rows = await listPlatformArtifacts({
      kind: "import-workbench",
      organizationId: workspace.orgId,
      workspaceId: workspace.id,
    });
  ...
  .get("/api/v2/import-workbenches/:workbench_id", async (context: ParamContext): Promise<unknown> => {
    const candidate = await db.query.durableJobs.findFirst({
      where: and(eq(durableJobs.id, context.params["workbench_id"] ?? ""), eq(durableJobs.kind, "import-workbench")),
    });
  ...
  .post("/api/v2/import-workbenches/:workbench_id/export", async (context: ParamContext): Promise<unknown> => {
    ...
    const row = await getPlatformArtifact(candidate.id, "import-workbench", workspace.orgId);
    if (row === undefined) return notFound(context.set);
    const generated = Array.isArray(row.payload["generated-configuration"])
      ? row.payload["generated-configuration"]
      : [];
  ...
  .post(
    "/api/v2/assessment-results/:assessment_result_id/drift-incident",
    async (context: ParamContext): Promise<unknown> => {
      ...
      const incidents = await listPlatformArtifacts({
        kind: "drift-incident",
        organizationId: workspace.orgId,
        workspaceId: workspace.id,
      });
      const existing = incidents.find(
        (row): boolean => row.payload["fingerprint"] === fingerprint && row.status !== "resolved",
      );
      const now = Date.now();
      const payload = driftIncidentPayload(
        existing,
        assessment,
        fingerprint,
        now,
        stringAttribute(attributesFrom(context.body), "assignee", "assigned-to"),
      );
      const artifact = await persistDriftIncident(
        existing,
        workspace,
        fingerprint,
        payload,
  ...
  .patch("/api/v2/drift-incidents/:incident_id", async (context: ParamContext): Promise<unknown> => {
    const attrs = attributesFrom(context.body);
    const row = await db.query.durableJobs.findFirst({
      where: and(eq(durableJobs.id, context.params["incident_id"] ?? ""), eq(durableJobs.kind, "drift-incident")),
    });
    if (row === undefined || typeof row.payload["organizationId"] !== "string") return notFound(context.set);
    ...
    if (!["open", "snoozed", "resolved"].includes(status))
      return errorDocument(context.set, 422, "status must be open, snoozed, or resolved");
    ...
    return {
      data: {
        ...
        attributes: {
          ...
          "incident-id": updated?.id ?? row.id,
        },
      },
    };
  })
  .post(
    "/api/v2/workspaces/:workspace_id/dependency-impact-previews",
    async (context: ParamContext): Promise<unknown> => {
      ...
      if (sourceRunId !== null) {
        const source = await findAuthorizedRun(
          sourceRunId,
          context.user?.id,
          context.orgId ?? null,
          context.teamId ?? null,
      ...
      const graph = dependencyImpact({
        rootWorkspaceId: workspace.id,
        edges: authorizedEdges,
        maxFanout: typeof attrs["max-fanout"] === "number" ? attrs["max-fanout"] : 100,
      });
      const artifact = await createPlatformArtifact({
  ...
  .post("/api/v2/dependency-impact-previews/:preview_id/queue", async (context: ParamContext): Promise<unknown> => {
    const row = await db.query.durableJobs.findFirst({
      where: and(eq(durableJobs.id, context.params["preview_id"] ?? ""), eq(durableJobs.kind, "dependency-impact")),
    });
    if (row === undefined || typeof row.payload["organizationId"] !== "string") return notFound(context.set);
    const organizationId = row.payload["organizationId"];
    const workspaceId = typeof row.payload["workspaceId"] === "string" ? row.payload["workspaceId"] : "";
    ...
    if (cycles.length > 0)
      return errorDocument(
        context.set,
        409,
        "The dependency graph contains a cycle; downstream planning was not queued",
        "Dependency Cycle",
      );
    ...
    const targets = [
      ...new Set(
        requested.length > 0
          ? requested
          : edges.map((edge): string => (typeof edge["to"] === "string" ? edge["to"] : "")).filter(Boolean),
      ),
    ];
    const queuedPlans = await queueDependencyRuns(
      targets,
      root.orgId,
      row.id,
      workspaceId,
      payload["source-run-id"],
      context,
    );
    const updated = await updatePlatformArtifact(row.id, "dependency-impact", organizationId, {
      status: "plans-queued",
      payload: { ...payload, "queued-plans": queuedPlans },
    });
    if (updated === undefined) return notFound(context.set);
    return { data: artifactResource(updated, "dependency-impact-previews") };
  })
  .post(
    "/api/v2/organizations/:org_name/fleet-operations/previews",
    async (context: ParamContext): Promise<unknown> => {
      const organization = await organizationForName(context.params["org_name"] ?? "");
      if (organization === undefined || !(await organizationCanManage(organization, context, "manage-workspaces")))
      ...
      if (action === null || !allowedActions.has(action))
        return errorDocument(context.set, 422, "action must be a bounded, reviewable fleet operation");
      if (["apply", "destroy", "bulk-apply", "bulk-destroy"].includes(action))
        return errorDocument(context.set, 422, "apply and destroy are not fleet operations");
      const targetIds = [...new Set(stringArrayAttribute(attrs, "target-ids", "workspace-ids"))];
      if (targetIds.length === 0 || targetIds.length > 500)
        return errorDocument(context.set, 422, "target-ids must contain between 1 and 500 workspaces");
      const workspacesInOrg = await db.query.workspaces.findMany({
        where: and(eq(workspaces.orgId, organization.id), inArray(workspaces.id, targetIds)),
        columns: { id: true, name: true },
      });
      const visible = new Map(
        workspacesInOrg.map((workspace): [string, typeof workspace] => [workspace.id, workspace]),
      );
      const permissionFailures = targetIds
        .filter((id): boolean => !visible.has(id))
        .map((id): Record<string, unknown> => ({ workspaceId: id, reason: "not-found-or-not-in-organization" }));
      const manifest = {
        action,
      ...
      const artifact = await createPlatformArtifact({
        ...
        payload: {
          ...
          action,
          manifest,
          "target-workspaces": targetIds.map(
            (id): Record<string, unknown> => ({ id, name: visible.get(id)?.name ?? null }),
          ),
  ...
  .post("/api/v2/organizations/:org_name/fleet-operations", async (context: ParamContext): Promise<unknown> => {
    ...
    if (previewId === null)
      return errorDocument(context.set, 422, "preview-id is required; commit an immutable preview manifest");
    const preview = await getPlatformArtifact(previewId, "fleet-operation", organization.id);
    if (preview === undefined || preview.status !== "preview")
      return errorDocument(
        context.set,
        409,
        "The fleet preview is unavailable or already committed",
        "Fleet Preview Unavailable",
      );
    const payload = artifactPayload(preview);
    const manifest = payload["manifest"];
    if (manifest === null || typeof manifest !== "object" || Array.isArray(manifest))
      return errorDocument(context.set, 409, "The preview has no immutable selection manifest");
    const manifestRecord = manifest as Record<string, unknown>;
    const expectedDigest =
      typeof manifestRecord["selection-digest"] === "string" ? manifestRecord["selection-digest"] : "";
    const suppliedDigest = stringAttribute(attrs, "selection-digest", "selectionDigest");
    if (suppliedDigest !== null && suppliedDigest !== expectedDigest)
      return errorDocument(
        context.set,
        409,
        "The selection manifest digest does not match the preview",
        "Selection Changed",
      );
    return errorDocument(
      context.set,
      501,
      "Fleet execution is not implemented. This manifest remains a preview; no target actions have been queued or completed.",
      "Fleet Execution Unavailable",
    );
  })
  .get(
    "/api/v2/organizations/:org_name/fleet-operations/:operation_id",
    async (context: ParamContext): Promise<unknown> => {
      const organization = await organizationForName(context.params["org_name"] ?? "");
      if (organization === undefined || !(await organizationCanManage(organization, context, "manage-workspaces")))
        return notFound(context.set);
  ...
  .post(
    "/api/v2/organizations/:org_name/fleet-operations/:operation_id/cancel",
    async (context: ParamContext): Promise<unknown> => {
      const organization = await organizationForName(context.params["org_name"] ?? "");
      if (organization === undefined || !(await organizationCanManage(organization, context, "manage-workspaces")))
        return notFound(context.set);
      ...
      const perTarget = targets.map((target): unknown =>
        target !== null &&
        typeof target === "object" &&
        !Array.isArray(target) &&
        (target as Record<string, unknown>)["status"] === "queued"
          ? { ...(target as Record<string, unknown>), status: "canceled", reason: "remaining-work-canceled" }
          : target,
      );
      const updated = await updatePlatformArtifact(row.id, "fleet-operation", organization.id, {
        status: "canceled",
        payload: { ...artifactPayload(row), "per-target": perTarget, "canceled-remaining": true },
      });
      if (updated === undefined) return notFound(context.set);
  ...
  .post("/api/v2/organizations/:org_name/promotions", async (context: ParamContext): Promise<unknown> => {
    ...
    if (configurationDigest === null || targetIds.length === 0 || targetIds.length > 25)
      return errorDocument(context.set, 422, "configuration-digest and 1 to 25 ordered target workspaces are required");
    const stages = targetIds.map(
      (workspaceId, index): Record<string, unknown> => ({
    ...
    const configurationVersionIds =
      attrs["configuration-version-ids"] !== null &&
      typeof attrs["configuration-version-ids"] === "object" &&
      !Array.isArray(attrs["configuration-version-ids"])
        ? (attrs["configuration-version-ids"] as Record<string, unknown>)
        : {};
    const release = {
      "configuration-digest": configurationDigest,
      "target-graph": stages,
      "configuration-version-ids": Object.fromEntries(
        Object.entries(configurationVersionIds).filter(
          ([workspaceId, id]): boolean => targetIds.includes(workspaceId) && typeof id === "string",
        ),
      ),
  ...
  .post(
    "/api/v2/organizations/:org_name/promotions/:promotion_id/advance",
    async (context: ParamContext): Promise<unknown> => {
      const organization = await organizationForName(context.params["org_name"] ?? "");
      if (organization === undefined || !(await organizationCanManage(organization, context, "manage-workspaces")))
        return notFound(context.set);
      ...
      if (payload["stopped"] === true)
        return errorDocument(
          context.set,
          409,
          "Promotion was explicitly stopped; create a deliberate retry from the recorded stage",
          "Promotion Stopped",
        );
      return errorDocument(
        context.set,
        501,
        "Promotion execution is unavailable until configuration digests and stage completion can be verified. Use the normal run endpoints for reviewed configurations.",
        "Promotion Execution Unavailable",
      );
    },
  )
  .post(
    "/api/v2/organizations/:org_name/promotions/:promotion_id/stop",
    async (context: ParamContext): Promise<unknown> => {
      const organization = await organizationForName(context.params["org_name"] ?? "");
      if (organization === undefined || !(await organizationCanManage(organization, context, "manage-workspaces")))
        return notFound(context.set);
      const row = await getPlatformArtifact(context.params["promotion_id"] ?? "", "promotion", organization.id);
      if (row === undefined) return notFound(context.set);
      const updated = await updatePlatformArtifact(row.id, "promotion", organization.id, {
        status: "stopped",
        payload: { ...artifactPayload(row), stopped: true, "stopped-at": new Date().toISOString() },
      });
      if (updated === undefined) return notFound(context.set);
  ...
  .post("/api/v2/workspaces/:workspace_id/upgrade-rehearsals", async (context: ParamContext): Promise<unknown> => {
    ...
    const state =
      baseline === null
        ? await db.query.stateVersions.findFirst({
            where: and(eq(stateVersions.workspaceId, workspace.id), eq(stateVersions.status, "finalized")),
            orderBy: [desc(stateVersions.serial)],
          })
        : await db.query.stateVersions.findFirst({
            where: and(eq(stateVersions.id, baseline), eq(stateVersions.workspaceId, workspace.id)),
          });
    if (state === undefined)
      return errorDocument(
        context.set,
        409,
        "A retained baseline state version is required for an upgrade rehearsal",
        "Baseline Unavailable",
      );
    const candidateLock = stringAttribute(attrs, "candidate-lock-digest", "lock-file-digest");
    const result = upgradeRehearsalResult({
      engine,
      version,
      baselineFresh: Date.now() - state.createdAt < 24 * 86_400_000,
      candidateLockDigest: candidateLock,
    });
    const artifact = await createPlatformArtifact({
      ...
      actorId: context.user?.id ?? null,
      payload: {
        "baseline-state-version-id": state.id,
        "baseline-digest": state.uploadSha256 ?? null,
        "candidate-engine": engine,
        "candidate-version": version,
        "candidate-lock-digest": candidateLock,
        result,
        "apply-authority": false,
        "default-settings-mutated": false,
      },
      status: result["status"] === "invalid" ? "invalid" : "review",
    });
    context.set.status = 201;
  ...
  .post("/api/v2/upgrade-rehearsals/:rehearsal_id/promote", async (context: ParamContext): Promise<unknown> => {
    const row = await db.query.durableJobs.findFirst({
      where: and(eq(durableJobs.id, context.params["rehearsal_id"] ?? ""), eq(durableJobs.kind, "upgrade-rehearsal")),
    });
    if (row === undefined || typeof row.payload["organizationId"] !== "string") return notFound(context.set);
    const workspaceId = typeof row.payload["workspaceId"] === "string" ? row.payload["workspaceId"] : "";
    ...
    return errorDocument(
      context.set,
      409,
      "Rehearsals are review-only. Promote an explicit workspace engine/settings change, then run the normal plan/apply flow",
  ...
  .post("/api/v2/organizations/:org_name/policy-playground", async (context: ParamContext): Promise<unknown> => {
    const organization = await organizationForName(context.params["org_name"] ?? "");
    if (organization === undefined || !(await organizationCanManage(organization, context, "manage-policies")))
      return notFound(context.set);
    const attrs = attributesFrom(context.body);
    ...
    const artifact = await createPlatformArtifact({
      kind: "policy-playground",
      organizationId: organization.id,
      actorId: context.user?.id ?? null,
      payload: {
        result,
        source: source.length > 0 ? "provided" : "missing",
        "policy-digest": result["policy-digest"],
        "plan-digest": result["plan-digest"],
        "apply-authority": false,
      },
      status: result["status"] === "invalid" ? "invalid" : "review-only",
    });
    context.set.status = 201;
    return { data: artifactResource(artifact, "policy-playground-runs") };
  })
  .get(
    "/api/v2/organizations/:org_name/policy-playground/:playground_id",
    async (context: ParamContext): Promise<unknown> => {
      const organization = await organizationForName(context.params["org_name"] ?? "");
      if (organization === undefined || !(await organizationCanManage(organization, context, "manage-policies")))
        return notFound(context.set);
      const row = await getPlatformArtifact(
        context.params["playground_id"] ?? "",
        "policy-playground",
        organization.id,
      );
      if (row === undefined) return notFound(context.set);
