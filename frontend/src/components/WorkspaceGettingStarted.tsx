...
function engineCheck(engine: string, workspacePath: string): ReadinessCheck {
  ...
  return {
    ...
    detail: missing ? "Select Terraform or OpenTofu in workspace settings." : `${engine === "tofu" ? "OpenTofu" : "Terraform"} is configured; the first plan verifies its availability.`,
    href: missing ? `${workspacePath}/settings` : undefined,
  };
}

function inputsCheck(
  inputState: ReadinessState,
...
function credentialsCheck(
  ...
  return {
    ...
    state: ready ? "ready" : "missing",
    detail: ready ? (localExecution ? "Credentials stay with the CLI session." : hasRepository ? "The connected repository supplies the configuration source." : "The server workspace mount supplies the configuration source.") : "Connect a source before credentials can be used.",
    href: ready ? undefined : `${workspacePath}/settings/version-control`,
  };
}
...
function GettingStartedIntro({ compact, localExecution, hasRepository, source, isLocked }: Readonly<{
  compact: boolean;
  localExecution: boolean;
  hasRepository: boolean;
  source: string | undefined;
...
function ReadinessSummary({ readiness, readinessComplete }: Readonly<{
...
function CliSetupGuide({ configuration, cli, hostname, localExecution, onCopy }: Readonly<{
    ...
    <>
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <Button size="icon-sm" variant="ghost" aria-label="Copy backend configuration" onClick={onCopy}><Copy aria-hidden="true" /></Button>
        </div>
...
function QuickActions({ usesServerCode, canQueueRun, isLocked, canReadVariable, hasRepository, canUpdate, workspacePath }: Readonly<{
  ...
  return (
    <div className="rounded-md border border-border bg-muted/20 p-4" aria-live="polite">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button size="sm" variant="outline" onClick={onRunPreflight} disabled={preflightLoading} aria-busy={preflightLoading}>
...
export function WorkspaceGettingStarted({
  ...
  return (
    <div className="space-y-5">
      <GettingStartedIntro compact={compact} localExecution={localExecution} hasRepository={hasRepository} source={source} isLocked={isLocked} />
