...
function parseInviteAttributes(body: unknown): { email: string; role: string } | { failure: InviteFailure } {
  ...
  if (email === null)
    return { failure: inviteFailure(422, "Unprocessable Entity", "A valid email is required for invitations") };
  if (
    attrs["role"] !== undefined &&
    (typeof attrs["role"] !== "string" || !["owner", "member"].includes(attrs["role"]))
  ) {
...
async function checkInviteOwnerGrant(
  ...
  return (
    role !== "owner" || user?.isSiteAdmin === true || checkOrgPermission(user?.id, orgId, "owner", tokenOrgId, null)
  );
}
...
async function checkInviteConflicts(orgId: string, email: string): Promise<{ ok: true } | { failure: InviteFailure }> {
  ...
  if (existingMember !== undefined) {
    const mem = await db.query.organizationMemberships.findFirst({
      where: and(eq(organizationMemberships.orgId, orgId), eq(organizationMemberships.userId, existingMember.id)),
    });
  ...
  const pending = await db.query.organizationInvitations.findFirst({
    where: and(
      ...
      eq(organizationInvitations.emailNormalized, email),
      eq(organizationInvitations.status, "pending"),
    ),
  });
  if (pending !== undefined)
    return { failure: inviteFailure(409, "Conflict", "An invitation for this email is already pending") };
  return { ok: true };
}
...
async function loadInviteByToken(rawToken: string): Promise<{ invite: InviteRow } | { failure: InviteFailure }> {
  ...
  const inviteRows = await db.query.organizationInvitations.findMany({
    where: inArray(organizationInvitations.tokenHash, [tokenHash, legacyTokenHash]),
    limit: 2,
  });
...
async function checkInviteAcceptable(invite: InviteRow): Promise<{ ok: true } | { failure: InviteFailure }> {
  if (invite.status !== "pending")
    return { failure: inviteFailure(422, "Unprocessable Entity", `Invitation is ${invite.status}`) };
...
function checkAcceptorEligibility(user: Acceptor, invite: InviteRow): { ok: true } | { failure: InviteFailure } {
  ...
  const acceptorEmail = normalizeEmail(
    ((user as unknown as Record<string, unknown>)["email"] as string | null | undefined) ?? null,
  );
...
async function acceptWithExistingMembership(
  ...
  try {
    activated = await db.transaction(async (tx: unknown): Promise<boolean> => {
      ...
      const claimed = await t
        .update(organizationInvitations)
        .set({ status: "accepted", acceptedBy: user.id, updatedAt: Date.now() })
      ...
      if (existing.status === "invited") {
        const activation = await t
          ...
          .set({ status: "active" })
          .where(and(eq(organizationMemberships.id, existing.id), eq(organizationMemberships.status, "invited")))
  ...
  await auditLog("accept", "organization-invitations", resultInvite.id, user.id, resultInvite.orgId, {
    email: resultInvite.email,
  });
  if (existing.status === "invited") {
    await auditLog("update", "organization-memberships", existing.id, user.id, resultInvite.orgId, {
      status: "active",
      via: "invitation",
    });
...
async function finalizeNewAcceptance(invite: InviteRow, user: Acceptor, set: SetObj): Promise<unknown> {
  ...
  await auditLog("create", "organization-memberships", invite.id, user.id, invite.orgId, {
    email: invite.email,
    role: invite.role,
    via: "invitation",
  });
  publish("authz.changed", { "user-id": user.id, "org-id": invite.orgId });
  const updated = await db.query.organizationInvitations.findFirst({
    where: eq(organizationInvitations.id, invite.id),
  });
...
export const organizationInvitationRoutes = new Elysia({ name: "organization-invitations" })
  ...
  .get(
    "/api/v2/organizations/:org_name/organization-invitations",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: Ctx): Promise<unknown> => {
      const org = await cachedOrgByName(params["org_name"] ?? "");
      if (
        org === undefined ||
        !(await checkOrganizationPermission(org.id, user?.id, tokenOrgId, tokenTeamId ?? null, "manage-membership"))
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
      const rows = await db.query.organizationInvitations.findMany({
        where: eq(organizationInvitations.orgId, org.id),
      });
  ...
  .post(
    "/api/v2/organizations/:org_name/organization-invitations",
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: Ctx): Promise<unknown> => {
      const org = await cachedOrgByName(params["org_name"] ?? "");
      if (
        org === undefined ||
        !(await checkOrganizationPermission(org.id, user?.id, tokenOrgId, tokenTeamId ?? null, "manage-membership"))
      ...
      if ("failure" in conflicts) {
        (set as { status: number }).status = conflicts.failure.status;
        return conflicts.failure.body;
      }
      const created = await insertInvitation(org.id, parsed.email, parsed.role, user?.id ?? null);
      if ("failure" in created) {
        (set as { status: number }).status = created.failure.status;
        return created.failure.body;
      }
  ...
  .delete(
    "/api/v2/organizations/:org_name/organization-invitations/:id",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: Ctx): Promise<unknown> => {
      const org = await cachedOrgByName(params["org_name"] ?? "");
      const id = params["id"] ?? "";
      if (org === undefined) {
      ...
      const row = await db.query.organizationInvitations.findFirst({
        where: and(eq(organizationInvitations.id, id), eq(organizationInvitations.orgId, org.id)),
      });
      if (
        row === undefined ||
        !(await checkOrganizationPermission(org.id, user?.id, tokenOrgId, tokenTeamId ?? null, "manage-membership"))
      ...
      if (row.status !== "pending") {
        ...
        return {
          errors: [
            { status: "422", title: "Unprocessable Entity", detail: "Only pending invitations can be cancelled" },
          ],
        };
      }
      await db
  ...
  .post("/api/v2/organization-invitations/:token/accept", async ({ params, user, set }: Ctx): Promise<unknown> => {
    ...
    if ("failure" in loaded) {
      (set as { status: number }).status = loaded.failure.status;
      return loaded.failure.body;
    }
    const acceptable = await checkInviteAcceptable(loaded.invite);
    if ("failure" in acceptable) {
      (set as { status: number }).status = acceptable.failure.status;
      return acceptable.failure.body;
    }
    const eligible = checkAcceptorEligibility(user, loaded.invite);
    if ("failure" in eligible) {
      (set as { status: number }).status = eligible.failure.status;
      return eligible.failure.body;
    }
    const existing = await db.query.organizationMemberships.findFirst({
      where: and(eq(organizationMemberships.orgId, loaded.invite.orgId), eq(organizationMemberships.userId, user.id)),
    });
    if (existing !== undefined) {
      const accepted = await acceptWithExistingMembership(loaded.invite, existing, user);
      if ("failure" in accepted) {
        (set as { status: number }).status = accepted.failure.status;
        return accepted.failure.body;
      }
