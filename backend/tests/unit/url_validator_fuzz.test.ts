...
const PUBLIC_V4 = ["8.8.8.8", "1.1.1.1", "93.184.216.34", "172.32.0.1", "128.0.0.1", "11.0.0.1"];
...
describe("fuzz: outbound URL validator", () => {
  ...
  it("rejects DNS rebinding answers with an injectable resolver", async (): Promise<void> => {
    ...
    expect(await validateExternalUrlResolved("http://127.0.0.1.nip.io/", false, rebindingResolver)).toContain(
      "private",
    );
    ...
    expect(await validateExternalUrlResolved("http://127.0.0.1.nip.io/", false, uncompressedResolver)).toContain(
      "private",
    );
  ...
  it("returns a validated address that callers can pin without a second DNS lookup", async () => {
    ...
    const mixed = await resolveExternalUrl("https://example.com/hook", false, async () => [
      "93.184.216.34",
      "127.0.0.1",
    ]);
