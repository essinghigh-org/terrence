...
// the provider reads; the relationship id must be the org NAME (the reference format uses names
// as org identifiers).
...
function providerSetUpdates(attrs: Record<string, unknown>): Partial<typeof providerSets.$inferInsert> {
  ...
  if (attrs["configuration-hcl"] !== undefined)
    updates.configurationHcl = typeof attrs["configuration-hcl"] === "string" ? attrs["configuration-hcl"] : null;
...
export const providerSetRoutes = new Elysia({ name: "provider-sets" })
  .use(authPlugin)
  .get(
    "/api/v2/organizations/:org_name/provider-sets",
  ...
  .post(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (name === "" || providerSource === "") {
        ...
        return {
          errors: [{ status: "422", title: "Unprocessable Entity", detail: "name and provider-source are required" }],
        };
      }
  ...
  .get(
    "/api/v2/provider-sets/:provider_set_id",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
  ...
  .get(
    "/api/v2/organizations/:org_name/provider-sets/:name",
    async ({ params, user, orgId: tokenOrgId, teamId, set }: ParamCtx): Promise<unknown> => {
      const orgName = params["org_name"] ?? "";
      ...
      if (
        ...
        return notFound(set);
      const row = await db.query.providerSets.findFirst({
        where: and(eq(providerSets.orgId, org.id), eq(providerSets.name, name)),
      });
      if (row === undefined) return notFound(set);
      return { data: providerSetResource(row, orgName) };
    },
