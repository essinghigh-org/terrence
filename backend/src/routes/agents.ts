...
function stackAgentJobResource(details: ClaimedStackAgentJob): Record<string, unknown> {
  ...
  return {
    ...
    relationships: {
      ...
      "stack-deployment-run": { data: { id: deploymentRun.id, type: "stack-deployment-runs" } },
      "stack-deployment-step": { data: { id: step.id, type: "stack-deployment-steps" } },
      agent: { data: { id: job.agentId, type: "agents" } },
    },
  };
}
...
export const agentRoutes = new Elysia({ name: "agents" })
  ...
  .post("/api/v2/agents/:agent_id/stack-jobs/poll", async ({ params, request, set }: ParamCtx): Promise<unknown> => {
    ...
    const job = await claimStackAgentJob(agent);
    if (job === undefined) {
      (set as { status: number }).status = 204;
      return {};
    }
