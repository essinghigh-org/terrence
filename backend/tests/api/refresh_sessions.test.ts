...
describe("browser refresh sessions", () => {
  ...
  const accountStatus = (token: string): Promise<Response> =>
    app.handle(
      new Request("http://terrence.test/api/v2/account/details", { headers: { Authorization: `Bearer ${token}` } }),
    );

  const authenticatedRequest = (path: string, token: string, method = "GET"): Promise<Response> =>
    app.handle(
      new Request(`http://terrence.test${path}`, {
        method,
  ...
  test("rotates once, detects reuse, and revokes the token family", async () => {
    ...
    const firstSession = await db.query.refreshSessions.findFirst({
      where: eq(refreshSessions.tokenHash, hashAuthenticationToken(firstRefreshToken)),
    });
    ...
    expect(
      (
        await db.query.refreshSessions.findMany({
          where: eq(refreshSessions.familyId, firstSession?.familyId ?? ""),
        })
      ).every((session): boolean => session.revokedAt === null),
    ).toBeTrue();
  ...
  test("logout clears the cookie and revokes its family", async () => {
    ...
    const session = await db.query.refreshSessions.findFirst({
      where: eq(refreshSessions.tokenHash, hashAuthenticationToken(rawCookieToken(refreshCookie))),
    });
    const loginDocument = (await loginResponse.json()) as {
    ...
    expect(
      ...
      ).every((row): boolean => row.revokedAt !== null),
    ).toBeTrue();
  });
  ...
  test("lists only safe own-session metadata and revokes whole families", async () => {
    ...
    const listResponse = await authenticatedRequest("/api/v2/account/sessions", rotatedDocument.data.attributes.token);
    ...
    expect(personalTokensDocument.data.map((token) => token.attributes.description)).not.toContain(
    ...
    expect(await accountStatus(otherDocument.data.attributes.token).then((response): number => response.status)).toBe(
      200,
    );
    expect(
      (
    ...
    expect(await accountStatus(secondDocument.data.attributes.token).then((response): number => response.status)).toBe(
      401,
    );
    expect(await accountStatus(rotatedDocument.data.attributes.token).then((response): number => response.status)).toBe(
      200,
    );
    expect(
      (
    ...
    expect(
      await db.query.apiTokens.findMany({
        where: inArray(
          apiTokens.id,
          currentFamily.map((session): string => session.accessTokenId),
        ),
      }),
    ).toHaveLength(0);
    expect(await accountStatus(otherDocument.data.attributes.token).then((response): number => response.status)).toBe(
  ...
  test("suspending a user invalidates refresh rotation", async () => {
    ...
    try {
      ...
      expect(
        (await db.query.refreshSessions.findMany({ where: eq(refreshSessions.userId, userId) })).every(
