  ...
  return (
    <PageShell>
      <div role={error !== null ? "alert" : undefined}>
        <EmptyState
          illustration={error !== null ? "failed" : "lost"}
          title={error !== null ? "Module unavailable" : "Module not found"}
          description={error ?? "This module may have moved or been removed."}
        />
        <div className="text-center">
          <Link className={buttonVariants({ variant: "outline" })} to={`/app/${encodeURIComponent(orgName)}/registry`}>
            Back to registry
          </Link>
        </div>
      </div>
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
        <div className="space-y-2">
          <h1 className="text-balance text-3xl font-bold tracking-tight text-foreground">{name}</h1>
          <p className="text-sm text-muted-foreground">Published by {namespace}</p>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Badge variant="secondary">{publishingLabel(module)}</Badge>
            {selectedVersion?.deprecated === true && <Badge variant="outline">Deprecated</Badge>}
            {selectedVersion?.revoked === true && <Badge variant="destructive">Revoked</Badge>}
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          {module.permissions.canResync && (
            <Button type="button" variant="outline" disabled={busy} onClick={onResync}>
          ...
          {module.permissions.canDelete && module.publishingWorkflow !== "tag" && (
      ...
      {selectedVersion?.ingestError != null && (
        <div
          role="alert"
          className="flex gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive"
        >
          <AlertTriangle aria-hidden="true" className="mt-0.5 size-4 shrink-0" />
          {selectedVersion.ingestError}
        </div>
      )}
    </div>
  );
    ...
    <Card size="sm">
      <CardContent className="grid gap-4 pt-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {versions.map(
              (version): React.JSX.Element => (
                <SelectItem key={version.id} value={version.version}>
                  {version.version}
                  {version.revoked ? " · revoked" : version.deprecated ? " · deprecated" : ""}
                </SelectItem>
              ),
            )}
          </Select>
        ...
        <div>
          ...
          <p className="mt-2 text-sm">{dateLabel(selectedVersion?.publishedAt ?? null)}</p>
        </div>
        <div>
          <p className="text-xs text-muted-foreground">Revision</p>
          <p className="mt-2 truncate font-mono text-sm" title={selectedVersion?.commitSha ?? undefined}>
            {revisionLabel(selectedVersion)}
          </p>
        </div>
        <div>
          <p className="text-xs text-muted-foreground">Repository</p>
          {module.vcsRepo?.repositoryUrl != null ? (
            <a
              className="mt-2 inline-flex items-center gap-1 text-sm text-primary hover:underline"
              href={module.vcsRepo.repositoryUrl}
              target="_blank"
              rel="noreferrer"
            >
              {module.vcsRepo.displayIdentifier ?? module.vcsRepo.identifier}
...
function ReadmePanel({ section }: Readonly<{ section: RegistryModuleSection }>): React.JSX.Element {
  if (section.readme === "")
    return <p className="text-sm text-muted-foreground">No README was published for this section.</p>;
          ...
          <Card size="sm" key={input.name}>
            <CardHeader>
              <CardTitle className="flex flex-wrap items-center gap-2 font-mono text-sm">
                ...
                {input.sensitive && <Badge variant="outline">Sensitive</Badge>}
              </CardTitle>
            ...
            <CardContent className="grid gap-2 text-xs sm:grid-cols-2">
              <div>
                ...
                <pre className="mt-1 overflow-x-auto rounded bg-muted p-2">{input.type}</pre>
              </div>
              {!input.required && (
                <div>
                  <span className="text-muted-foreground">Default</span>
                  <pre className="mt-1 max-h-28 overflow-auto rounded bg-muted p-2">
                    {displayDefault(input.defaultValue)}
                  </pre>
                </div>
              )}
            </CardContent>
          ...
          <Card size="sm" key={output.name}>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 font-mono text-sm">
                {output.name}
                {output.sensitive && <Badge variant="outline">Sensitive</Badge>}
              </CardTitle>
      ...
      <Card size="sm">
        ...
        <CardContent className="space-y-3">
          {section.providers.length === 0 ? (
            <p className="text-sm text-muted-foreground">No provider requirements.</p>
          ) : (
            section.providers.map(
              (dependency): React.JSX.Element => (
                <div key={dependency.name}>
                  <code>{dependency.source ?? dependency.name}</code>
                  <p className="text-xs text-muted-foreground">
                    {dependency.versionConstraint ?? "No version constraint"}
                  </p>
                </div>
              ),
            )
      ...
      <Card size="sm">
        ...
        <CardContent className="space-y-3">
          {section.modules.length === 0 ? (
            <p className="text-sm text-muted-foreground">No child modules.</p>
          ) : (
            section.modules.map(
              (dependency): React.JSX.Element => (
                <div key={dependency.name}>
                  ...
                  <p className="break-all text-xs text-muted-foreground">
                    {dependency.source ?? "Local source"}
                    {dependency.versionConstraint === null ? "" : ` · ${dependency.versionConstraint}`}
                  </p>
                </div>
              ),
            )
    ...
    <div className="space-y-2">
      {section.resources.map(
        (resource): React.JSX.Element => (
          <div
            key={`${resource.mode}-${resource.type}-${resource.name}`}
          ...
          <Select id="module-section" value={section?.path ?? "."} onValueChange={onSectionPathChange}>
            {sections.map(
              (item): React.JSX.Element => (
                <SelectItem value={item.path} key={item.path}>
                  {item.path === "."
                    ? "Root module"
                    : item.path.startsWith("modules/")
                      ? `Submodule · ${item.path.slice(8)}`
                      : `Example · ${item.path.slice(9)}`}
                </SelectItem>
              ),
            )}
          </Select>
        ...
        {tabs.map(
          (item, index): React.JSX.Element => (
            <button
              ...
              className={cn(
                "-mb-px whitespace-nowrap border-b-2 px-3 py-2 text-sm font-medium outline-none focus-visible:ring-2 focus-visible:ring-ring",
                tab === item.id
        ...
        {section === null ? (
          <Empty>
            <EmptyHeader>
              ...
              <EmptyDescription>This version has not produced registry documentation.</EmptyDescription>
            </EmptyHeader>
          </Empty>
        ) : tab === "readme" ? (
          <ReadmePanel section={section} />
        ) : tab === "inputs" ? (
          <InputsPanel section={section} />
        ) : tab === "outputs" ? (
          <OutputsPanel section={section} />
        ) : tab === "dependencies" ? (
          <DependenciesPanel section={section} />
        ) : (
...
function ModuleAside({
  ...
  return (
    <aside className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle>Use this module</CardTitle>
          <CardDescription>Pin the selected version in Terraform or OpenTofu.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <pre className="overflow-x-auto rounded-md bg-code-background p-3 text-xs text-code-foreground">
            <code>{usage}</code>
          </pre>
          <Button
            type="button"
          ...
          >
            {copied === "config" ? <Check aria-hidden="true" /> : <Copy aria-hidden="true" />}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Use a personal API token locally. Terrence workers receive scoped run credentials automatically.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <Button
            ...
            className="w-full"
            onClick={(): void => {
              onCopy(credentials, "credentials");
            }}
          >
            {copied === "credentials" ? <Check aria-hidden="true" /> : <Copy aria-hidden="true" />}
            {copied === "credentials" ? "Copied" : "Copy credentials block"}
          </Button>
          <Link className="block text-center text-sm text-primary hover:underline" to="/app/account#api-tokens">
      ...
      {module.permissions.canDelete && selectedVersion !== null && (
        <Card>
          <CardHeader>
            <CardTitle>Manage version</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-2">
            <Button
              type="button"
              variant="outline"
              disabled={busy || selectedVersion.revoked}
              onClick={(): void => {
                onMutate(
                  async (): Promise<unknown> =>
                    await fetchApi(`/registry-module-versions/${selectedVersion.id}`, {
                      method: "PATCH",
                      body: JSON.stringify({
                        data: {
                          type: "registry-module-versions",
                          attributes: { deprecated: !selectedVersion.deprecated },
                        },
                      }),
                    }),
                  selectedVersion.deprecated ? "Deprecation reverted" : "Version deprecated",
                );
              }}
            >
              {selectedVersion.deprecated ? "Revert deprecation" : "Deprecate"}
            </Button>
            <Button
              type="button"
              variant="outline"
              ...
              onClick={(): void => {
                if (selectedVersion.revoked) {
                  onMutate(
                    async (): Promise<unknown> =>
                      await fetchApi(`/registry-module-versions/${selectedVersion.id}/actions/revert-revocation`, {
                        method: "POST",
                      }),
                    "Revocation reverted",
                  );
            ...
            >
              {selectedVersion.revoked ? "Revert revocation" : "Revoke"}
            </Button>
            <Button
              type="button"
              variant="outline"
              disabled={busy || selectedVersion.status !== "ok"}
              onClick={(): void => {
                onMutate(
                  async (): Promise<unknown> =>
                    await fetchApi(`/registry-modules/${module.id}/versions/${selectedVersion.version}/actions/test`, {
                      method: "POST",
                    }),
            ...
            <Button
              ...
              variant="destructive"
              onClick={(): void => {
                onConfirmRequest("delete-version");
              }}
            >
              Delete version
            </Button>
            <Button
              ...
              variant="destructive"
              onClick={(): void => {
                onConfirmRequest("delete-module");
              }}
            >
              Delete module
            </Button>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {module.publishingMechanism === "manual"
              ? "Upload a real archive for a new semantic version."
              : "Publish the current configured branch revision as a new semantic version."}
          </DialogDescription>
        </DialogHeader>
        <FieldGroup>
          <Field>
            ...
            <Input
              id="detail-new-version"
              value={newVersion}
              onChange={(event): void => {
                onNewVersionChange(event.target.value);
              }}
              placeholder="1.1.0"
          ...
          {module.publishingMechanism === "manual" && (
            <Field>
              ...
              <Input
                id="detail-new-archive"
                type="file"
                accept=".tar.gz,application/gzip"
                onChange={(event): void => {
                  onNewArchiveChange(event.target.files?.[0] ?? null);
                }}
              />
            </Field>
          )}
        ...
        <DialogFooter>
          ...
          <Button type="button" disabled={busy} onClick={onPublish}>
            {busy ? "Publishing…" : pendingVersionId === null ? "Publish version" : "Retry upload"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function SettingsDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Publication settings</DialogTitle>
          <DialogDescription>
            Workflow transitions are intentionally disabled because they change release semantics.
          </DialogDescription>
        </DialogHeader>
        <FieldGroup>
          <Field>
            <FieldLabel>VCS connection</FieldLabel>
            <Input value={module.vcsRepo?.displayIdentifier ?? "Manual / API"} disabled />
          </Field>
          <Field>
            <FieldLabel htmlFor="detail-source-directory">Source directory</FieldLabel>
            <Input
              id="detail-source-directory"
              value={sourceDirectory}
              onChange={(event): void => {
                onSourceDirectoryChange(event.target.value);
              }}
              disabled={module.publishingMechanism !== "vcs"}
            />
          </Field>
          {module.publishingWorkflow === "tag" && (
            <Field>
              <FieldLabel htmlFor="detail-tag-prefix">Tag prefix</FieldLabel>
              <Input
                id="detail-tag-prefix"
                value={tagPrefix}
                onChange={(event): void => {
                  onTagPrefixChange(event.target.value);
                }}
              />
            </Field>
          )}
        ...
        <DialogFooter>
          ...
          <Button type="button" disabled={busy || module.publishingMechanism !== "vcs"} onClick={onSave}>
            Save
          </Button>
        </DialogFooter>
      </DialogContent>
...
function ModuleConfirms({
  ...
  return (
    <ConfirmDialog
      ...
      description={
        confirmation === "revoke"
          ? "Revoked versions are removed from Terraform version discovery and cannot be downloaded."
          : "This permanently removes the selected registry artifact."
      }
...
export function RegistryModuleDetail(): React.JSX.Element {
  ...
  useEffect((): (() => void) => {
    ...
    void fetchApi(path, { signal: controller.signal })
      .then(async (response): Promise<void> => {
        ...
        // SAFETY: the fixture matches the JSON:API envelope the component consumes.
        const versionResponse = (await fetchApi(`/registry-modules/${encodeURIComponent(loadedModule.id)}/versions`, {
          signal: controller.signal,
        })) as { data?: unknown[] };
        if (controller.signal.reason !== undefined) return;
        const loadedVersions = Array.isArray(versionResponse.data)
          ? versionResponse.data.map(registryModuleVersionFromResource)
          : [];
      ...
      .catch((caught: unknown): void => {
        if (!controller.signal.aborted)
          setError(caught instanceof Error ? caught.message : "Module could not be loaded.");
      })
      .finally((): void => {
  ...
  const selectedVersion =
    versions.find((version): boolean => version.version === searchParams.get("version")) ?? versions[0] ?? null;
  const metadata = selectedVersion?.metadata ?? null;
  const sections = useMemo(
    (): RegistryModuleSection[] => (metadata === null ? [] : [metadata, ...metadata.submodules, ...metadata.examples]),
    [metadata],
  );
  ...
  const discardPendingVersion = (): void => {
    ...
    if (versionId !== null) {
      void fetchApi(`/registry-module-versions/${encodeURIComponent(versionId)}`, { method: "DELETE" }).catch(
        (): void => {
          return;
        },
      );
    ...
    try {
      ...
      if (versionId === null) {
        ...
        const response = (await fetchApi(`/registry-modules/${encodeURIComponent(module.id)}/versions`, {
          method: "POST",
          body: JSON.stringify({
            data: { type: "registry-module-versions", attributes: { version: newVersion.trim() } },
          }),
        })) as { data: { id: string } };
  ...
  const handleConfirmAction = async (): Promise<void> => {
    ...
    if (confirmation === "revoke")
      await mutate(
        async (): Promise<unknown> =>
          await fetchApi(`/registry-module-versions/${selectedVersion.id}/actions/revoke`, { method: "POST" }),
        "Version revoked",
      );
    if (confirmation === "delete-version")
      await mutate(
        async (): Promise<unknown> =>
          await fetchApi(`/registry-module-versions/${selectedVersion.id}`, { method: "DELETE" }),
        "Version deleted",
      );
    if (confirmation === "delete-module") {
      setBusy(true);
      await fetchApi(`/registry-modules/${module.id}`, { method: "DELETE" });
      await navigate(registryPath);
    }
  ...
  const handleSaveSettings = (): void => {
    void mutate(async (): Promise<void> => {
      await fetchApi(
        `/organizations/${encodeURIComponent(orgName)}/registry-modules/private/${encodeURIComponent(namespace)}/${encodeURIComponent(name)}/${encodeURIComponent(provider)}`,
        {
          method: "PATCH",
          body: JSON.stringify({
            data: {
              type: "registry-modules",
              attributes: { "source-directory": sourceDirectory, "tag-prefix": tagPrefix },
            },
          }),
        },
      );
      ...
      <div className="space-y-6">
        <ModuleHeader
          ...
          onResync={(): void => {
            void mutate(
              async (): Promise<unknown> =>
                await fetchApi(`/registry-modules/${module.id}/actions/resync`, { method: "POST" }),
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
          <div className="min-w-0 space-y-5">
            <VersionInfoCard
              ...
              versions={versions}
              selectedVersion={selectedVersion}
              onSelectVersion={updateSelectedVersion}
            />
            <ModuleDocs
              sections={sections}
              section={section}
          ...
          <ModuleAside
            ...
            copied={copied}
            onCopy={(value, kind): void => {
              void copy(value, kind);
            }}
            module={module}
            selectedVersion={selectedVersion}
            busy={busy}
            onMutate={(operation, success): void => {
              void mutate(operation, success);
            }}
