export type IntegrationConfiguration = Readonly<
  Record<"GITHUB_API_URL" | "GITHUB_APP_API_URL", string> &
    ...
    Record<
      "TERRENCE_AGENT_UPDATE_VERSION" | "TERRENCE_AGENT_UPDATE_URL" | "TERRENCE_AGENT_UPDATE_SHA256",
      string | null
    > &
...
type AgentUpdateConfiguration = Pick<
...
function agentUpdate(environment: Readonly<Record<string, string | undefined>>): AgentUpdateConfiguration {
  ...
  if (version === undefined || url === undefined || sha256 === undefined)
    throw new Error("Agent update version, URL, and SHA256 must be configured together");
  if (!/^[a-zA-Z0-9._+-]{1,128}$/.test(version) || !/^[a-fA-F0-9]{64}$/.test(sha256))
    throw new Error("Invalid agent update version or SHA256 configuration");
  try {
    ...
    throw new Error("Agent update URL must be HTTP(S) without credentials or fragment");
  }
  return {
    TERRENCE_AGENT_UPDATE_VERSION: version,
    TERRENCE_AGENT_UPDATE_URL: url,
    TERRENCE_AGENT_UPDATE_SHA256: sha256.toLowerCase(),
  };
}

export function parseIntegrationConfiguration(
  environment: Readonly<Record<string, string | undefined>>,
): IntegrationConfiguration {
  const status = (environment["TERRENCE_NODE_STATUS"] ?? "active").toLowerCase();
  if (status !== "active" && status !== "draining" && status !== "maintenance")
    throw new Error("TERRENCE_NODE_STATUS must be active, draining, or maintenance");
  const general =
    httpBase(environment["GITHUB_API_URL"], "GITHUB_API_URL", "https://api.github.com") ?? "https://api.github.com";
