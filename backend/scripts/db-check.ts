...
type CheckResult = {
  ...
  detail: string[];
  wal: Readonly<{
    checkpoint: "NOOP" | "PASSIVE" | "TRUNCATE";
    busy: number;
    log: number;
    checkpointed: number;
...
function walStats(engine: Database, mode: "NOOP" | "PASSIVE" | "TRUNCATE"): CheckResult["wal"] {
  ...
  let checkpointed = -1;
  try {
    const row = engine.query(`PRAGMA wal_checkpoint(${mode})`).get() as {
      busy: number;
      log: number;
      checkpointed: number;
    } | null;
...
function run(): void {
  ...
  const engine = checkpoint ? new Database(dbPath) : new Database(dbPath, { readonly: true });
  ...
  const rows = engine.query(`PRAGMA ${pragma}`).all() as (
    | { quick_check?: string; integrity_check?: string }
    | Record<string, unknown>
  )[];
  const detail = rows.map((row): string => {
    const value =
      "quick_check" in row
        ? String(row.quick_check)
        : "integrity_check" in row
          ? String(row.integrity_check)
          : toComparableString(Object.values(row)[0] ?? "");
    return value;
  });
  ...
  if (asJson) {
    ...
    console.log(
      `db-check: ${result.mode} OK (${dbPath}, ${result.dbSizeBytes ?? -1} bytes, wal ${result.wal.walSizeBytes ?? -1} bytes)`,
    );
  } else {
      ...
      JSON.stringify(
        {
          ...
          detail: [message],
          wal: {
            checkpoint: checkpoint ? "TRUNCATE" : "PASSIVE",
            busy: -1,
            log: -1,
            checkpointed: -1,
            walSizeBytes: null,
