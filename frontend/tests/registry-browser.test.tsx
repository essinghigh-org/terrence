...
const moduleResource = (canManage = false) => ({
  ...
  attributes: {
    ...
    "version-statuses": [
      { version: "2.0.0", status: "ok", deprecated: false, revoked: false },
...
test("browses and filters registry cards with distinct module permissions", async () => {
        ...
        data: [
          {
            id: "hashicorp/aws",
            type: "provider-icons",
            attributes: { "icon-url": "/icons/providers/hashicorp-aws.svg" },
          },
          {
            id: "acme/sendgrid",
            type: "provider-icons",
            attributes: { "icon-url": "/icons/providers/acme-sendgrid.svg" },
          },
        ],
      });
    if (url === "/api/v2/organizations/acme")
      return json({
        data: { attributes: { permissions: { "can-manage-providers": true, "can-manage-modules": false } } },
      });
    if (url.startsWith("/api/v2/organizations/acme/registry-modules?"))
      return json({
        data: [moduleResource()],
        meta: { pagination: { "total-pages": 2, "total-count": 1 }, providers: ["aws", "azurerm"] },
      });
  ...
  await waitFor((): void => {
    expect(view.getByAltText("aws provider logo")).toBeTruthy();
  });
  ...
  await waitFor((): void => {
    const latest = new URL(
      requests.filter((url): boolean => url.includes("registry-modules?")).at(-1) ?? "",
      "http://localhost",
    );
  ...
  await waitFor((): void => {
    expect(requests.some((url): boolean => url.includes("page%5Bnumber%5D=2"))).toBeTrue();
  });
...
test("renders the provider icon fallback after artwork loading fails", async () => {
  ...
  await waitFor((): void => {
    expect(view.getByTestId("provider-icon-fallback")).toBeTruthy();
  });
});
...
test("uses provider dependency sources without guessing namespaces", async () => {
  ...
  await waitFor((): void => {
    expect(view.getByAltText("cloudflare provider logo").getAttribute("src")).toBe(
      "/api/v2/provider-icons/registry.terraform.io/cloudflare/cloudflare",
    );
    expect(view.getByAltText("github provider logo").getAttribute("src")).toBe(
      "/api/v2/provider-icons/registry.terraform.io/integrations/github",
    );
    expect(view.getByAltText("tfe provider logo").getAttribute("src")).toBe(
      "/api/v2/provider-icons/registry.terraform.io/hashicorp/tfe",
    );
    expect(view.getByAltText("community provider logo").getAttribute("src")).toBe(
      "/api/v2/provider-icons/registry.terraform.io/acme/widgets",
    );
  });
...
test("shows loading, retryable errors, and an honest empty state", async () => {
  ...
  const first = new Promise<Response>((resolve): void => {
  ...
  globalThis.fetch = mock(async (input: string | URL | Request): Promise<Response> => {
    const url = urlOf(input);
    if (url === "/api/v2/organizations/acme")
      return json({ data: { attributes: { permissions: { "can-manage-modules": true } } } });
    if (url.startsWith("/api/v2/organizations/acme/registry-modules?")) {
      attempts += 1;
...
test("renders version-specific module documentation, usage, lifecycle, and keyboard tabs", async () => {
  ...
  globalThis.fetch = mock(async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
    ...
    if (url === "/api/v2/organizations/acme/registry-modules/private/acme/network/aws")
      return json({ data: moduleResource(true) });
    if (url === "/api/v2/registry-modules/mod-network/versions")
      return json({
        data: [
          versionResource("version-2", "2.0.0", "# Version two"),
          versionResource("version-1", "1.0.0", "# Root docs"),
        ],
      });
    if (url === "/api/v2/registry-module-versions/version-1" && method === "PATCH")
      return json({ data: versionResource("version-1", "1.0.0", "# Root docs") });
    throw new Error(`Unexpected request: ${method} ${url}`);
  }) as unknown as typeof fetch;

  const view = render(
    <MemoryRouter initialEntries={["/app/acme/registry/modules/acme/network/aws?version=1.0.0"]}>
      <Routes>
        <Route path="/app/:orgName/registry/modules/:namespace/:name/:provider" element={<RegistryModuleDetail />} />
      </Routes>
    </MemoryRouter>,
  );
  ...
  await waitFor((): void => {
    expect(
      requests.some(
        (request): boolean => request.method === "PATCH" && request.url.endsWith("/registry-module-versions/version-1"),
      ),
    ).toBeTrue();
  });
});
...
test("keeps module management hidden from a read-only detail viewer", async () => {
  ...
  globalThis.fetch = mock(async (input: string | URL | Request): Promise<Response> => {
    const url = urlOf(input);
    if (url.endsWith("/organizations/acme/registry-modules/private/acme/network/aws"))
      return json({ data: moduleResource(false) });
    if (url.endsWith("/registry-modules/mod-network/versions"))
      return json({ data: [versionResource("version-1", "1.0.0", "# Root docs")] });
    throw new Error(`Unexpected request: ${url}`);
  }) as unknown as typeof fetch;
  const view = render(
...
test("keeps legacy registry settings URLs as redirects", async () => {
  const modules = render(
    <MemoryRouter initialEntries={["/app/acme/settings/registry-modules"]}>
      <Routes>
        <Route path="/app/:orgName/settings/registry-modules" element={<RegistrySettingsRedirect tab="modules" />} />
        <Route path="/app/:orgName/registry" element={<LocationProbe />} />
      </Routes>
    </MemoryRouter>,
  );
  ...
  const providers = render(
    <MemoryRouter initialEntries={["/app/acme/settings/registry-providers"]}>
      <Routes>
        <Route
          path="/app/:orgName/settings/registry-providers"
          element={<RegistrySettingsRedirect tab="providers" />}
        />
        <Route path="/app/:orgName/registry" element={<LocationProbe />} />
      </Routes>
    </MemoryRouter>,
