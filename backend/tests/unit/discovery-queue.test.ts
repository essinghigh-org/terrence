  ...
  try {
    ...
    requests.push(
      ...Array.from({ length: 32 }, (_, i) =>
        discover(`other-${i}.example`, async () => {
          await gate;
          return "other";
        }),
      ),
    ...
    release();
    await Promise.all(requests.filter((request): request is Promise<string | null> => request !== null));
  }
  await Bun.sleep(0);
...
test("expires queued work without running it and keeps canceled active work counted until it stops", async () => {
  ...
  const active = Array.from({ length: 2 }, () =>
    discover("timeout.example", async (signal) => {
      signals.push(signal);
      await gate; // Deliberately ignore cancellation to check admission safety.
      return true;
    }),
  );
  const queued = discover("timeout.example", async () => {
    queuedStarted = true;
    return true;
  });
  try {
...
test("propagates caller cancellation without allowing a queued slot to start", async () => {
  ...
  let queuedStarted = false;
  const active = Array.from({ length: 2 }, () =>
    discover("Cancel.Example.", async (signal) => {
