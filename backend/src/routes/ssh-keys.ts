...
export const sshKeyRoutes = new Elysia({ name: "sshKeys" })
  .use(authPlugin)
  .get("/api/v2/organizations/:org_name/ssh-keys", async ({ params, user, orgId: tokenOrgId, set }: ParamCtx): Promise<unknown> => {
  ...
  .post("/api/v2/organizations/:org_name/ssh-keys", async ({ params, body, user, orgId: tokenOrgId, set }: ParamCtx): Promise<unknown> => {
    const orgName = params["org_name"] ?? "";
    const org = await db.query.organizations.findFirst({ where: eq(organizations.name, orgName) });
    if (org === undefined || !(await checkOrgPermission(user?.id, org.id, "member", tokenOrgId))) { (set as { status: number }).status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
  ...
  .get("/api/v2/ssh-keys/:ssh_key_id", async ({ params, user, orgId: tokenOrgId, set }: ParamCtx): Promise<unknown> => {
    const sshKeyId = params["ssh_key_id"] ?? "";
    const key = await db.query.sshKeys.findFirst({ where: eq(sshKeys.id, sshKeyId) });
    if (key === undefined || !(await checkOrgPermission(user?.id, key.orgId, "member", tokenOrgId))) { (set as { status: number }).status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
    return { data: { id: key.id, type: "ssh-keys", attributes: { name: key.name } } };
  })
  .patch("/api/v2/ssh-keys/:ssh_key_id", async ({ params, body, user, orgId: tokenOrgId, set }: ParamCtx): Promise<unknown> => {
    const sshKeyId = params["ssh_key_id"] ?? "";
    const key = await db.query.sshKeys.findFirst({ where: eq(sshKeys.id, sshKeyId) });
    if (key === undefined || !(await checkOrgPermission(user?.id, key.orgId, "member", tokenOrgId))) { (set as { status: number }).status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
    const attrs = getAttrs(body);
  ...
  .delete("/api/v2/ssh-keys/:ssh_key_id", async ({ params, user, orgId: tokenOrgId, set }: ParamCtx): Promise<Record<string, never> | { errors: { status: string; title: string }[] }> => {
    const sshKeyId = params["ssh_key_id"] ?? "";
    const key = await db.query.sshKeys.findFirst({ where: eq(sshKeys.id, sshKeyId) });
    if (key === undefined || !(await checkOrgPermission(user?.id, key.orgId, "member", tokenOrgId))) { (set as { status: number }).status = 404; return { errors: [{ status: "404", title: "Not Found" }] }; }
