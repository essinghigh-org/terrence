...
test("restores a saved plan beside a single-root archive and records apply preflight diagnostics", async () => {
  const result = await runWorkerScript(`
    ...
    await writeFile(binaryPath, [
      ...
      'if [ "$1" = plan ]; then echo "Plan: 1 to add, 0 to change, 0 to destroy."; : > tfplan; exit 2; fi',
...
test("ignores a stale client plan bookmark in the uploaded archive during saved-plan apply", async () => {
  const result = await runWorkerScript(`
    ...
    await writeFile(binaryPath, [
      ...
      'if [ "$1" = plan ]; then echo "Plan: 1 to add, 0 to change, 0 to destroy."; echo real-plan-marker > tfplan; exit 2; fi',
      'if [ "$1" = show ]; then printf "{}"; exit 0; fi',
...
test("restores configuration into the working directory without double-nesting", async () => {
  const result = await runWorkerScript(`
    ...
    await writeFile(binaryPath, [
      ...
      'record_dir=' + JSON.stringify(recordDir),
      'if [ "$1" = init ]; then exit 0; fi',
      'if [ "$1" = plan ]; then echo "Plan: 1 to add, 0 to change, 0 to destroy."; echo subdir-marker > tfplan; exit 2; fi',
      'if [ "$1" = show ]; then printf "{}"; exit 0; fi',
