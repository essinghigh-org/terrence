...
async function validateScimToken(
  ...
  const now = Date.now();
  const tokenRows = await db.query.scimTokens.findMany({
    where: inArray(scimTokens.tokenHash, [hash, legacyHash]),
    limit: 2,
  });
  ...
  if (!settings?.enabled || settings.paused === true) {
    (set as { status: number }).status = 401;
    return false;
  }
  ...
  switch (filter.attribute.toLowerCase()) {
    case "id":
      return typeof filter.value === "string"
        ? eq(scimUserIdentities.id, filter.value)
        : { error: "id filter value must be a string" };
    case "username":
      return typeof filter.value === "string"
        ? sql`lower(coalesce(${users.username}, ${scimUserIdentities.username})) = lower(${filter.value})`
        : { error: "userName filter value must be a string" };
    case "externalid":
      return typeof filter.value === "string"
        ? scimCaseInsensitiveEquals(scimUserIdentities.externalId, filter.value)
        : { error: "externalId filter value must be a string" };
    case "emails.value":
      return typeof filter.value === "string"
        ? sql`lower(nullif(${users.email}, '')) = lower(${filter.value})`
        : { error: "emails.value filter value must be a string" };
    case "active":
...
function scimGroupWhere(filter: ScimFilter | undefined): SQL | { error: string } | undefined {
  ...
  switch (filter.attribute.toLowerCase()) {
    case "id":
      return typeof filter.value === "string"
        ? eq(scimGroups.id, filter.value)
        : { error: "id filter value must be a string" };
    case "displayname":
      return typeof filter.value === "string"
        ? scimCaseInsensitiveEquals(scimGroups.name, filter.value)
        : { error: "displayName filter value must be a string" };
    case "externalid":
      return typeof filter.value === "string"
        ? scimCaseInsensitiveEquals(scimGroups.externalId, filter.value)
        : { error: "externalId filter value must be a string" };
    default:
...
function applyUserPatchOperation(updates: ScimPayload, rawOperation: unknown): ScimPatchFailure | null {
  if (rawOperation === null || typeof rawOperation !== "object")
    return { status: 400, detail: "Invalid PATCH operation" };
  ...
  if (path === "") {
    if (operation["value"] === null || typeof operation["value"] !== "object")
      return { status: 400, detail: "PATCH value must be an object" };
...
function resolveUserPatchFields(
  ...
  if (updates["emails"] !== undefined && email === null)
    return { failure: { status: 400, detail: "emails cannot be cleared" } };
    ...
    await db.transaction(async (tx): Promise<void> => {
      ...
      if (active === false) {
        ...
        await tx
          ...
          .where(and(eq(refreshSessions.userId, currentUser.id), isNull(refreshSessions.revokedAt)));
      }
      await tx
        .update(scimUserIdentities)
        .set({
          username: userName,
          ...(externalId !== undefined ? { externalId: typeof externalId === "string" ? externalId : null } : {}),
          updatedAt,
        })
        .where(eq(scimUserIdentities.id, identity.id));
      await tx
        .update(identityLinks)
        .set({ emailAtLinkTime: email })
        .where(
          and(
...
function applyGroupWholeValue(state: GroupPatchState, value: ScimPayload): ScimPatchFailure | null {
  if (typeof value["displayName"] === "string" && value["displayName"].trim() !== "")
    state.name = value["displayName"].trim();
  if (value["externalId"] !== undefined)
    state.externalId = typeof value["externalId"] === "string" ? value["externalId"] : null;
...
function applyGroupMembersPath(state: GroupPatchState, op: string, value: unknown): ScimPatchFailure | null {
  ...
  state.memberIds =
    op === "add"
      ? [...new Set([...state.memberIds, ...ids])]
      : op === "remove"
        ? state.memberIds.filter((id) => !new Set(ids).has(id))
        : ids;
  state.membersChanged = true;
  return null;
}
...
function applyGroupDisplayName(state: GroupPatchState, op: string, value: unknown): ScimPatchFailure | null {
  if (op === "remove" || typeof value !== "string" || value.trim() === "")
    return { status: 400, detail: "displayName is required" };
...
async function parseGroupPatchOperations(
  ...
  const state: GroupPatchState = {
    ...
    memberIds: (
      await db.query.scimGroupMemberships.findMany({ where: eq(scimGroupMemberships.groupId, group.id) })
  ...
  return {
    ...
    members: memberships.map(
      (membership): Record<string, string> => ({
        value: membership.scimUserId,
        display: names.get(membership.scimUserId) ?? membership.scimUserId,
      }),
    ),
...
async function scimGroupResource(group: typeof scimGroups.$inferSelect): Promise<Record<string, unknown>> {
  const memberships = await db.query.scimGroupMemberships.findMany({
    where: eq(scimGroupMemberships.groupId, group.id),
  });
  const identityIds = memberships.map((membership): string => membership.scimUserId);
  const identities =
    identityIds.length === 0
      ? []
      : await db.query.scimUserIdentities.findMany({ where: inArray(scimUserIdentities.id, identityIds) });
  const userIds = identities.map((identity): string => identity.userId);
  const userRows =
    userIds.length === 0
      ? []
      : await db.query.users.findMany({ where: inArray(users.id, userIds), columns: { id: true, username: true } });
  const usernames = new Map(userRows.map((user): [string, string] => [user.id, user.username]));
  const names = new Map(
    identities.map((identity): [string, string] => [identity.id, usernames.get(identity.userId) ?? identity.username]),
  );
  return buildScimGroupResource(group, memberships, names);
}

async function scimGroupResources(
  groups: readonly (typeof scimGroups.$inferSelect)[],
  ...
  return groups.map(
    (group): Record<string, unknown> => buildScimGroupResource(group, membershipsByGroup.get(group.id) ?? [], names),
  );
}
...
function scimEmail(payload: ScimPayload): string | null {
  ...
  const primary = emails.find(
    (email): boolean =>
      email !== null && typeof email === "object" && (email as Record<string, unknown>)["primary"] === true,
  );
  ...
  const replace = async (transactionContext: unknown): Promise<boolean> => {
    ...
    const identities =
      ids.length === 0
        ? []
        : await tx
            .select({ id: scimUserIdentities.id })
    ...
    if (ids.length > 0) {
      await tx.insert(scimGroupMemberships).values(
        ids.map((scimUserId): typeof scimGroupMemberships.$inferInsert => ({
          id: newResourceId("scimmember"),
          groupId,
          scimUserId,
        })),
      );
...
async function reconcileMappedTeams(groupId: string, transaction?: unknown): Promise<void> {
  const reconcile = async (transactionContext: unknown): Promise<void> => {
    ...
    const mappedTeams = await tx.query.teams.findMany({
      where: inArray(
        teams.id,
        mappings.map((mapping): string => mapping.teamId),
      ),
...
async function removeMappedTeamRows(groupIds: readonly string[]): Promise<void> {
  ...
  const mappings = await db.query.teamScimGroupMappings.findMany({
    where: inArray(teamScimGroupMappings.scimGroupId, [...groupIds]),
    columns: { teamId: true },
  });
  if (mappings.length === 0) return;
...
export const scimRoutes = new Elysia({ name: "scim" })
  ...
  .get("/scim/v2/Users", async ({ request, set }: RequestCtx): Promise<unknown> => {
    ...
    const userRowsQuery = db
      .select({ identity: scimUserIdentities, user: users })
      .from(scimUserIdentities)
  ...
  .post("/scim/v2/Users", async ({ request, body, set }: RequestCtx): Promise<unknown> => {
    ...
    try {
      await db.transaction(async (tx): Promise<void> => {
          ...
          if (active !== undefined) {
            ...
            if (active === false) {
              ...
              await tx
                .update(refreshSessions)
                .set({ revokedAt: Date.now() })
                .where(and(eq(refreshSessions.userId, existing.id), isNull(refreshSessions.revokedAt)));
            }
          }
        }
        ...
        if (email !== null) {
          const pending = await tx.query.organizationInvitations.findFirst({
            where: and(
              eq(organizationInvitations.emailNormalized, email.toLowerCase()),
          ...
          if (pending !== undefined) {
            ...
            await tx
              ...
              .set({ status: "accepted", acceptedBy: linkedUserId, updatedAt: Date.now() })
    ...
    if (rejectedExistingAccount)
      return scimError(set, 409, "Existing site administrator accounts cannot be linked through SCIM");
    const identity = await db.query.scimUserIdentities.findFirst({ where: eq(scimUserIdentities.id, scimIdentityId) });
    const created =
      identity === undefined ? undefined : await db.query.users.findFirst({ where: eq(users.id, identity.userId) });
    (set as { status: number }).status = 201;
    return identity === undefined
      ? scimError(set, 500, "User identity was not created")
      : scimUserResource(identity, created);
  })
  .get("/scim/v2/Users/:id", async ({ params, request, set }: RequestCtx): Promise<unknown> => {
    if (!(await validateScimToken(request, set))) return scimError(set, 401, "Unauthorized");
    set.headers["Content-Type"] = "application/scim+json";
    const identity = await db.query.scimUserIdentities.findFirst({
      where: eq(scimUserIdentities.id, params["id"] ?? ""),
    });
  ...
  .delete("/scim/v2/Users/:id", async ({ params, request, set }: RequestCtx): Promise<unknown> => {
    ...
    await db.transaction(async (tx): Promise<void> => {
      ...
      const mappedTeams =
        groupLinks.length === 0
          ? []
          : await tx.query.teamScimGroupMappings.findMany({
              where: inArray(
                teamScimGroupMappings.scimGroupId,
                groupLinks.map((link): string => link.groupId),
              ),
              columns: { teamId: true },
            });
      if (mappedTeams.length > 0) {
        await tx.delete(teamMemberships).where(
          and(
            eq(teamMemberships.userId, identity.userId),
            eq(teamMemberships.ssoSource, "scim"),
            inArray(
              teamMemberships.teamId,
      ...
      await tx
        .update(refreshSessions)
        .set({ revokedAt: Date.now() })
        .where(and(eq(refreshSessions.userId, identity.userId), isNull(refreshSessions.revokedAt)));
      await reconcileScimSiteAdmins(tx);
    });
  ...
  .put("/scim/v2/Users/:id", async ({ params, body, request, set }: RequestCtx): Promise<unknown> => {
    ...
    const updatedIdentity = await db.query.scimUserIdentities.findFirst({
      where: eq(scimUserIdentities.id, identity.id),
    });
    const updatedUser = await db.query.users.findFirst({ where: eq(users.id, identity.userId) });
    return updatedIdentity === undefined
      ? scimError(set, 500, "User identity was not updated")
      : scimUserResource(updatedIdentity, updatedUser);
  })
  .patch("/scim/v2/Users/:id", async ({ params, body, request, set }: RequestCtx): Promise<unknown> => {
    ...
    if (active === null) return scimError(set, 400, "active must be a boolean");
    const persisted = await persistUserPatch(
      identity,
      currentUser,
      fields.userName,
      fields.email,
