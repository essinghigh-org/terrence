...
function checkGeneralSettingsAttrs(
  ...
  if (
    attrs["local-signup-enabled"] !== undefined &&
    attrs["local-signup-enabled"] !== null &&
    typeof attrs["local-signup-enabled"] !== "boolean"
  ) {
        ...
        errors: [
          {
            ...
            detail: "local-signup-enabled must be a boolean or null to use the environment setting",
          },
        ],
      },
    };
  ...
  if (attrs["local-auth-enabled"] !== undefined && typeof attrs["local-auth-enabled"] !== "boolean") {
    ...
    return {
      error: {
        errors: [{ status: "422", title: "Unprocessable Entity", detail: "local-auth-enabled must be a boolean" }],
      },
    };
  }
  if (
    attrs["trusted-client-ip-headers"] !== undefined &&
        ...
        errors: [
          {
            status: "422",
            title: "Unprocessable Entity",
            detail: "trusted-client-ip-headers must be an array of header names (highest priority first)",
          },
        ],
      },
    };
...
async function checkGeneralAuthLockout(
  ...
  const localAuthEnabled =
    typeof attrs["local-auth-enabled"] === "boolean"
      ? attrs["local-auth-enabled"]
      : current["local-auth-enabled"] !== false;
  const [saml, oidc, ldap] = await Promise.all([currentSamlSettings(), getSettings("oidc"), ldapSettings()]);
  const authError = await authLockoutResponse(
    set,
    {
...
export const settingsRoutes = new Elysia({ name: "admin-settings" })
  ...
  .get("/api/v2/admin/data-retention-policy-settings", async ({ user, set }: ParamCtx): Promise<unknown> => {
    ...
    if (values["delete-older-than-n-days"] === null) {
      (set as { status: number }).status = 404;
      return { errors: [{ status: "404", title: "Not Found" }] };
    }
