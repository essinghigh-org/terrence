...
export type AuthAdminProps = Readonly<{
  general: Readonly<{
    loading: boolean;
    saving: boolean;
    error: string | null;
...
function GeneralAuthSection({ section }: Readonly<{ section: AuthAdminProps["general"] }>): React.JSX.Element {
  const {
          ...
          <form onSubmit={handleSaveGeneral} className="space-y-5">
            ...
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input
                ...
                onChange={(e): void => {
                  setLocalAuthEnabled(e.target.checked);
                }}
            ...
            <p className="text-xs text-muted-foreground">
              When disabled, the sign-in page accepts only single sign-on (SAML, OIDC, or LDAP where configured).
              Existing local accounts cannot sign in with a password until this is re-enabled.
            </p>
            <div className="space-y-2">
              <label htmlFor="local-signup-enabled" className="block text-sm font-medium">
                New account registration
              </label>
              <Select
                id="local-signup-enabled"
                name="local-signup-enabled"
                value={localSignup}
                onValueChange={(value: string): void => {
                  if (isLocalSignupMode(value)) setLocalSignup(value);
                }}
              >
              ...
              <p className="text-xs text-muted-foreground">
                Invite only is a good fit for a private lab or small team. Registered users still need organization
                access. Changes take effect immediately after saving.
              </p>
              {localSignup === "environment" && (
                <p className="text-xs text-muted-foreground">
                  Uses TERRENCE_ENABLE_LOCAL_SIGNUP from the server environment.
                </p>
              )}
            </div>
            <div className="space-y-2 pt-1">
              ...
              <Input
                ...
                onChange={(e): void => {
                  setTrustedClientIpHeaders(e.target.value);
                }}
                placeholder="CF-Connecting-IP, X-Forwarded-For"
                className="max-w-md"
              />
              <p className="text-xs text-muted-foreground">
                Comma-separated proxy headers, ordered by priority, used to determine the real client IP for browser
                sessions and rate limiting. Only set this when the app sits behind a trusting reverse proxy (e.g.
                Cloudflare). Leave empty to always trust the direct connection.
              </p>
            </div>
            <Button
              type="submit"
              disabled={
                generalSaving ||
                persistedSamlEnabled === null ||
                persistedOidcEnabled === null ||
                persistedLdapEnabled === null
              }
            >
            ...
            {(persistedSamlEnabled === null || persistedOidcEnabled === null || persistedLdapEnabled === null) && (
              <p className="text-xs text-warning">
                Waiting for all SSO settings to load before saving local authentication.
              </p>
            )}
          </form>
        )}
...
function SamlAuthSection({
  ...
  return (
    <Disclosure
      ...
      meta={
        <span className="text-xs font-normal text-muted-foreground">{samlEnabled ? "Enabled" : "Not enabled"}</span>
      }
      open={samlEnabled || samlError !== null || userOpen}
      onToggle={onToggle}
    >
      {/* SAML SSO */}
        ...
        <CardContent>
          {samlLoading ? (
            ...
            <form onSubmit={handleSaveSaml} className="space-y-5">
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input
                    ...
                    checked={samlEnabled}
                    onChange={(e): void => {
                      setSamlEnabled(e.target.checked);
                    }}
                    className="size-4 accent-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                ...
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input
                    ...
                    onChange={(e): void => {
                      setSamlLinkByEmail(e.target.checked);
                    }}
                    className="size-4 accent-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              ...
              <div className="space-y-2">
                <label htmlFor="saml-sso-url" className="text-sm font-medium text-foreground">
                  SSO Endpoint URL
                </label>
                <Input
                  id="saml-sso-url"
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e): void => {
                    setSamlIdpEntityId(e.target.value);
                  }}
                  aria-label="IdP Entity ID"
                />
              </div>
              <div className="space-y-2">
                <label htmlFor="saml-slo-url" className="text-sm font-medium text-foreground">
                  SLO Endpoint URL
                </label>
                <Input
                  id="saml-slo-url"
              ...
              <div className="border-t pt-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <label htmlFor="saml-attr-username" className="text-sm font-medium text-foreground">
                      Username attribute
                    </label>
                    <Input
                      ...
                      onChange={(e): void => {
                        setSamlAttrUsername(e.target.value);
                      }}
                    />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="saml-attr-groups" className="text-sm font-medium text-foreground">
                      Groups attribute
                    </label>
                    <Input
                      id="saml-attr-groups"
                      ...
                      onChange={(e): void => {
                        setSamlAttrGroups(e.target.value);
                      }}
                    />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="saml-attr-email" className="text-sm font-medium text-foreground">
                      Email attribute
                    </label>
                    <Input
                      id="saml-attr-email"
                      ...
                      value={samlAttrEmail}
                      onChange={(e): void => {
                        setSamlAttrEmail(e.target.value);
                      }}
                      aria-label="SAML email attribute"
                    />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="saml-attr-site-admin" className="text-sm font-medium text-foreground">
                      Site admin attribute
                    </label>
                    <Input
                      ...
                      onChange={(e): void => {
                        setSamlAttrSiteAdmin(e.target.value);
                      }}
                    />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="saml-site-admin-role" className="text-sm font-medium text-foreground">
                      Site admin role value
                    </label>
                    <Input
                      ...
                      onChange={(e): void => {
                        setSamlSiteAdminRole(e.target.value);
                      }}
                    />
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <label htmlFor="saml-timeout" className="text-sm font-medium text-foreground">
                  SSO API token session timeout (seconds)
                </label>
                <Input
                  id="saml-timeout"
                  ...
                  value={samlTimeout}
                  onChange={(e): void => {
                    setSamlTimeout(Number(e.target.value));
                  }}
                />
              </div>
              <div className="border-t pt-4">
                ...
                <dl className="mt-2 space-y-1 text-xs">
                  <div className="flex flex-col gap-0.5">
                    ...
                    <dd className="break-all text-muted-foreground font-mono">
...
function OidcAuthSection({
  section,
  userOpen,
  onToggle,
}: Readonly<{
  section: AuthAdminProps["oidc"];
  userOpen: boolean;
  onToggle: (open: boolean) => void;
}>): React.JSX.Element {
  const {
  ...
  return (
    <Disclosure
      ...
      meta={
        <span className="text-xs font-normal text-muted-foreground">{oidcEnabled ? "Enabled" : "Not enabled"}</span>
      }
      open={oidcEnabled || oidcError !== null || userOpen}
      onToggle={onToggle}
    >
      {/* OIDC */}
      <Card className="border-0 border-t rounded-t-none shadow-none">
            ...
            <form onSubmit={handleSaveOidc} className="space-y-5">
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  checked={oidcEnabled}
                  onChange={(e): void => {
                    setOidcEnabled(e.target.checked);
                  }}
                  className="size-4 accent-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={(e): void => {
                    setOidcLinkByEmail(e.target.checked);
                  }}
                  className="size-4 accent-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={oidcScopes}
                    onChange={(e): void => {
                      setOidcScopes(e.target.value);
                    }}
                  />
                </div>
                <div className="space-y-2">
                  <label htmlFor="oidc-pkce-method" className="text-sm font-medium text-foreground">
                    PKCE Method
                  </label>
                  <Input
                    id="oidc-pkce-method"
                    ...
                    onChange={(e): void => {
                      setOidcPkceMethod(e.target.value);
                    }}
                  />
                </div>
                <div className="space-y-2">
                  <label htmlFor="oidc-signing-alg" className="text-sm font-medium text-foreground">
                  ...
                  <Select
                    ...
                    onChange={(e): void => {
                      setOidcSigningAlg(e.target.value);
                    }}
...
function LdapAuthSection({
  section,
  userOpen,
  onToggle,
}: Readonly<{
  section: AuthAdminProps["ldap"];
  userOpen: boolean;
  onToggle: (open: boolean) => void;
}>): React.JSX.Element {
  const {
  ...
  return (
    <Disclosure
      ...
      meta={
        <span className="text-xs font-normal text-muted-foreground">{ldapEnabled ? "Enabled" : "Not enabled"}</span>
      }
      open={ldapEnabled || ldapError !== null || userOpen}
      onToggle={onToggle}
    >
      {/* LDAP */}
      <Card className="border-0 border-t rounded-t-none shadow-none">
            ...
            <form onSubmit={handleSaveLdap} className="space-y-5">
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  checked={ldapEnabled}
                  onChange={(e): void => {
                    setLdapEnabled(e.target.checked);
                  }}
                  className="size-4 accent-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={(e): void => {
                    setLdapLinkByEmail(e.target.checked);
                  }}
                  className="size-4 accent-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <div className="space-y-2">
                  <label htmlFor="ldap-host" className="text-sm font-medium text-foreground">
                    Host
                  </label>
                  <Input
                    id="ldap-host"
                    ...
                    value={ldapHost}
                    onChange={(e): void => {
                      setLdapHost(e.target.value);
                    }}
                    aria-label="LDAP host"
                  />
                </div>
                <div className="space-y-2">
                  <label htmlFor="ldap-port" className="text-sm font-medium text-foreground">
                    Port
                  </label>
                  <Input
                    id="ldap-port"
                    ...
                    value={ldapPort}
                    onChange={(e): void => {
                      setLdapPort(Number(e.target.value));
                    }}
                    aria-label="LDAP port"
                  />
                </div>
                <div className="space-y-2">
                  <label htmlFor="ldap-encryption" className="text-sm font-medium text-foreground">
                    Encryption
                  </label>
                  <Select
                    id="ldap-encryption"
                    ...
                    onChange={(e): void => {
                      setLdapEncryption(e.target.value);
                    }}
                  ...
                  {ldapEncryption === "plain" && (
                    <p className="text-xs text-warning">
                      Warning: plain LDAP transmits the bind password and user passwords without encryption. Use
                      StartTLS or LDAPS when possible.
                    </p>
                  )}
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <label htmlFor="ldap-bind-dn" className="text-sm font-medium text-foreground">
                    Bind DN (service account, optional)
                  </label>
                  <Input
                    id="ldap-bind-dn"
                    name="ldap-bind-dn"
              ...
              <div className="border-t pt-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <label htmlFor="ldap-user-filter" className="text-sm font-medium text-foreground">
                      User filter (containing &#123;&#123;username&#125;&#125;)
                    </label>
                    <Input
                      id="ldap-user-filter"
                      ...
                      onChange={(e): void => {
                        setLdapUserFilter(e.target.value);
                      }}
                      aria-label="LDAP user filter"
                    />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="ldap-attr-username" className="text-sm font-medium text-foreground">
                      Username attribute
                    </label>
                    <Input
                      ...
                      onChange={(e): void => {
                        setLdapAttrUsername(e.target.value);
                      }}
                      aria-label="LDAP username attribute"
                    />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="ldap-attr-email" className="text-sm font-medium text-foreground">
                      Email attribute
                    </label>
                    <Input
                      id="ldap-attr-email"
                      ...
                      value={ldapAttrEmail}
                      onChange={(e): void => {
                        setLdapAttrEmail(e.target.value);
                      }}
                      aria-label="LDAP email attribute"
                    />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="ldap-attr-display-name" className="text-sm font-medium text-foreground">
                    ...
                    <Input
                      ...
                      onChange={(e): void => {
                        setLdapAttrDisplayName(e.target.value);
                      }}
                ...
                <p className="mt-3 text-xs text-muted-foreground">
                  The sign-in form first attempts LDAP, then falls back to local passwords (when enabled). A user who
                  already exists locally with the same username will block LDAP provisioning to avoid account takeover.
                </p>
              </div>
...
export function AuthAdmin(props: AuthAdminProps): React.JSX.Element {
  const { general, saml, oidc, ldap } = props;
  ...
  return (
    <div className="space-y-6">
      <GeneralAuthSection section={general} />
      <SamlAuthSection section={saml} userOpen={samlUserOpen} onToggle={setSamlUserOpen} />
      <OidcAuthSection section={oidc} userOpen={oidcUserOpen} onToggle={setOidcUserOpen} />
      <LdapAuthSection section={ldap} userOpen={ldapUserOpen} onToggle={setLdapUserOpen} />
    </div>
  );
}
