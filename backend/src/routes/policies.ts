...
// Full the reference format policy-set relationship shape. The provider's workspace/project
...
async function policySetRelationships(policySet: PsItem): Promise<Record<string, unknown>> {
  const [workspaceLinks, projLinks, exclLinks, projExclLinks, policyRows] = await Promise.all([
    ...
    db.query.policySetProjectExclusions.findMany({ where: eq(policySetProjectExclusions.policySetId, policySet.id) }),
...
function policyEnforcementLevels(kind: string): readonly string[] {
  return kind === "opa" ? ["advisory", "mandatory"] : ["advisory", "soft-mandatory", "hard-mandatory"];
}
...
async function policyResource(pol: PolicyRow, orgName: string | null): Promise<Record<string, unknown>> {
  return {
    ...
    attributes: {
      ...
      kind: pol.kind,
      policy: pol.source ?? "",
      ...(pol.kind === "opa" ? { query: pol.query ?? "data" } : {}),
      "enforcement-level": pol.enforcementLevel ?? "soft-mandatory",
      // `enforce` is deprecated in the reference format; go-tfe still decodes it into a
...
// go-tfe's PolicySetParameter carries a pointer "policy-set" relationship;
// omitting it makes the provider nil-deref in modelFromTFEPolicySetParameter.
function policySetParameterResource(p: ParamItem, policySetId: string): Record<string, unknown> {
  return {
    id: p.id,
    type: "vars",
    attributes: { key: p.key, value: p.sensitive === true ? null : p.value, sensitive: p.sensitive, hcl: p.hcl },
    relationships: { configurable: { data: { id: policySetId, type: "policy-sets" } } },
  };
}
