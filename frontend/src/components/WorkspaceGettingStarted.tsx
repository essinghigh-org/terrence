...
function ReadinessIcon({ state }: Readonly<{ state: ReadinessState }>): React.JSX.Element {
  if (state === "checking")
    return <Loader2 className="mt-0.5 size-4 animate-spin text-muted-foreground" aria-hidden="true" />;
...
function configurationCheck(
  ...
  const ready = localExecution || hasRepository || source === "local";
  return {
    id: "configuration",
    label: "Configuration source",
    state: ready ? "ready" : "missing",
    detail: ready ? "A configuration source is connected." : "Connect a repository or configure the CLI backend.",
    href: ready ? undefined : `${workspacePath}/settings/version-control`,
  };
}

function engineCheck(engine: string, workspacePath: string): ReadinessCheck {
  const missing = engine.trim() === "";
  return {
    id: "engine",
    label: "Engine",
    state: missing ? "missing" : "ready",
    detail: missing
      ? "Select Terraform or OpenTofu in workspace settings."
      : `${engine === "tofu" ? "OpenTofu" : "Terraform"} is configured; the first plan verifies its availability.`,
    href: missing ? `${workspacePath}/settings` : undefined,
  };
}

function inputsCheck(inputState: ReadinessState, canReadVariable: boolean, workspacePath: string): ReadinessCheck {
  return {
    id: "inputs",
    ...
    detail:
      inputState === "ready"
        ? "Variable access is configured; code-specific requirements are verified by a plan."
        : inputState === "missing"
          ? "No workspace variables are configured yet; check the configuration requirements."
          : inputState === "checking"
            ? "Checking whether a variable source is available. Secret values are never fetched for this check."
            : "Unable to verify inputs for this user. Review variables before queueing a run.",
...
function targetCheck(
  ...
  const targetState: ReadinessState =
    effectiveExecutionMode === "agent" && agentPoolConfigured === undefined
      ? "unknown"
      : targetReady
        ? "ready"
        : "missing";
  const executionDetail =
    effectiveExecutionMode === "agent"
      ? agentPoolConfigured === true
        ? "An agent pool is assigned; the first plan verifies that an agent is online."
        : "Assign an agent pool before queueing a run."
      : localExecution
        ? "The CLI is the execution target for this workspace."
        : "The Terrence worker is the execution target.";
  return {
    id: "target",
...
function credentialsCheck(
  ...
  return {
    ...
    detail: ready
      ? localExecution
        ? "Credentials stay with the CLI session."
        : hasRepository
          ? "The connected repository supplies the configuration source."
          : "The server workspace mount supplies the configuration source."
      : "Connect a source before credentials can be used.",
    href: ready ? undefined : `${workspacePath}/settings/version-control`,
  };
}
...
function sandboxCheck(
  localExecution: boolean,
  effectiveExecutionMode: string,
  sandboxState: ReadinessState,
): ReadinessCheck {
  return {
    id: "sandbox",
    label: "Run capability",
    state: localExecution || effectiveExecutionMode === "agent" ? "unknown" : sandboxState,
    detail: localExecution
      ? "Local execution does not depend on the server sandbox."
      : effectiveExecutionMode === "agent"
        ? "Agent availability is verified when the assigned agent reports readiness."
        : sandboxState === "missing"
          ? "The remote run sandbox is unavailable; remote runs will be blocked."
          : sandboxState === "ready"
            ? "The remote run sandbox is available."
            : sandboxState === "checking"
              ? "Checking remote run capability."
              : "Unable to verify remote run capability.",
    href: sandboxState === "missing" ? "/app/docs/reverse-proxy" : undefined,
  };
}

function permissionCheck(canQueueRun: boolean): ReadinessCheck {
  return {
    id: "permission",
    label: "Permission to plan",
    state: canQueueRun ? "ready" : "missing",
    detail: canQueueRun
      ? "You can queue a plan from this workspace."
      : "You can review readiness, but an administrator must grant plan permission.",
  };
}

type ReadinessStep = Readonly<{ label: string; ready: boolean }>;
...
function readinessSteps(
  ...
  return [
    {
      label: localExecution
        ? "Local execution is configured"
        : hasRepository
          ? "Repository is connected"
          : "Configuration connection is still needed",
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
        {compact
          ? "The checks below stay available after a successful workflow so a later run can be diagnosed before it starts."
          : localExecution
            ? "Connect your CLI to store state in Terrence while running plans on your computer."
            : hasRepository
              ? "Your repository is connected. Add any required variables, then preview the changes in your code."
              : source === "local"
                ? "Mount your configuration directory on the Terrence server, then add any variables your code needs."
      ...
      {isLocked && (
        <p className="mt-2 text-sm font-medium text-warning-text">
          This workspace is locked. Unlock it before starting a run.
        </p>
      )}
    </div>
  );
  ...
  return (
    <section aria-labelledby="workspace-readiness-heading" className="rounded-md border border-border bg-muted/20 p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h4 id="workspace-readiness-heading" className="text-sm font-semibold">
          Workspace readiness
        </h4>
        <span
          className={readinessComplete ? "text-xs font-medium text-success" : "text-xs font-medium text-warning-text"}
        >
...
function ReadinessCheckList({ checks }: Readonly<{ checks: readonly ReadinessCheck[] }>): React.JSX.Element {
  return (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1">
                ...
                <span
                  className={`text-xs font-medium ${check.state === "ready" ? "text-success-text" : check.state === "missing" ? "text-warning-text" : "text-muted-foreground"}`}
                >
    ...
    <>
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <Button size="icon-sm" variant="ghost" aria-label="Copy backend configuration" onClick={onCopy}>
      ...
      <div className="space-y-2">
        ...
        <pre className="overflow-x-auto rounded-md bg-muted/60 p-4 text-xs leading-relaxed">
          <code>{`${cli} login ${hostname}\n${cli} init\n${cli} plan`}</code>
        </pre>
        {window.location.protocol !== "https:" && (
          <p className="text-sm text-muted-foreground">
            CLI login requires HTTPS.{" "}
            <Link className="underline underline-offset-4" to="/app/docs/reverse-proxy">
              Set up HTTPS
            </Link>{" "}
            and open Terrence at that address before copying the settings.
          </p>
        )}
        <p className="text-xs text-muted-foreground">
          {localExecution
            ? "Plans execute on your computer. Terrence stores the state."
            : "The run appears here when the CLI uploads your configuration."}
        </p>
      </div>
    </>
  );
...
function QuickActions({
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      {usesServerCode && canQueueRun && !isLocked && (
        <Link className={buttonVariants({ size: "sm" })} to={`${workspacePath}/runs?new-run=true`}>
          Start first plan
        </Link>
      )}
      {canReadVariable && (
        <Link className={buttonVariants({ variant: "outline", size: "sm" })} to={`${workspacePath}/variables`}>
          Configure variables
        </Link>
      )}
      {!hasRepository && canUpdate && (
        <Link
          className="text-sm font-medium text-primary hover:underline"
          to={`${workspacePath}/settings/version-control`}
        >
          Connect a Git repository
        </Link>
      )}
      <Link className="text-sm font-medium text-primary hover:underline" to="/app/docs/quickstart">
        Quick start guide
      </Link>
    </div>
  );
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Check configuration, inputs, engine, execution capacity, and storage before the first plan.
          </p>
        </div>
        <Button
          size="sm"
            ...
            return (
              <div key={`${id}-${index}`} className="flex items-start justify-between gap-3 text-xs">
                ...
                <span className="text-right text-muted-foreground">
                  <span className="font-medium text-foreground">{status}</span> — {detail}
                </span>
              </div>
            );
          })}
          {preflight.attributes?.["can-run-anyway"] === true &&
...
function preflightViewState(preflight: PreflightData | null): Readonly<{
  checks: PreflightCheck[];
  status: string;
}> {
  const checks = Array.isArray(preflight?.attributes?.checks) ? (preflight.attributes.checks as PreflightCheck[]) : [];
  const status = typeof preflight?.attributes?.status === "string" ? preflight.attributes.status : "unknown";
...
export function WorkspaceGettingStarted({
  ...
  const readiness = readinessSteps(localExecution, hasRepository, source, canReadVariable, canQueueRun);
  ...
  const handleCopyConfiguration = (): void => {
    void copyTextToClipboard(configuration).then((copied): void => {
      toast.add({
        title: copied ? "Backend configuration copied" : "Could not copy. Select and copy the configuration below.",
  ...
  const preflightView = preflightViewState(preflight);
  const preflightChecks = preflightView.checks;
  const preflightStatus = preflightView.status;
  ...
  useEffect((): (() => void) => {
    ...
    if (canReadVariable) {
      void fetchApi<{ data?: unknown[] }>(`/workspaces/${encodeURIComponent(workspaceId)}/vars?page[size]=1`, {
  ...
  const checks = useMemo(
    (): readonly ReadinessCheck[] => [
      configurationCheck(localExecution, hasRepository, source, workspacePath),
      engineCheck(engine, workspacePath),
      inputsCheck(inputState, canReadVariable, workspacePath),
      targetCheck(effectiveExecutionMode, agentPoolConfigured, localExecution, source, workspacePath),
      credentialsCheck(localExecution, hasRepository, source, workspacePath),
      sandboxCheck(localExecution, effectiveExecutionMode, sandboxState),
      permissionCheck(canQueueRun),
    ],
    [
      agentPoolConfigured,
      canQueueRun,
  ...
  return (
    <div className="space-y-5">
      <GettingStartedIntro
        ...
        isLocked={isLocked}
      />
      <ReadinessSummary readiness={readiness} readinessComplete={readinessComplete} />
