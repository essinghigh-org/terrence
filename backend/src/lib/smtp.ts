...
function validateEmailRequest(settings: SmtpSettings, message: EmailMessage): void {
...
function smtpAuthMode(settings: SmtpSettings): string {
...
async function sendSmtpMessage(settings: SmtpSettings, message: EmailMessage, session: Session, step: SmtpStep): Promise<void> {
  ...
  const terminated = composeSmtpMessage(settings, message);
