...
export const versionsRoutes = new Elysia({ name: "admin-versions" })
  ...
  .get("/api/v2/admin/terraform-versions", async ({ user, request, set }: ParamCtx): Promise<unknown> => {
    ...
    const [versions, countRows] = await Promise.all([
      db.query.adminTerraformVersions.findMany({
        orderBy: [desc(adminTerraformVersions.createdAt)],
        limit: size,
    ...
    return {
      data: versions.map((v: VerItem): Record<string, unknown> => versionResource(v, "terraform-versions")),
  ...
  .post("/api/v2/admin/terraform-versions", async ({ body, user, set }: ParamCtx): Promise<unknown> => {
    ...
    if (version === "") {
      ...
      return { errors: [{ status: "422", title: "Unprocessable Entity", detail: "Version is required" }] };
    }
    ...
    await db
      .insert(adminTerraformVersions)
      .values({ id, version, url, sha, deprecated, enabled, isDefault, createdAt: Date.now() });
  ...
  .patch(
    "/api/v2/admin/terraform-versions/:version_id",
    async ({ params, body, user, set }: ParamCtx): Promise<unknown> => {
      ...
      if (Object.keys(updates).length > 0)
        await db.update(adminTerraformVersions).set(updates).where(eq(adminTerraformVersions.id, versionId));
      const updated = await db.query.adminTerraformVersions.findFirst({
        where: eq(adminTerraformVersions.id, versionId),
      });
  ...
  .get("/api/v2/admin/sentinel-versions", async ({ user, request, set }: ParamCtx): Promise<unknown> => {
    ...
    const { number, size } = pageRequest(request);
    const [versions, countRows] = await Promise.all([
      db.query.adminSentinelVersions.findMany({
        orderBy: [desc(adminSentinelVersions.createdAt)],
        limit: size,
        offset: (number - 1) * size,
      }),
    ...
    const totalCount = countRows[0]?.total ?? 0;
    return {
      data: versions.map((v: VerItem): Record<string, unknown> => versionResource(v, "sentinel-versions")),
      ...pagination(request, number, size, totalCount),
    };
  })
  ...
  .patch(
    "/api/v2/admin/sentinel-versions/:version_id",
    async ({ params, body, user, set }: ParamCtx): Promise<unknown> => {
      const versionId = params["version_id"] ?? "";
      if (user?.isSiteAdmin !== true) {
        (set as { status: number }).status = 404;
      ...
      fillVersionUpdates(attrs, updates);
      if (Object.keys(updates).length > 0)
        await db.update(adminSentinelVersions).set(updates).where(eq(adminSentinelVersions.id, versionId));
      const updated = await db.query.adminSentinelVersions.findFirst({
        where: eq(adminSentinelVersions.id, versionId),
      });
      if (updated === undefined) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
  ...
  .get("/api/v2/admin/opa-versions", async ({ user, request, set }: ParamCtx): Promise<unknown> => {
    ...
    const { number, size } = pageRequest(request);
    const [versions, countRows] = await Promise.all([
      db.query.adminOpaVersions.findMany({
        orderBy: [desc(adminOpaVersions.createdAt)],
        limit: size,
        offset: (number - 1) * size,
      }),
    ...
    const totalCount = countRows[0]?.total ?? 0;
    return {
      data: versions.map((v: VerItem): Record<string, unknown> => versionResource(v, "opa-versions")),
      ...pagination(request, number, size, totalCount),
    };
  })
  ...
  .patch("/api/v2/admin/opa-versions/:version_id", async ({ params, body, user, set }: ParamCtx): Promise<unknown> => {
    ...
    fillVersionUpdates(attrs, updates);
    if (Object.keys(updates).length > 0)
      await db.update(adminOpaVersions).set(updates).where(eq(adminOpaVersions.id, versionId));
  ...
  .delete(
    "/api/v2/admin/opa-versions/:version_id",
    async ({
      params,
      user,
      set,
