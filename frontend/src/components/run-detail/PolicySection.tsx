...
function PolicyCheckRow({ check }: Readonly<{ check: PolicyCheck }>): React.JSX.Element {
  return (
    <TableRow>
      <TableCell>
        <div className="text-sm font-medium">{check.attributes["policy-name"] ?? check.id}</div>
        {check.attributes["policy-name"] !== null && check.attributes["policy-name"] !== undefined && (
      ...
      <TableCell>
        ...
        >
          {isAdvisoryPolicyIssue(check)
            ? `advisory ${check.attributes.status === "failed" ? "failed" : check.attributes.status.replace(/_/g, " ")}`
    ...
    <section aria-labelledby="policy-heading" className="overflow-hidden rounded-lg border border-border bg-card">
      <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <h3 id="policy-heading" className="font-semibold text-foreground">
            Policy check
          </h3>
        </div>
      ...
      {policyChecks.length > 0 && (
        <div className="border-t border-border px-5 py-3">
          <Table>
            ...
            <TableBody>
              {policyChecks.map(
                (check: PolicyCheck): React.JSX.Element => (
