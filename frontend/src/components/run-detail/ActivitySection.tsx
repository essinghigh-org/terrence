...
function ActivityEventRow({ event }: Readonly<{ event: RunEvent }>): React.JSX.Element {
  ...
  const eventLabel =
    (RUN_EVENT_LABELS as Readonly<Record<string, string | undefined>>)[event.attributes.action] ??
      ...
      <div className="min-w-0 flex-1 text-sm">
        <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
          <p className="text-foreground/85">
            <span className="font-semibold text-foreground">{actor}</span> {eventLabel}
          </p>
          <time className="text-xs text-muted-foreground" dateTime={event.attributes["created-at"]}>
...
export function CommentForm({
  ...
  return (
    ...
    >
      <label htmlFor="run-comment" className="mb-2 block text-sm font-medium text-foreground">
        Add a comment
      </label>
      <Textarea
        id="run-comment"
      ...
      <div className="mt-2 flex justify-end">
        <Button type="submit" disabled={commentBody.trim() === "" || pendingAction !== ""}>
          Add comment
        </Button>
      </div>
    </form>
...
export function ActivitySection(props: ActivitySectionProps): React.JSX.Element {
  ...
  const showCombinedEmptyActivity = TERMINAL_STATUSES.has(status) && runEvents.length === 0 && comments.length === 0;
    ...
    return (
      <section aria-labelledby="activity-heading" className="rounded-lg border border-border bg-card">
        <div className="flex items-center gap-3 border-b border-border px-5 py-4">
          ...
          <h3 id="activity-heading" className="font-semibold text-foreground">
            Activity &amp; comments
          </h3>
  ...
  return (
    <>
      <section aria-labelledby="activity-heading" className="rounded-lg border border-border bg-card">
        ...
        {runEvents.length === 0 ? (
          ...
          <ol className="divide-y divide-border/60">
            {runEvents.map(
              (event: RunEvent): React.JSX.Element => (
      ...
      <section aria-labelledby="comments-heading" className="rounded-lg border border-border bg-card">
        <div className="flex items-center gap-3 border-b border-border px-5 py-4">
          <MessageSquare className="size-5 text-muted-foreground/70" aria-hidden="true" />
          <h3 id="comments-heading" className="font-semibold text-foreground">
            Comments
          </h3>
        ...
        <div className="divide-y divide-border/60">
          {comments.length === 0 ? (
            ...
            comments.map((comment: RunComment): React.JSX.Element => <CommentRow key={comment.id} comment={comment} />)
          )}
        </div>
        {commentForm}
      </section>
