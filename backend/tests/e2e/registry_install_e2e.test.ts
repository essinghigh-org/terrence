...
if (!["floor", "current", "canary"].includes(MATRIX_TIER))
...
if (E2E_CLI_FILTER !== null && !["terraform", "tofu"].includes(E2E_CLI_FILTER))
  throw new Error("Unsupported TERRENCE_E2E_CLI value");
...
async function startTlsProxy(
  backendPort: number,
  workDir: string,
): Promise<{ server: Awaited<ReturnType<typeof Bun.serve>>; port: number; certPath: string }> {
  const port = await freeOperationalTestPort();
 ...
 */
async function tryStartPortlessProxy(
  backendPort: number,
  workDir: string,
): Promise<{ server: Awaited<ReturnType<typeof Bun.serve>> } | null> {
  const cert = await Bun.file(join(workDir, "cert.pem")).arrayBuffer();
  const key = await Bun.file(join(workDir, "key.pem")).arrayBuffer();
  try {
    const server = Bun.serve({
...
async function startFixtureOrigin(
  workDir: string,
  files: Readonly<Record<string, Uint8Array>>,
): Promise<{ server: Awaited<ReturnType<typeof Bun.serve>>; port: number }> {
  const port = await freeOperationalTestPort();
  const cert = await Bun.file(join(workDir, "cert.pem")).arrayBuffer();
  const key = await Bun.file(join(workDir, "key.pem")).arrayBuffer();
  const server = Bun.serve({
...
async function ensureCli(tool: "terraform" | "tofu"): Promise<string> {
  ...
  const version =
    (cliMatrix as Record<string, Record<string, string>>)[tool]?.[MATRIX_TIER === "floor" ? "floor" : "current"] ??
    "latest";
...
describe("genuine registry installation", () => {
  ...
  test.each([["terraform"], ["tofu"]])(
    "%s installs the latest module version through discovery",
    async (tool) => {
  ...
  test.each([["terraform"], ["tofu"]])(
    "%s reports a missing version as a version error, not a discovery failure",
    async (tool) => {
      const target = clis.find((c): boolean => c.tool === tool);
      if (target === undefined) return;
  ...
  test("anonymous clients cannot read the private registry surface", async () => {
    ...
    const versions = await fetch(
      `http://127.0.0.1:${backend.port}/api/registry/v1/modules/${orgName}/${moduleName}/${moduleProvider}/versions`,
    );
    expect(versions.status).toBe(404);
    const authed = await api(
      backend.port,
      "GET",
      `/api/registry/v1/modules/${orgName}/${moduleName}/${moduleProvider}/versions`,
      undefined,
      token,
    );
  ...
  async function publishProviderFixture(): Promise<void> {
    ...
    expect(created.status).toBe(201);
    for (const [version, sum] of [
      ["1.0.0", providerShasum],
      ["2.0.0", "0".repeat(64)],
    ] as const) {
      const ver = await api(
        backend.port,
        "POST",
        `/api/v2/organizations/${orgName}/registry-providers/private/${orgName}/${providerType}/versions`,
        {
      ...
      const plat = await api(
        backend.port,
        "POST",
        `/api/v2/organizations/${orgName}/registry-providers/private/${orgName}/${providerType}/versions/${version}/platforms`,
        {
          data: {
  ...
  test("network mirror metadata carries versions and hashes behind auth", async () => {
    ...
    if (proxy !== undefined) {
      const viaProxy = await fetch(
        `https://127.0.0.1:${proxy.port}/api/registry/v1/provider-mirror/127.0.0.1:${proxy.port}/${orgName}/${providerType}/index.json`,
        {
  ...
  test.each([["terraform"], ["tofu"]])(
    "%s installs a provider through the network mirror and verifies its checksum",
    async (tool) => {
      const target = clis.find((c): boolean => c.tool === tool);
      ...
      expect(installedOutput).not.toMatch(
        /checksum|hash|mismatch|signature|integrity|401|403|404|refused|certificate|unknown authority/i,
      );
