...
describe("persisted configuration contract", (): void => {
  test("validates maintenance clocks, timezones, and webhook URLs", (): void => {
    ...
    for (const invalid of [
      { ...window, timezone: "private-marker" },
      { ...window, days: [7] },
      { ...window, "start-time": "25:00" },
    ]) {
      throws(
        (): void => {
          validateSettings("maintenance-windows", { windows: [invalid] });
        },
        (error: unknown): boolean =>
          error instanceof SettingsValidationError && !error.message.includes("private-marker"),
      );
    }
    throws((): void => {
      validateSettings("approval-webhook", { url: "https://user:private-marker@example.com" });
    }, SettingsValidationError);
    validateSettings("approval-webhook", { url: "https://example.com/hook?token=value" });
    throws((): void => {
      validateSettings("plan-explainer", { "reasoning-effort": "hgh" });
    }, SettingsValidationError);
    throws((): void => {
      validateSettings("logging", { error: "caller metadata" });
    }, SettingsValidationError);
  });
  test("rejects incomplete or contradictory enabled authentication", (): void => {
    for (const values of [
      ...
      throws((): void => {
        validateSettings("oidc", values, true);
      }, SettingsValidationError);
    validateSettings("oidc", {
      enabled: true,
      issuer: "https://example.com",
      "client-id": "client",
      "pkce-method": "S256",
  ...
  test("validates SMTP enums and ports and redacts report strings", (): void => {
    for (const values of [
      ...
      throws(
        (): void => {
          validateSettings("smtp", values, true);
        },
        (error: unknown): boolean =>
          error instanceof SettingsValidationError && !error.message.includes("private-marker"),
      );
  ...
  test("rejects invalid types, ranges, names, and header syntax", (): void => {
    for (const values of [
      ...
      { "plan-timeout": -1 },
      { "apply-timeout": 1.5 },
      { "api-rate-limit": Infinity },
      { "local-auth-enabled": "false" },
      { "local-signup-enabled": "private-marker" },
      { "private-marker": true },
      ...
      throws(
        (): void => {
          validateSettings("general", values, true);
        },
        (error: unknown): boolean => {
          return (
  ...
  test("distinguishes broken stored configuration from rejected input", (): void => {
    throws(
      (): void => {
        validateSettings("general", { "plan-timeout": -1 });
      },
      (error: unknown): boolean => {
    ...
    expect((): void => {
      validateSettings("retention", { "delete-older-than-n-days": 7 });
    }).not.toThrow();
  });
});
