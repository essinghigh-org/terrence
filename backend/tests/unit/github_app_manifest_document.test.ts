...
test("manifest is JSON in a POST field, never an action query parameter", () => {
  ...
  const manifest = { name: "Terrence", public: false, hook_attributes: { url: "https://terrence.test/api/webhooks/github" } };
...
test("registration explicitly distinguishes personal and organization ownership", () => {
  ...
  assert.equal(githubAppRegistrationUrl("https://github.com", " essinghigh-org ").pathname, "/organizations/essinghigh-org/settings/apps/new");
  assert.equal(githubAppRegistrationUrl("https://github.enterprise.test:8443", "team").origin, "https://github.enterprise.test:8443");
});

test("organization input cannot change the registration origin or inject a path", () => {
  for (const owner of ["https://github.com/team", "../settings", "team/another", "x?state=bad", "-team", "team-", "a".repeat(40)]) {
...
test("manifest handoff rejects unsafe destinations and missing state", () => {
  for (const action of [
    ...
    "https://github.com/settings/apps/new", "https://github.com/login?state=a",
...
test("database credentials, not an import label, establish environment removal safety", () => {
  assert.deepEqual(githubAppCredentialStorage({ status: "active", configuration }, true), {
    "credential-storage": "database", "environment-removable": true,
  });
  assert.deepEqual(githubAppCredentialStorage(null, true), {
    "credential-storage": "environment", "environment-removable": false,
  });
  assert.deepEqual(githubAppCredentialStorage(null, false), {
    "credential-storage": "none", "environment-removable": false,
  });
});
...
test("disconnect tombstone does not imply environment fallback", () => {
  assert.equal(githubAppCredentialStorage({ status: "disconnected", configuration: null }, true)["credential-storage"], "none");
});
...
test("settings links use the validated account type", () => {
  assert.equal(githubAppSettingsUrl("https://github.com", "terrence", "essinghigh-org", "Organization"), "https://github.com/organizations/essinghigh-org/settings/apps/terrence");
  assert.equal(githubAppSettingsUrl("https://github.com", "terrence", "user", "User"), "https://github.com/settings/apps/terrence");
  assert.equal(githubAppSettingsUrl("https://github.com", "terrence", "unknown", undefined), "https://github.com/apps/terrence");
});
