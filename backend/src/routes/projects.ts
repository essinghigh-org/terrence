...
function resolveExecutionMode(
  ...
  const defaultExecutionMode =
    rawMode === undefined ? (existing?.defaultExecutionMode ?? "remote") : typeof rawMode === "string" ? rawMode : "";
...
function resolveAutoDestroyDuration(
  ...
  const autoDestroyActivityDuration =
    rawDuration === undefined ? (existing?.autoDestroyActivityDuration ?? null) : rawDuration;
  if (autoDestroyActivityDuration !== null && !isAutoDestroyDuration(autoDestroyActivityDuration)) {
...
async function resolveAgentPoolId(
  ...
  const defaultAgentPoolId = parsedPool.provided
    ...
    : defaultExecutionMode === "agent"
      ? (existing?.defaultAgentPoolId ?? null)
      : null;
...
async function insertProject(
  newProj: typeof projects.$inferInsert,
  set: SetObj,
): Promise<{ created: typeof projects.$inferSelect } | { error: unknown }> {
  try {
    ...
    return {
      error: {
        errors: [
          { status: "409", title: "Conflict", detail: "A project with this name already exists in the organization" },
        ],
      },
    };
  }
...
export const projectRoutes = new Elysia({ name: "projects" })
  .use(authPlugin)
  .get(
    "/api/v2/organizations/:org_name/projects",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, request, set }: ParamCtx): Promise<unknown> => {
      ...
      const nameFilter = new URL(request.url).searchParams
        .getAll("filter[names]")
      ...
      return {
        data: await Promise.all(
          projList.map(async (project): Promise<Record<string, unknown>> => {
  ...
  .post(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (
        org === undefined ||
        !(await checkOrganizationPermission(org.id, user?.id, tokenOrgId, tokenTeamId ?? null, "manage-projects"))
      ) {
        (set as { status: number }).status = 404;
        return { errors: [{ status: "404", title: "Not Found" }] };
      }
      ...
      const description =
        typeof fields.attributes["description"] === "string" ? fields.attributes["description"] : null;
  ...
  .patch(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      const updatedCounts = (await countsByProject([projectId])).get(projectId) ?? {
  ...
  .delete(
    ...
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (workspace !== undefined) {
        (set as { status: number }).status = 409;
        return {
          errors: [{ status: "409", title: "Conflict", detail: "Project must be empty before it can be deleted" }],
        };
      }
  ...
  .get(
    "/api/v2/projects/:project_id/tag-bindings",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const projectId = params["project_id"] ?? "";
      const project = await db.query.projects.findFirst({ where: eq(projects.id, projectId) });
      if (
      ...
      return {
        data: tags.map(
          (t: Readonly<typeof projectTags.$inferSelect>): Record<string, unknown> => projectTagBindingResource(t),
        ),
      };
    },
  )
  .get(
    "/api/v2/projects/:project_id/effective-tag-bindings",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const projectId = params["project_id"] ?? "";
      const project = await db.query.projects.findFirst({ where: eq(projects.id, projectId) });
      if (
      ...
      return {
        data: tags.map(
          (t: Readonly<typeof projectTags.$inferSelect>): Record<string, unknown> => projectTagBindingResource(t, true),
        ),
      };
    },
  )
  .patch(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      const entries: TagEntry[] = items.map((item): TagEntry => {
        ...
        const attrs =
          resource["attributes"] !== null && typeof resource["attributes"] === "object"
          ...
          errors: [
            {
              ...
              detail: "Each tag binding requires a string key and value",
            },
          ],
        };
      }
          ...
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "At least one and no more than ten tag bindings are required",
            },
          ],
        };
      }
      if (new Set(entries.map((entry): string => entry.key)).size !== entries.length) {
        (set as { status: number }).status = 422;
        return {
          errors: [{ status: "422", title: "Unprocessable Entity", detail: "Tag binding keys must be unique" }],
        };
      }
      await db
        .insert(projectTags)
        .values(
          entries.map((entry): typeof projectTags.$inferInsert => ({
            id: newResourceId("ptag"),
            projectId,
            key: entry.key,
            value: entry.value,
          })),
        )
        .onConflictDoUpdate({ target: [projectTags.projectId, projectTags.key], set: { value: sql`excluded.value` } });
      const tags = await db.query.projectTags.findMany({ where: eq(projectTags.projectId, projectId) });
  ...
  .post(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      await db
        .insert(projectTags)
        .values(
          entries.map((e: TagEntry): typeof projectTags.$inferInsert => ({
            id: newResourceId("ptag"),
            projectId,
            key: e.key,
            value: e.value,
      ...
      const allTags = await db.query.projectTags.findMany({
        where: and(
          ...
          inArray(
            projectTags.key,
            entries.map((e): string => e.key),
          ),
        ),
      });
      const created = allTags.map(
        (pt: Readonly<typeof projectTags.$inferSelect>): Record<string, unknown> => projectTagBindingResource(pt),
      );
      (set as { status: number }).status = 201;
      return { data: created };
    },
    ...
    async ({
      ...
      const keys = tagList
        .map((item: unknown): string => {
          ...
          const attrs =
            typeof i["attributes"] === "object" && i["attributes"] !== null
      ...
      if (keys.length > 0)
        await db.delete(projectTags).where(and(eq(projectTags.projectId, projectId), inArray(projectTags.key, keys)));
      (set as { status: number }).status = 204;
      return {};
    },
  )
  .post(
    "/api/v2/projects/:project_id/relationships/workspaces",
    async ({
      params,
      body,
      user,
      ...
      const workspaceIds = items
        .map((item): string => {
          ...
          return typeof value["id"] === "string" && (value["type"] === undefined || value["type"] === "workspaces")
            ? value["id"]
            : "";
        })
          ...
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "data must contain workspace resource identifiers",
            },
          ],
        };
      }
      ...
      if (
        sourceWorkspaces.length !== workspaceIds.length ||
        sourceWorkspaces.some((workspace): boolean => workspace.orgId !== destination.orgId)
      ) {
        (set as { status: number }).status = 422;
        return {
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "Workspaces must belong to the project organization",
            },
          ],
        };
      }
      ...
      await db.transaction(async (tx): Promise<void> => {
        for (const workspace of sourceWorkspaces) {
          ...
          if (workspace.inheritsProjectAutoDestroy)
            updates.autoDestroyActivityDuration = destination.autoDestroyActivityDuration;
