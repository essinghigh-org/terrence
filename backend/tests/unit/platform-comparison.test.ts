...
function version(id: string, serial: number, payload: string): Parameters<typeof compareStateVersions>[0] {
  return {
    ...
    uploadSha256: null,
    runId: `run-${serial}`,
    createdAt: Date.UTC(2026, 0, serial),
  };
}
...
describe("platform comparison projections", () => {
  it("reports state additions and masks sensitive resource changes", () => {
    const before = version(
      "state-1",
      1,
      state(1, [resource("web", "i-1", "before")], { endpoint: { value: "old", sensitive: false } }),
    );
    const after = version(
      "state-2",
      2,
      state(2, [resource("web", "i-1", "after"), resource("worker", "i-2", "new")], {
        endpoint: { value: "new", sensitive: false },
      }),
    );
    ...
    expect(comparison.resources.added).toEqual([
      {
        address: "example_resource.worker",
        mode: "managed",
        type: "example_resource",
        provider: 'provider["registry.example/example"]',
      },
  ...
  it("masks Terraform path metadata on both sides while retaining public changes", () => {
    ...
    for (const metadata of [
      ...
      const comparison = compareStateVersions(
        before,
        version("state-2", 2, state(2, [instance("new", "hidden-after", metadata)])),
      );
  ...
  it("does not expose sensitive IDs through inventory identities or moves", () => {
    const sensitive = (name: string) => ({
      ...
      provider: "example",
      instances: [
        { attributes: { id: "hidden-identity" }, sensitive_attributes: [[{ type: "get_attr", value: "id" }]] },
      ],
    });
...
describe("platform workflow safety helpers", () => {
  it("coalesces equivalent drift observations across assessment IDs", () => {
    ...
    expect(driftFingerprint({ workspaceId: "ws-1", assessmentId: "assessment-a", drifted: true, checks })).toBe(
      driftFingerprint({ workspaceId: "ws-1", assessmentId: "assessment-b", drifted: true, checks }),
    );
  });
  ...
  it("validates import mappings and emits reviewable import blocks", () => {
    ...
    const invalid = validateImportMappings([
      { address: "bad address", "provider-id": "vpc-123" },
      { address: "example_vpc.main", "provider-id": "vpc-123" },
    ]);
  ...
  it("keeps upgrade rehearsal and policy playground review-only", () => {
    expect(
      upgradeRehearsalResult({ engine: "terraform", version: "1.9.0", baselineFresh: true, candidateLockDigest: null }),
    ).toMatchObject({ status: "ready-for-speculative-plan", "apply-authority": false });
    expect(
      policyPlaygroundResult({ kind: "opa", source: "package terrence\ndefault allow := true", plan: {} }),
    ).toMatchObject({ status: "review-only", "apply-authority": false });
    expect(policyPlaygroundResult({ kind: "opa", source: "default allow := true", plan: {} })["status"]).toBe(
      "invalid",
    );
  });

  it("builds inventory observations only from retained Terraform state", () => {
    const observations = stateInventoryObservations(
      version("state-1", 1, state(1, [resource("web", "i-1", "secret")])),
    );
    expect(observations).toHaveLength(1);
    expect(observations[0]).toMatchObject({
      identity: 'provider["registry.example/example"]|example_resource|i-1',
      identitySource: "provider-id",
