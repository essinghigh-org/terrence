...
function notificationUserIds(body: unknown, attributes: Readonly<Record<string, unknown>>): readonly string[] | false {
  ...
  return Array.isArray(attributeIds) &&
    attributeIds.every((id: unknown): id is string => typeof id === "string" && id !== "")
...
function createValidationErrors(
  ...
  if (isEncryptedTokenInput(token))
    errors.push({
      status: "422",
      title: "Unprocessable Entity",
      detail: "token must be plaintext",
  ...
  if (!isNotificationDestination(destinationType))
    errors.push({
      status: "422",
      title: "Unprocessable Entity",
      detail: "destination-type must be one of generic, slack, discord, microsoft-teams, or email",
      source: { pointer: "/data/attributes/destination-type" },
    });
  if (destinationType === "email") {
    if (!isValidEmailAddresses(emailAddresses)) {
      errors.push({
        status: "422",
        title: "Unprocessable Entity",
        detail: "email-addresses must contain at least one valid email address",
        source: { pointer: "/data/attributes/email-addresses" },
      });
    }
    ...
    errors.push({
      status: "422",
      title: "Unprocessable Entity",
      detail: "URL must be a valid http(s) webhook",
...
async function subscriptionFor(configuration: NcItem): Promise<Subscription | undefined> {
  if (configuration.workspaceId !== null) {
    ...
    return workspace === undefined ? undefined : { id: workspace.id, type: "workspaces", orgId: workspace.orgId };
  }
  if (configuration.projectId !== null) {
    ...
    return project === undefined ? undefined : { id: project.id, type: "projects", orgId: project.orgId };
  }
  if (configuration.teamId !== null) {
    const team = await db.query.teams.findFirst({ where: eq(teams.id, configuration.teamId) });
    return team === undefined ? undefined : { id: team.id, type: "teams", orgId: team.orgId };
  }
  return undefined;
}
...
function isCreatePayload(body: unknown): boolean {
  ...
  return (
    data !== null &&
    typeof data === "object" &&
    (data as Record<string, unknown>)["type"] === "notification-configurations"
  );
}
...
function resolveCreateEmails(
  ...
  const emailAllMembers =
    typeof attributes["email-all-members"] === "boolean"
...
function buildCreateValues(
  ...
  return {
    ...
    emailAddresses:
      scalars.destinationType === "email" && isEmailAddressArray(scalars.emailAddresses)
        ? [...scalars.emailAddresses]
        : null,
...
function createValues(
  ...
  if (triggers === null || !createDestinationValid(scalars, triggers, emails.emailAllMembers, emails.emailUserIds))
    return undefined;
  ...
  return db.transaction(async (tx: unknown): Promise<boolean> => {
    ...
    if (values.workspaceId !== null && values.workspaceId !== undefined) {
      const rows = await t
        ...
        .where(eq(notificationConfigurations.workspaceId, values.workspaceId));
      const currentCount = rows[0]?.total ?? 0;
      await t
        .insert(notificationWorkspaceCounters)
        .values({
        ...
        .onConflictDoNothing();
      await t
        .update(notificationWorkspaceCounters)
        .set({
          configurationCount: currentCount,
          updatedAt: Date.now(),
        })
    ...
    await t
      ...
      .values({ ...values, token: await encryptNotificationToken(values.token) });
    return true;
  });
}
...
async function decryptedNotification(configuration: NcItem): Promise<NcItem> {
  return configuration.token === null
    ...
    : {
        ...configuration,
        token: isEncryptedSecret(configuration.token) ? await decryptSecret(configuration.token) : configuration.token,
      };
}
...
function parsePatchScalars(attributes: Record<string, unknown>): PatchField {
  ...
  if (attributes["name"] !== undefined) {
    if (typeof attributes["name"] !== "string" || attributes["name"].trim() === "") {
      return {
        error: {
          errors: [{ status: "422", title: "Unprocessable Entity", detail: "name must be a non-empty string" }],
        },
      };
    }
  ...
  if (attributes["destination-type"] !== undefined) {
    if (
      ...
      return {
        error: { errors: [{ status: "422", title: "Unprocessable Entity", detail: "destination-type is invalid" }] },
      };
    }
...
function parsePatchContactFields(attributes: Record<string, unknown>): PatchField {
          ...
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "email-addresses must contain only valid email addresses",
              source: { pointer: "/data/attributes/email-addresses" },
            },
          ],
        },
          ...
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "triggers contains an unsupported notification trigger",
            },
          ],
        },
      };
  ...
  if (attributes["token"] !== undefined) {
    if (isEncryptedTokenInput(attributes["token"])) {
      return {
        error: { errors: [{ status: "422", title: "Unprocessable Entity", detail: "token must be plaintext" }] },
      };
    }
...
async function resolvePatchRecipients(
  ...
  const relationshipIds = relationshipUserIds(body);
  const recipientIds =
    relationshipIds ?? (attributes["email-user-ids"] === undefined ? undefined : notificationUserIds(body, attributes));
        ...
        errors: [
          {
            status: "422",
            title: "Unprocessable Entity",
            detail: "users must contain valid user resource identifiers",
          },
        ],
      },
    };
        ...
        errors: [
          {
            status: "422",
            title: "Unprocessable Entity",
            detail: "All notification users must be active organization members",
          },
        ],
      },
    };
  ...
  const emailAllMembers =
    recipientIds.length === 0 && configuration.workspaceId === null && attributes["email-all-members"] === undefined;
  return { recipientIds, emailAllMembers };
}

function emailCandidateComplete(
  candidate: Readonly<{
  ...
  return (
    (candidate.emailAddresses?.length ?? 0) > 0 ||
...
async function validatePatchCandidate(
  ...
  const destinationChanged = ["destination-type", "url", "email-addresses", "token"].some(
    (key): boolean => attributes[key] !== undefined,
  );
  ...
  if (candidate.destinationType === "email") {
          ...
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "Email notifications require users, email-all-members, or email-addresses",
            },
          ],
        },
      };
    ...
    if (!isWebhookUrl(candidate.url)) {
      return {
        status: 422,
        error: {
          errors: [{ status: "422", title: "Unprocessable Entity", detail: "URL must be a valid http(s) webhook" }],
        },
      };
    }
  ...
  if (
    ...
    (updates.enabled === true || destinationChanged) &&
    !(await verifyDestinationBeforeUpdate(candidate))
  ) {
    return {
      status: 400,
      error: {
        errors: [
          {
            status: "400",
            title: "Bad Request",
            detail: "Notification verification did not return a successful response",
          },
        ],
      },
    };
...
async function persistPatchUpdates(
  ...
  if (attributes["token"] !== undefined)
    persistedUpdates.token = await encryptNotificationToken(persistedUpdates.token);
...
export const notificationRoutes = new Elysia({ name: "notifications" })
  .use(authPlugin)
  .get(
    "/api/v2/workspaces/:workspace_id/notification-configurations",
    async ({ params, request, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const workspaceId = params["workspace_id"] ?? "";
      if ((await findAuthorizedWorkspace(workspaceId, user?.id, tokenOrgId, tokenTeamId ?? null)) === undefined)
        return notFound(set);
      ...
      return {
        data: configurations.map(
          (configuration: NcItem): Record<string, unknown> => notificationResource(configuration),
        ),
              ...
              : [
                  {
                    status: "422",
                    title: "Unprocessable Entity",
                    detail: "Valid name, url or email-addresses, and destination-type are required",
                  },
                ],
        };
      }
          ...
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "A workspace can have at most 20 notification configurations",
            },
          ],
        };
      }
  ...
  .get(
    "/api/v2/projects/:project_id/notification-configurations",
    async ({ params, request, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
  ...
  .get(
    ...
    async ({ params, request, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (
        team === undefined ||
        !(await checkOrgPermission(user?.id, team.orgId, "member", tokenOrgId, tokenTeamId ?? null))
      )
        return notFound(set);
  ...
  .get(
    "/api/v2/notification-configurations/:nc_id",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const configuration = await authorizedConfiguration(
        params["nc_id"] ?? "",
        user?.id,
        tokenOrgId,
        tokenTeamId,
  ...
  .get(
    "/api/v2/notification-configurations/:nc_id/relationships/workspaces",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const configuration = await authorizedConfiguration(
        params["nc_id"] ?? "",
        user?.id,
  ...
  .post(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      const validResources = raw.every(
        (item): boolean =>
          item !== null &&
          typeof item === "object" &&
      ...
      if (project === undefined || ids.length === 0 || !validResources) {
        (set as { status: number }).status = 422;
        return {
          errors: [{ status: "422", title: "Unprocessable Entity", detail: "data must contain workspace resources" }],
        };
      }
      const workspacesInProject = await db.query.workspaces.findMany({
        where: and(eq(workspaces.projectId, project.id), inArray(workspaces.id, ids)),
        columns: { id: true },
      });
          ...
          errors: [
            {
              status: "422",
              title: "Unprocessable Entity",
              detail: "All excluded workspaces must belong to the notification project",
            },
          ],
        };
      }
      await db
        .insert(notificationConfigurationWorkspaceExclusions)
        .values(
          [...new Set(ids)].map((workspaceId): typeof notificationConfigurationWorkspaceExclusions.$inferInsert => ({
  ...
  .delete(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      const ids = validResources ? raw.map((item): string => (item as Record<string, string>)["id"] ?? "") : [];
      if (ids.length === 0 || !validResources) {
        (set as { status: number }).status = 422;
        return { errors: [{ status: "422", title: "Unprocessable Entity" }] };
      }
      await db
        .delete(notificationConfigurationWorkspaceExclusions)
        .where(
          and(
            eq(notificationConfigurationWorkspaceExclusions.notificationConfigurationId, configuration.id),
            inArray(notificationConfigurationWorkspaceExclusions.workspaceId, [...new Set(ids)]),
          ),
        );
  ...
  .patch(
    ...
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      if (!isPatchPayload(body)) {
        ...
        return {
          errors: [
            { status: "422", title: "Unprocessable Entity", detail: "data.type must be notification-configurations" },
          ],
        };
      }
  ...
  .post(
    "/api/v2/notification-configurations/:nc_id/actions/verify",
    async ({
      params,
      query,
      user,
      ...
      teamId: tokenTeamId,
      set,
    }: ParamCtx & { query: Record<string, string | undefined> }): Promise<unknown> => {
      const configuration = await authorizedConfiguration(
        params["nc_id"] ?? "",
        user?.id,
        tokenOrgId,
      ...
      if (!delivery.successful) {
        ...
        return {
          errors: [
            { status: "400", title: "Bad Request", detail: `Notification verification returned HTTP ${delivery.code}` },
          ],
        };
      }
      return {
  ...
  .post(
    "/api/v2/notification-configurations/:nc_id/actions/verify-ownership",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const configuration = await authorizedConfiguration(
        params["nc_id"] ?? "",
        user?.id,
  ...
  .get(
    "/api/v2/notification-configurations/:nc_id/ownership-verified",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      const configuration = await authorizedConfiguration(
        params["nc_id"] ?? "",
        user?.id,
