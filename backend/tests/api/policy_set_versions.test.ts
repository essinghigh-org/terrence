...
describe("policy set version uploads", () => {
  ...
  it("creates a version, accepts its one-time signed tar.gz upload, and reports ready", async () => {
    ...
    // POL-010: the reference format documents the policy-set-version upload link as expiring
    // one hour after creation. Pin the TTL so the parity doesn't drift.
    const uploadUrl = new URL(upload);
    const expires = Number(uploadUrl.searchParams.get("expires"));
    const now = Math.floor(Date.now() / 1000);
    expect(expires - now).toBeGreaterThanOrEqual(3600 - 30); // ~1h, minus clock skew
    expect(expires - now).toBeLessThanOrEqual(3600 + 30);
