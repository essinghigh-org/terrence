...
test.skipIf(process.env["LOG_ARCHIVE_LOAD"] !== "1")(
  "measures status responsiveness during large archive writes and reads",
  async () => {
    ...
    try {
        ...
        for (let offset = 0; offset < 2000; offset += 100) {
          await db.insert(logs).values(
            Array.from({ length: 100 }, (_, i) => ({
              ...
              runId: id,
              phase: "plan",
              outputText: `${offset + i}:` + "0123456789abcdef".repeat(512),
              createdAt: offset + i,
            })),
          );
        }
      ...
      const monitor = Promise.all(
        Array.from({ length: process.env["LOG_ARCHIVE_STATUS"] === "0" ? 0 : 40 }, async () => {
      ...
      const p95 = (values: number[]): number =>
        values.sort((a, b) => a - b)[Math.min(values.length - 1, Math.floor(values.length * 0.95))] ?? 0;
      const report = {
        phase: process.env["LOG_ARCHIVE_PHASE"] ?? "after",
        rowsPerRun: 2000,
        payloadBytesPerRun: 2000 * 8192,
        statusRequests: latencies.length,
        statusP95Ms: p95(latencies),
        statusMaxMs: Math.max(0, ...latencies),
        eventLoopP95Ms: p95(delays),
        eventLoopMaxMs: Math.max(0, ...delays),
        startingRss,
