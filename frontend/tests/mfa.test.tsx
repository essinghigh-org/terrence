...
test("enrolls MFA after verifying an authenticator code", async () => {
  ...
  globalThis.fetch = mock(async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
    ...
    if (requestUrl === "/api/v2/account/mfa" && init?.method === undefined)
      return json({ data: { attributes: { enabled: false } } });
    if (requestUrl === "/api/v2/account/mfa/enroll")
      return json({
        data: { attributes: { secret: "JBSWY3DPEHPK3PXP", "otpauth-url": "otpauth://totp/Terrence:alice" } },
      });
  ...
  await waitFor((): void => {
    expect(view.getByText("Multi-factor authentication enabled")).toBeTruthy();
  });
  expect(
    calls.some(([requestUrl, init]) => requestUrl === "/api/v2/account/mfa/verify" && init?.method === "POST"),
  ).toBeTrue();
});
...
test("completes an MFA login challenge without exposing the password again", async () => {
  ...
  globalThis.fetch = mock(async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
    ...
    if (requestUrl === "/api/v2/users/login" && init?.method === "POST")
      return json({ data: { attributes: { "mfa-required": true, "mfa-challenge-token": "challenge-1" } } });
...
test("disables MFA with a current authenticator code and password", async () => {
  ...
  globalThis.fetch = mock(async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
    ...
    if (requestUrl === "/api/v2/account/sessions") return json({ data: [] });
    if (requestUrl === "/api/v2/account/mfa" && init?.method === undefined)
      return json({ data: { attributes: { enabled: true } } });
  ...
  const disableCall = calls.find(
    ([requestUrl, init]) => requestUrl === "/api/v2/account/mfa" && init?.method === "DELETE",
  );
  expect(disableCall?.[1]?.body).toBe(
    JSON.stringify({ data: { attributes: { code: "123456", current_password: "password" } } }),
  );
});
