...
const pathFor = (surface: Surface, workspace = ws): string => {
  ...
  switch (surface) {
    case "state":
      return `/api/v2/state-versions/${targetState}/download`;
    case "raw-plan":
      return `/api/v2/plans/plan-${targetRun}/json-output`;
    case "public-plan":
      return `/api/v2/plans/plan-${targetRun}/json-output-redacted`;
    case "state-link":
      return `/api/v2/runs/${targetRun}/input-state-version`;
    case "variables":
      return `/api/v2/workspaces/${workspace}/vars`;
  }
};
...
beforeAll(async () => {
  ...
  for (const workspace of [ws, otherWs, foreignWs]) {
    ...
    await db
      .insert(workspaces)
      .values({ id: workspace, orgId: workspace === foreignWs ? foreign.orgId : seed.orgId, name: workspace });
    await db.insert(runs).values({
      id: targetRun,
      workspaceId: workspace,
      status: "planned",
      createdAt: Date.now(),
      statusTimestamps: { "input-state-version-id": targetState },
    });
    await db.insert(stateVersions).values({
      ...
      statePayload: JSON.stringify({
        ...
        lineage: workspace,
        resources: [],
        outputs: { password: { value: canary, sensitive: true, type: "string" } },
      }),
    ...
    await db
      ...
      .values({ id: `${workspace}-var`, workspaceId: workspace, key: "password", value: canary, sensitive: true });
    await writePlanJsonArtifact(targetRun, {
      format_version: "1.2",
      variables: { password: { value: canary } },
      resource_changes: [
        {
          address: "test.example",
          change: { actions: ["create"], after: { password: canary }, after_sensitive: { password: true } },
        },
      ],
    });
  }
  for (const name of ["ordinary", "suspended", "site-admin"]) {
    await db.insert(users).values({
      ...
      username: id(name),
      passwordHash: "unused",
      isSuspended: name === "suspended",
      isSiteAdmin: name === "site-admin",
    });
    await db.insert(organizationMemberships).values({
      id: id(`${name}-membership`),
      userId: id(name),
      orgId: seed.orgId,
      role: name === "suspended" ? "owner" : "member",
    });
    await db
      .insert(apiTokens)
      .values({ id: id(`${name}-token`), userId: id(name), token: hashAuthenticationToken(id(`${name}-credential`)) });
  }
  ...
  await db.insert(teamWorkspaces).values({
    ...
    workspaceId: ws,
    access: "custom",
    permissions: { runs: "read", "state-versions": "none", variables: "none" },
  });
  await db.insert(apiTokens).values([
    ...
    { id: id("org-token"), orgId: seed.orgId, token: hashAuthenticationToken(id("org-credential")) },
    {
      id: scopedId,
      userId: seed.userId,
      token: hashAuthenticationToken(id("scoped-credential")),
  ...
  await db
    .insert(agentPoolTokens)
    .values({ id: id("pool-token"), agentPoolId: id("pool"), token: hashAuthenticationToken(id("agent-credential")) });
  principals.push(
    ...
    {
      name: "run token",
      token: await mintRunToken(runId, ws, seed.orgId),
      allows: ["state", "raw-plan", "public-plan"],
    },
    { name: "agent pool token", token: id("agent-credential"), allows: [] },
    {
      name: "site administrator",
      token: id("site-admin-credential"),
      allows: all,
      crossWorkspace: true,
...
afterAll(async () => {
  for (const workspace of [ws, otherWs, foreignWs])
    await deletePlanJsonArtifact(workspace === ws ? runId : `${workspace}-run`);
  ...
  await db
    ...
    .where(
      inArray(apiTokens.id, [
        ...
        id("org-token"),
        scopedId,
        ...["ordinary", "suspended", "site-admin"].map((name) => id(`${name}-token`)),
      ]),
    );
  ...
  await db
    ...
    .where(inArray(organizationMemberships.userId, [id("ordinary"), id("suspended"), id("site-admin")]));
  ...
  for (const principal of principals) {
    for (const workspace of [ws, otherWs, foreignWs]) {
      for (const surface of all) {
        const allowed =
          principal.allows.includes(surface) &&
          (workspace === ws ||
            (workspace === otherWs ? principal.crossWorkspace === true : principal.crossOrganization === true));
        const response = await request(pathFor(surface, workspace), {
          headers: principal.token === undefined ? {} : jsonHeaders(principal.token),
        });
...
test("read-only and non-user credentials cannot plan, apply or write variables", async () => {
  for (const principal of principals.filter(
    (entry) => entry.name !== "organization owner" && entry.name !== "site administrator",
  )) {
    const headers =
      principal.token === undefined ? { "Content-Type": "application/vnd.api+json" } : jsonHeaders(principal.token);
    const operations = [
      {
        path: "/api/v2/runs",
        body: {
      ...
      ...(principal.name === "organization token"
        ? []
        : [
            {
              path: `/api/v2/workspaces/${ws}/vars`,
              body: {
                data: { type: "vars", attributes: { key: "unauthorized", value: canary, category: "terraform" } },
              },
            },
          ]),
    ];
    for (const operation of operations) {
      ...
      expect([401, 403, 404], `${principal.name}: ${operation.path}: ${await response.text()}`).toContain(
        response.status,
      );
    }
  }
  ...
  expect(
    (await db.query.workspaceVariables.findMany({ where: eq(workspaceVariables.workspaceId, ws) })).map(
...
test("only a site administrator can use the administration surface", async () => {
  for (const principal of principals) {
    const response = await request("/api/v2/admin/users", {
      headers: principal.token === undefined ? {} : jsonHeaders(principal.token),
    });
  ...
  for (const granted of [false, true, false]) {
    await db
      ...
      .set({
        scopes: JSON.stringify({
          ...
          orgs: [seed.orgId],
          workspaces: [ws],
          permissions: { "workspaces:read": true, "runs:read": true, ...(granted ? { "state:read": true } : {}) },
        }),
      })
      .where(eq(apiTokens.id, scopedId));
    ...
    const mcp = await request("/mcp", {
      ...
      body: JSON.stringify({
        ...
        id: 1,
        method: "tools/call",
        params: { name: "get_plan_json", arguments: { workspace_id: ws, run_id: runId } },
      }),
    });
    ...
    for (const workspace of [ws, foreignWs]) {
      const response = await request("/mcp", {
        ...
        headers: {
          ...
          ...(principal.token === undefined ? {} : { Authorization: `Bearer ${principal.token}` }),
        },
        body: JSON.stringify({
          jsonrpc: "2.0",
          id: 2,
          method: "tools/call",
          params: { name: "get_workspace_state", arguments: { workspace_id: workspace } },
        }),
      });
      const text = await response.text();
      const allowed =
        principal.name !== "run token" &&
        principal.allows.includes("state") &&
        (workspace === ws || principal.crossOrganization === true);
      if (allowed) {
...
test("MCP rechecks suspension, expiry and revoked browser families on every request", async () => {
  const call = (): Promise<Response> =>
    request("/mcp", {
      ...
      body: JSON.stringify({
        ...
        id: 3,
        method: "tools/call",
        params: { name: "get_workspace_state", arguments: { workspace_id: ws } },
      }),
    });
  ...
  try {
    for (const condition of ["suspended", "expired", "revoked-family"] as const) {
      await db
        .update(users)
        .set({ isSuspended: condition === "suspended" })
        .where(eq(users.id, seed.userId));
      await db
        .update(apiTokens)
        .set({
          expiresAt: condition === "expired" ? Date.now() - 1 : null,
          refreshFamilyId: condition === "revoked-family" ? id("revoked-family") : null,
        })
        .where(eq(apiTokens.id, seed.tokenId));
