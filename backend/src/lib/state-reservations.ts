...
export function stateReservationObsolete(reservation: Reservation, workspace: Workspace, now = Date.now()): boolean {
  return (
    now >= (reservation.uploadExpiresAt ?? reservation.createdAt + STATE_UPLOAD_TTL_MS) ||
...
export async function fenceStateWorkspace(tx: DeepReadonly<typeof db>, workspace: Workspace): Promise<boolean> {
  const rows = await tx
    .update(workspaces)
    .set({ locked: workspace.locked })
    .where(
      and(
        eq(workspaces.id, workspace.id),
        workspace.locked === null ? isNull(workspaces.locked) : eq(workspaces.locked, workspace.locked),
        workspace.lockOwnerType === null
          ? isNull(workspaces.lockOwnerType)
          : eq(workspaces.lockOwnerType, workspace.lockOwnerType),
        workspace.lockOwnerId === null
          ? isNull(workspaces.lockOwnerId)
          : eq(workspaces.lockOwnerId, workspace.lockOwnerId),
...
export async function pruneStateReservations(tx: DeepReadonly<typeof db>, workspace: Workspace): Promise<void> {
  const pending = await tx.query.stateVersions.findMany({
    where: and(
      eq(stateVersions.workspaceId, workspace.id),
      eq(stateVersions.status, "pending"),
      isNull(stateVersions.statePayload),
    ),
  });
  for (const reservation of pending) {
    if (!stateReservationObsolete(reservation, workspace)) continue;
    await discardStateReservation(
      tx,
      reservation,
      workspace,
      Date.now() >= (reservation.uploadExpiresAt ?? reservation.createdAt + STATE_UPLOAD_TTL_MS)
        ? "upload-expired"
        : "lock-changed",
    );
  }
...
export async function discardStateReservation(
  tx: DeepReadonly<typeof db>,
  reservation: Reservation,
  workspace: Workspace,
  reason: "upload-expired" | "lock-changed" | "discarded",
): Promise<boolean> {
