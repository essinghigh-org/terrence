...
function validateBootstrapPassword(password: string, username: string): void {
  ...
  const message = isOnlyLength
    ? `ADMIN_PASSWORD ${policy.errors[0]?.replace(/^Password /, "").toLowerCase()}`
    : `ADMIN_PASSWORD is invalid: ${policy.errors.join(" ")}`;
...
export async function bootstrapInitialAdmin(): Promise<"created" | "disabled" | "skipped"> {
  ...
  const created = await db.transaction(
    async (tx: unknown): Promise<{ created: boolean; organizationCreated: boolean }> => {
...
export async function resetAdminPassword(): Promise<"reset" | "disabled"> {
  ...
  if (readResetMarker() === marker) {
    console.log(
      "[terrence] ADMIN_PASSWORD_RESET already consumed for this configuration; remove TERRENCE_ADMIN_PASSWORD_RESET and ADMIN_PASSWORD to re-arm recovery.",
    );
    return "disabled";
  }
