...
import { MAX_NOTIFICATION_SNOOZE_MS, notificationSnooze, setNotificationSnooze } from "../lib/notification-state";
...
async function visibleWorkspaces(
  ...
  const checks = await Promise.all(
    candidates.map(
      async (workspace): Promise<SafeWorkspace | null> =>
        (await checkWorkspacePermission(workspace, user?.id, tokenOrgId ?? null, tokenTeamId ?? null, "read"))
          ? workspace
          : null,
    ),
  );
...
function matchWorkspaceResults(workspacesForUser: readonly SafeWorkspace[], needle: string): Record<string, unknown>[] {
  ...
  for (const workspace of workspacesForUser) {
    ...
    const reasons = fields.flatMap((field, index): string[] =>
      searchMatch(field, needle) ? [["id", "name", "description", "source", "repository"][index] ?? "metadata"] : [],
    );
    if (reasons.length > 0) {
      results.push(
        resource(
...
function matchProjectResults(
  projectRows: readonly (typeof projects.$inferSelect)[],
  needle: string,
): Record<string, unknown>[] {
  const results: Record<string, unknown>[] = [];
  for (const project of projectRows) {
    const reasons = [project.id, project.name, project.description].flatMap((field, index): string[] =>
      searchMatch(field, needle) ? [["id", "name", "description"][index] ?? "metadata"] : [],
    );
    if (reasons.length > 0)
      results.push(
        resource(
          ...
          "projects",
          {
            ...
            "match-reasons": reasons,
            freshness: freshness(project.createdAt),
          },
          { self: `/api/v2/projects/${encodeURIComponent(project.id)}` },
        ),
      );
  }
  return results;
...
function matchRunResults(
  ...
  for (const run of runRows) {
    ...
    const values: readonly unknown[] = [
      ...
      ingress?.commitSha,
      ingress?.branch,
      ingress?.commitMessage,
    ];
    const reasons = values.flatMap((field, index): string[] =>
      searchMatch(field, needle)
        ? [["id", "message", "status", "commit-sha", "branch", "commit-message"][index] ?? "metadata"]
        : [],
    );
...
async function searchResources(ctx: ParamCtx): Promise<unknown> {
  ...
  const workspaceById = new Map(
    workspacesForUser.map((workspace): [string, SafeWorkspace] => [workspace.id, workspace]),
  );
  const projectIds = [
    ...new Set(
      workspacesForUser.flatMap((workspace): string[] => (workspace.projectId === null ? [] : [workspace.projectId])),
    ),
  ];
  const [projectRows, runRows, cvRows] = await Promise.all([
    projectIds.length === 0
      ? Promise.resolve([] as (typeof projects.$inferSelect)[])
      : db.query.projects.findMany({ where: inArray(projects.id, projectIds), limit: MAX_CANDIDATES }),
    workspaceIds.length === 0
      ? Promise.resolve([] as (typeof runs.$inferSelect)[])
    ...
    workspaceIds.length === 0
      ? Promise.resolve([] as (typeof configurationVersions.$inferSelect)[])
      : db.query.configurationVersions.findMany({
          where: inArray(configurationVersions.workspaceId, workspaceIds),
  ...
  results.sort((a, b): number =>
    `${toComparableString(a["type"])}:${toComparableString(a["id"])}`.localeCompare(
      `${toComparableString(b["type"])}:${toComparableString(b["id"])}`,
    ),
  );
  const { number, size } = pageRequest(request);
  return {
    data: results.slice((number - 1) * size, number * size),
    ...pagination(request, number, size, results.length),
  };
}
...
function timelineEvent(
  ...
  return resource(id, "operational-events", {
    type,
...
function runStatusEvents(
  ...
  for (const [status, timestamp] of Object.entries(statusTimestamps ?? {})) {
    ...
    if (occurredAt !== null)
      events.push(
        timelineEvent(`run:${runId}:${status}`, "run-status", occurredAt, { status: status.replace(/-at$/, "") }),
      );
  }
  return events;
}
...
function configurationIngressEvents(
  ...
  if (commitAt === null || ingress === undefined || (ingress.commitSha === undefined && ingress.branch === undefined))
    return [];
  return [
    timelineEvent(`configuration:${configurationVersion?.id ?? "unknown"}:ingress`, "configuration-ingress", commitAt, {
...
function configurationStatusEvents(
  ...
  for (const [status, timestamp] of Object.entries(configurationVersion?.statusTimestamps ?? {})) {
    ...
    if (occurredAt !== null)
      events.push(
        timelineEvent(
          `configuration:${configurationVersion?.id ?? "unknown"}:${status}`,
          "configuration-status",
          occurredAt,
          { status: status.replace(/-at$/, ""), "configuration-version-id": configurationVersion?.id ?? null },
        ),
      );
  }
...
function auditTimelineEvents(auditRows: readonly (typeof auditLogs.$inferSelect)[]): Record<string, unknown>[] {
  ...
  for (const audit of auditRows) {
    ...
    if (occurredAt !== null)
      events.push(
        timelineEvent(`audit:${audit.id}`, "audit", occurredAt, {
          action: audit.action,
          "resource-type": audit.resourceType,
          "actor-id": audit.userId,
        }),
      );
  }
  return events;
...
function byOccurredAt(a: Record<string, unknown>, b: Record<string, unknown>): number {
  ...
  const aTime =
    aAttributes !== null && typeof aAttributes === "object"
      ? (aAttributes as Record<string, unknown>)["occurred-at"]
      : "";
  const bTime =
    bAttributes !== null && typeof bAttributes === "object"
      ? (bAttributes as Record<string, unknown>)["occurred-at"]
      : "";
...
async function runTimeline({
  ...
  const [configurationVersion, auditRows] = await Promise.all([
    authorized.run.configurationVersionId === null
      ? Promise.resolve(undefined)
      : db.query.configurationVersions.findFirst({
          where: eq(configurationVersions.id, authorized.run.configurationVersionId),
        }),
    db.query.auditLogs.findMany({
      where: and(
        or(eq(auditLogs.resourceType, "run"), eq(auditLogs.resourceType, "runs")),
  ...
  return {
    ...
    meta: {
      ...pageInfo.meta,
      run: { id: runId, status: authorized.run.status, workspace: authorized.workspace.name },
...
function resolveBlueprintReferenceIds(parameters: Record<string, unknown>, errors: string[]): Record<string, unknown> {
  ...
  for (const key of ["variable-set-ids", "policy-set-ids"] as const) {
    ...
    if (value !== undefined) {
      if (
        !Array.isArray(value) ||
        value.some((item: unknown): boolean => typeof item !== "string" || item.trim() === "")
      )
        errors.push(`${key} must be an array of IDs`);
...
function blueprintPreview(
  ...
  attributes: Readonly<Record<string, unknown>>,
): Readonly<{ valid: boolean; errors: readonly string[]; configuration: Record<string, unknown> }> {
  const parameters =
...
async function workspaceAdoptionExport(workspace: SafeWorkspace): Promise<
  Readonly<{
...
> {
  const [organization, project, variableLinks, policyLinks] = await Promise.all([
    ...
    workspace.projectId === null
      ? Promise.resolve(undefined)
      : db.query.projects.findFirst({ where: eq(projects.id, workspace.projectId), columns: { name: true } }),
    db.query.variableSetWorkspaces.findMany({
      where: eq(variableSetWorkspaces.workspaceId, workspace.id),
      columns: { variableSetId: true },
    }),
    db.query.policySetWorkspaces.findMany({
      where: eq(policySetWorkspaces.workspaceId, workspace.id),
      columns: { policySetId: true },
    }),
  ]);
  const [variableSetRows, policySetRows] = await Promise.all([
    variableLinks.length === 0
      ? Promise.resolve([] as (typeof variableSets.$inferSelect)[])
      : db.query.variableSets.findMany({
          where: inArray(
            variableSets.id,
            variableLinks.map((link): string => link.variableSetId),
          ),
          columns: { id: true, name: true },
        }),
    policyLinks.length === 0
      ? Promise.resolve([] as (typeof policySets.$inferSelect)[])
      : db.query.policySets.findMany({
          where: inArray(
            policySets.id,
            policyLinks.map((link): string => link.policySetId),
          ),
          columns: { id: true, name: true },
        }),
  ]);
...
async function accessReviewForWorkspace(
  ...
  const permissionValues = Object.fromEntries(
    await Promise.all(
      permissions.map(
        async (permission): Promise<[string, boolean]> => [
          permission,
          subjectId === null
            ? false
            : await checkWorkspacePermission(workspace, subjectId, tokenOrgId, tokenTeamId, permission),
        ],
      ),
    ),
  );
  const [membership, teamMembershipRows] =
    subjectId === null
      ? [undefined, [] as (typeof teamMemberships.$inferSelect)[]]
      : await Promise.all([
          db.query.organizationMemberships.findFirst({
            where: and(
              eq(organizationMemberships.orgId, workspace.orgId),
  ...
  const [teamRows, workspaceGrants] = await Promise.all([
    teamIds.length === 0
      ...
      : db.query.teams.findMany({ where: and(eq(teams.orgId, workspace.orgId), inArray(teams.id, teamIds)) }),
    teamIds.length === 0
      ? Promise.resolve([] as (typeof teamWorkspaces.$inferSelect)[])
      : db.query.teamWorkspaces.findMany({
          where: and(eq(teamWorkspaces.workspaceId, workspace.id), inArray(teamWorkspaces.teamId, teamIds)),
        }),
  ]);
  return {
    ...
    grants: [
      ...(membership === undefined
        ? []
        : [{ source: "organization-membership", id: membership.id, role: membership.role, status: membership.status }]),
      ...teamRows.map(
        (team): Record<string, unknown> => ({
          source: "team",
          id: team.id,
          name: team.name,
          "organization-access": Object.keys(team.organizationAccess).filter(
            (key): boolean => team.organizationAccess[key] === true,
          ),
        }),
      ),
      ...workspaceGrants.map(
    ...
    "pre-issued-capability-note":
      "Fresh authorization is checked on every request; existing signed capabilities expire according to their endpoint TTL.",
  };
}

async function notificationOrgId(
  configuration: Readonly<{ workspaceId: string | null; projectId: string | null; teamId: string | null }>,
): Promise<string | null> {
  ...
  if (configuration.projectId !== null)
    return (
      (await db.query.projects.findFirst({ where: eq(projects.id, configuration.projectId), columns: { orgId: true } }))
        ?.orgId ?? null
    );
  if (configuration.teamId !== null)
    return (
      (await db.query.teams.findFirst({ where: eq(teams.id, configuration.teamId), columns: { orgId: true } }))
        ?.orgId ?? null
    );
  return null;
}
...
function durationFromBody(body: unknown): number {
  ...
  if (typeof rawSeconds === "string" && rawSeconds.trim() !== "" && Number.isFinite(Number(rawSeconds)))
    return Number(rawSeconds) * 1_000;
...
async function applySnoozeDuration(
  ...
  if (!Number.isFinite(duration))
    return { failure: apiError(set, 422, "Unprocessable Entity", "duration must be finite") };
...
async function notificationSnoozeResponse(
  ...
  return {
    data: resource(id, "notification-snoozes", {
      active: current !== null,
      until: current === null ? null : safeIso(current.until),
      reason: current?.reason ?? null,
      "max-duration-ms": MAX_NOTIFICATION_SNOOZE_MS,
    }),
  };
}
  ...
  return grants.filter(
    (grant): boolean =>
      (removeMembershipId !== null &&
        grant["source"] === "organization-membership" &&
        grant["id"] === removeMembershipId) ||
      (removeTeamId !== null &&
        ((grant["source"] === "team" && grant["id"] === removeTeamId) ||
          (grant["source"] === "team-workspace" && grant["team-id"] === removeTeamId))),
  );
}
...
function reviewRemovalPreview(
  ...
  afterPermissions: Record<string, boolean>;
...
export const operationsIntelligenceRoutes = new Elysia({ name: "operations-intelligence" })
  ...
  .get("/api/v2/admin/webhook-deliveries", async (ctx: ParamCtx): Promise<unknown> => {
    ...
    const [jobs, totals] = await Promise.all([
      db.query.durableJobs.findMany({
        where,
        orderBy: [desc(durableJobs.updatedAt), desc(durableJobs.id)],
        limit: size,
    ...
    const deliveries =
      deliveryIds.length === 0
        ? []
        : await db.query.githubWebhookDeliveries.findMany({ where: inArray(githubWebhookDeliveries.id, deliveryIds) });
  ...
  .get("/api/v2/admin/webhook-deliveries/:delivery_id", async ({ params, user, set }: ParamCtx): Promise<unknown> => {
    ...
    const job = await db.query.durableJobs.findFirst({
      where: and(eq(durableJobs.kind, "vcs-webhook"), or(eq(durableJobs.id, id), eq(durableJobs.dedupeKey, id))),
    });
    ...
    const delivery =
      deliveryId === null
        ? undefined
        : await db.query.githubWebhookDeliveries.findFirst({ where: eq(githubWebhookDeliveries.id, deliveryId) });
    return { data: webhookResource(job, delivery, true) };
  })
  .post(
    "/api/v2/admin/webhook-deliveries/:delivery_id/actions/replay",
    async ({ params, user, set }: ParamCtx): Promise<unknown> => {
      ...
      const job = await db.query.durableJobs.findFirst({
        where: and(
          eq(durableJobs.kind, "vcs-webhook"),
          or(eq(durableJobs.id, requestedId), eq(durableJobs.dedupeKey, requestedId)),
        ),
      });
      if (job === undefined) return hidden(set);
      const body = webhookPayloadFromJob(job);
      ...
      if (deliveryId === null)
        return apiError(
          ...
          409,
          "Conflict",
          "This delivery has no durable provider identity and cannot be replayed safely",
        );
      ...
      if (!retried)
        return apiError(
          ...
          409,
          "Conflict",
          "Only failed deliveries can be replayed; the original identity remains idempotent",
        );
      await auditLog("retry", "webhook-deliveries", deliveryId, user?.id ?? null, null, {
        jobId: job.id,
        replay: "same-logical-event",
      });
  ...
  .post(
    "/api/v2/admin/webhook-deliveries/:delivery_id/actions/retry",
    async ({ params, user, set }: ParamCtx): Promise<unknown> => {
      const authorization = await adminRequired(user, set);
      if (authorization !== true) return authorization;
      const requestedId = params["delivery_id"] ?? "";
      ...
      const body = webhookPayloadFromJob(job);
      const deliveryId = typeof body["deliveryId"] === "string" ? body["deliveryId"] : null;
      if (deliveryId === null || !(await retryFailedVcsWebhookDelivery(deliveryId)))
        return apiError(set, 409, "Conflict", "Only failed deliveries can be retried");
      await auditLog("retry", "webhook-deliveries", deliveryId, user?.id ?? null, null, {
        jobId: job.id,
        replay: "same-logical-event",
      });
  ...
  .get(
    "/api/v2/workspace-blueprints",
    async ({ user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      if (!hasPrincipal(user, tokenOrgId, tokenTeamId)) return unauthorized(set);
      return { data: WORKSPACE_BLUEPRINTS.map(blueprintResource) };
    },
  )
  .get(
    "/api/v2/workspace-blueprints/:blueprint_id",
  ...
  .post(
    "/api/v2/workspace-blueprints/:blueprint_id/actions/preview",
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      const attributes = attributesFrom(body);
      const workspaceId =
        typeof attributes["workspace-id"] === "string"
      ...
      if (
        workspaceId !== null &&
        (await findAuthorizedWorkspace(workspaceId, user?.id, tokenOrgId ?? null, tokenTeamId ?? null, "admin")) ===
          undefined
      )
        return hidden(set);
      const preview = blueprintPreview(blueprint, attributes);
      return {
        data: resource(blueprint.id, "workspace-blueprint-previews", {
          ...preview,
          "will-mutate": false,
  ...
  .get(
    "/api/v2/workspaces/:workspace_id/adoption-export",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, request, set }: ParamCtx): Promise<unknown> => {
      ...
      await auditLog("export", "workspace-configuration", workspace.id, user?.id ?? null, workspace.orgId, {
        format: "hcl",
        secretValues: false,
      });
      return {
        data: resource(
          ...
          "configuration-exports",
          {
            ...
            "stable-resource-name": stableConfigName(workspace.name),
            "secret-values-included": false,
          },
          { self: request.url },
        ),
  ...
  .get(
    "/api/v2/workspaces/:workspace_id/access-review",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      if (!hasPrincipal(user, tokenOrgId, tokenTeamId)) return unauthorized(set);
      const workspace = await findAuthorizedWorkspace(
        params["workspace_id"] ?? "",
      ...
      return {
        data: resource(
          ...
          await accessReviewForWorkspace(workspace, user?.id ?? null, tokenOrgId ?? null, tokenTeamId ?? null),
        ),
      };
    },
  )
  .post(
    "/api/v2/workspaces/:workspace_id/access-review/preview",
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      if (!hasPrincipal(user, tokenOrgId, tokenTeamId)) return unauthorized(set);
      const workspace = await findAuthorizedWorkspace(
        params["workspace_id"] ?? "",
      ...
      const removeMembershipId =
        typeof attributes["remove-membership-id"] === "string" ? attributes["remove-membership-id"] : null;
      ...
      await auditLog("access-review", "workspaces", workspace.id, user?.id ?? null, workspace.orgId, {
        targetUserId,
        removeMembershipId,
        removeTeamId,
        affectedGrantCount: preview.affected.length,
      });
      return {
        data: resource(workspace.id, "workspace-access-review-previews", {
          target: targetUserId,
          "current-permissions": preview.currentPermissions,
          "after-removal-permissions": preview.afterPermissions,
          "affected-grants": preview.affected,
          "automatic-revocation": false,
          "fresh-capabilities-rechecked": true,
          "pre-issued-capability-note": current["pre-issued-capability-note"],
        }),
      };
    },
  )
  ...
  .post(
    "/api/v2/policy-packs/:pack_id/actions/preview",
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      if (!hasPrincipal(user, tokenOrgId, tokenTeamId)) return unauthorized(set);
      const pack = OPINIONATED_POLICY_PACKS.find((item): boolean => item.id === (params["pack_id"] ?? ""));
      ...
      if (runId !== null) {
        if (
          (await findAuthorizedRun(runId, user?.id, tokenOrgId ?? null, tokenTeamId ?? null, "run-read")) === undefined
        )
          return hidden(set);
      } else if (
        workspaceId !== null &&
        (await findAuthorizedWorkspace(workspaceId, user?.id, tokenOrgId ?? null, tokenTeamId ?? null, "read")) ===
          undefined
      )
        return hidden(set);
      return {
        data: resource(pack.id, "policy-pack-previews", {
          pack: policyPackResource(pack),
          mode: "advisory",
          scope: { workspaceId, runId },
          outcomes: pack.rules.map(
            (rule): Record<string, unknown> => ({
              rule: rule.id,
              result: "unknown",
              explanation: "A plan artifact is required for a deterministic evaluation.",
            }),
          ),
          "will-enforce": false,
        }),
  ...
  .get(
    ...
    async ({ request, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> => {
      ...
      const matches = documentationMatches(query, limit).map(
        (match): Record<string, unknown> =>
          resource(
            ...
            "runbooks",
            {
              ...
              category: match.category,
              description: match.description,
              tags: [match.category.toLocaleLowerCase(), "reference"],
              status: "reference",
              "matched-on": query,
  ...
  .get(
    "/api/v2/runs/:run_id/runbooks",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, request, set }: ParamCtx): Promise<unknown> => {
      if (!hasPrincipal(user, tokenOrgId, tokenTeamId)) return unauthorized(set);
      const runId = params["run_id"] ?? "";
      if ((await findAuthorizedRun(runId, user?.id, tokenOrgId ?? null, tokenTeamId ?? null, "run-read")) === undefined)
        return hidden(set);
      const query = queryValue(request, "q") || "troubleshooting";
      return {
        data: documentationMatches(query, parseLimit(queryValue(request, "limit"), 8)).map(
          (match): Record<string, unknown> =>
            resource(
              match.slug,
              "runbooks",
  ...
  .get("/api/v2/admin/maintenance-windows/preview", async ({ user, set }: ParamCtx): Promise<unknown> => {
    ...
    const windows = Array.isArray(settings["windows"])
      ? settings["windows"].map((window): unknown => {
          ...
          return {
            days: Array.isArray(value["days"])
              ? value["days"].filter(
                  (day): boolean => typeof day === "number" && Number.isInteger(day) && day >= 0 && day <= 6,
                )
              : [],
            "start-time": value["start-time"],
            "end-time": value["end-time"],
            timezone: value["timezone"],
          };
        })
      : [];
    return {
      data: resource("maintenance-windows", "maintenance-schedules", {
  ...
  // --- Notification snooze ----------------------------------------------
  .get(
    "/api/v2/notification-configurations/:nc_id/snooze",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> =>
      notificationSnoozeResponse(params["nc_id"] ?? "", user, tokenOrgId, tokenTeamId, set, "read"),
  )
  .get(
    "/api/v2/notification-configurations/:nc_id/actions/snooze",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> =>
      notificationSnoozeResponse(params["nc_id"] ?? "", user, tokenOrgId, tokenTeamId, set, "read"),
  )
  .post(
    "/api/v2/notification-configurations/:nc_id/snooze",
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> =>
      notificationSnoozeResponse(params["nc_id"] ?? "", user, tokenOrgId, tokenTeamId, set, "manage", body),
  )
  .post(
    "/api/v2/notification-configurations/:nc_id/actions/snooze",
    async ({ params, body, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> =>
  ...
  .delete(
    "/api/v2/notification-configurations/:nc_id/snooze",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, set }: ParamCtx): Promise<unknown> =>
      notificationSnoozeResponse(params["nc_id"] ?? "", user, tokenOrgId, tokenTeamId, set, "manage", {
        "duration-ms": 0,
      }),
  )
  .delete(
  ...
  .get(
    "/api/v2/variable-sets/:variable_set_id/impact",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, request, set }: ParamCtx): Promise<unknown> => {
      if (!hasPrincipal(user, tokenOrgId, tokenTeamId)) return unauthorized(set);
      ...
      for (const link of links) {
        ...
        const planned = await db.query.runs.findMany({
          where: and(
            eq(runs.workspaceId, workspace.id),
            inArray(runs.status, ["pending", "planning", "planned", "planned_and_saved", "apply_queued", "confirmed"]),
          ),
          orderBy: [desc(runs.createdAt)],
          limit: 20,
        ...
        consumers.push({
          workspace: { id: workspace.id, name: workspace.name },
          "last-observed-use": planned[0] === undefined ? null : safeIso(planned[0].createdAt),
          "planned-runs": planned.map(
            (run): Record<string, unknown> => ({
              id: run.id,
              status: run.status,
              createdAt: safeIso(run.createdAt),
              "input-policy": "run input remains bound to its captured version",
            }),
          ),
        });
      }
      return {
        data: resource(
          variableSet.id,
          "variable-set-impact-reports",
          {
            name: variableSet.name,
            consumers,
            "value-matching-used": false,
            "source-version-tracked": false,
            "future-runs-only": true,
            "external-credential-revocation-claimed": false,
            "controlled-overlap-supported": false,
          },
          { self: request.url },
        ),
      };
  ...
  .get(
    "/api/v2/workspaces/:workspace_id/secret-impact",
    async ({ params, user, orgId: tokenOrgId, teamId: tokenTeamId, request, set }: ParamCtx): Promise<unknown> => {
      if (!hasPrincipal(user, tokenOrgId, tokenTeamId)) return unauthorized(set);
      const workspace = await findAuthorizedWorkspace(
        params["workspace_id"] ?? "",
      ...
      const variables = await db.query.workspaceVariables.findMany({
        ...
        columns: { id: true, key: true, sensitive: true, description: true, category: true },
      });
      const runsForWorkspace = await db.query.runs.findMany({
        where: and(
          eq(runs.workspaceId, workspace.id),
          inArray(runs.status, ["pending", "planning", "planned", "planned_and_saved", "apply_queued", "confirmed"]),
        ),
      ...
      return {
        data: resource(
          workspace.id,
          "workspace-secret-impact-reports",
          {
            workspace: { id: workspace.id, name: workspace.name },
            sources: variables
              .filter((variable): boolean => variable.sensitive === true)
              .map(
            ...
            "planned-runs": runsForWorkspace.map(
              (run): Record<string, unknown> => ({
                id: run.id,
                status: run.status,
                createdAt: safeIso(run.createdAt),
                "input-policy": "captured input is not rewritten by source changes",
              }),
            ),
            "value-matching-used": false,
            "external-revocation-claimed": false,
          },
          { self: request.url },
        ),
      };
