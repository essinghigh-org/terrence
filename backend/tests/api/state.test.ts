...
describe("the reference format API v2 - State Versions & Locking", () => {
  ...
  beforeAll(async () => {
    ...
    const orgName = `homelab-state-${Date.now()}`;
  ...
  it("should fetch current state version (initially null/404 if not found depending on the reference format standard, but let's say 404 for no state)", async () => {
    const response = await app.handle(
