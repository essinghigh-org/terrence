...
test("manages CIDR lists and ranges through the JSON API", async () => {
  ...
  const fetchMock = mock(async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
    ...
    if (url === "/api/v2/organizations/acme/cidr-range-lists")
      return json({ data: [{ id: "list-1", attributes: { name: "Private" } }] });
    if (url.startsWith("/api/v2/cidr-ranges?")) return json({ data: ranges });
    if (url === "/api/v2/cidr-ranges" && init?.method === "POST") {
      ranges = [...ranges, { id: "range-2", attributes: { value: "192.168.0.0/16" } }];
      return json({ data: ranges[1]! }, 201);
    }
    if (url === "/api/v2/cidr-ranges/range-1" && init?.method === "DELETE") {
      ranges = [];
  ...
  await waitFor(() => {
    expect(view.getByText("10.0.0.0/8")).toBeTruthy();
  });
  ...
  await waitFor(() => {
    expect(view.getByText("192.168.0.0/16")).toBeTruthy();
  });
...
test("lists and creates workspace configuration versions", async () => {
  const fetchMock = mock(async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
    const url = urlOf(input);
    if (url === "/api/v2/workspaces/ws-1/configuration-versions" && init?.method === "POST")
      return json({ data: { id: "cv-2", attributes: { status: "pending", source: "tfe-api" } } }, 201);
    if (url.startsWith("/api/v2/workspaces/ws-1/configuration-versions"))
      return json({ data: [{ id: "cv-1", attributes: { status: "uploaded", source: "vcs" } }] });
    throw new Error(`Unexpected request: ${url}`);
  });
  globalThis.fetch = fetchMock as unknown as typeof fetch;
  const view = render(<WorkspaceConfigurationVersions workspaceId="ws-1" />);
  await waitFor(() => {
    expect(view.getByText("cv-1")).toBeTruthy();
  });
  fireEvent.click(view.getByRole("button", { name: "Prepare upload" }));
  await waitFor(() => {
    expect(view.getByText("cv-2")).toBeTruthy();
  });
});
...
test("removing a CIDR range requires typing the value (issue #588)", async () => {
  ...
  await waitFor(() => {
    expect(view.getByRole("heading", { name: "Remove CIDR range?" })).toBeTruthy();
  });
  // No request fires before confirmation.
  expect(
    fetchMock.mock.calls.some(
      ([input, init]) => urlOf(input) === "/api/v2/cidr-ranges/range-1" && init?.method === "DELETE",
    ),
  ).toBe(false);
  ...
  fireEvent.click(view.getByRole("button", { name: "Remove range" }));
  await waitFor(() => {
    expect(view.queryByText("10.0.0.0/8")).toBeNull();
  });
  expect(
    fetchMock.mock.calls.some(
      ([input, init]) => urlOf(input) === "/api/v2/cidr-ranges/range-1" && init?.method === "DELETE",
