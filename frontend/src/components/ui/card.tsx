...
type CardHeaderProps = DeepReadonly<React.ComponentProps<"div">> &
  Readonly<{ variant?: "default" | "section" | "danger" }>;
  ...
  return (
    <div
      ...
      className={cn(
        ...
        variant === "danger" &&
          "-mt-(--card-spacing) border-b border-destructive/30 bg-destructive/15 pt-(--card-spacing) text-destructive",
        className,
      )}
      // SAFETY: DeepReadonly is a compile-time-only structural wrapper; the props object is the primitive's own shape at runtime.
      {...(props as React.ComponentProps<"div">)}
...
function CardFooter({ className, ...props }: DeepReadonly<React.ComponentProps<"div">>): React.JSX.Element {
  return (
    <div
      ...
      className={cn("flex flex-wrap items-center gap-3 border-t bg-muted/30 p-(--card-spacing)", className)}
      // SAFETY: DeepReadonly is a compile-time-only structural wrapper; the props object is the primitive's own shape at runtime.
      {...(props as React.ComponentProps<"div">)}
    />
  );
