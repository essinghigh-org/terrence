...
describe("bounded subprocess supervision", () => {
  it("drains both streams concurrently", async () => {
    const output = await runBoundedProcess(
      script("process.stdout.write('o'.repeat(200000)); process.stderr.write('e'.repeat(200000))"),
  ...
  it("rejects noisy stderr and stdout overflow", async () => {
    await rejects(
      runBoundedProcess(script("setInterval(() => process.stderr.write('x'.repeat(10000)), 1)"), {
        maxStderrBytes: 1000,
      }),
      /byte limit/,
    );
    await rejects(
      runBoundedProcess(script("process.stdout.write('x'.repeat(10000))"), { maxStdoutBytes: 1000 }),
      /byte limit/,
    );
  });
